...
```bash
curl -s http://localhost:3100/v1/wallets/<WALLET_ID>/nfts/<CONTRACT_ADDRESS>/<TOKEN_ID>?network=ethereum-mainnet \
  -H 'Authorization: Bearer <session-token>'
```
...
**Action**: NFT를 자기 주소로 전송하는 dry-run을 실행한다. **메인넷이므로 가스비를 반드시 사전 확인한다.**
```bash

...
| **DESK-02** | 대시보드 + 승인/거부 인터페이스 + 세션/에이전트 관리 UI | 섹션 7 (기존 Admin Web UI 19페이지 + Desktop 전용 확장 3개) |
...
try {
  const status = await startDaemon({ port: 0 }); // port=0: OS가 빈 포트 할당 (섹션 4.2.1)
} catch (error: unknown) {
...
| 포트 충돌 | 동적 포트 할당으로 자동 해결 (섹션 4.2.1) | TCP bind(0) |

### 4.2.1 데몬 포트 동적 할당 프로토콜

> **(v33.0 추가)** Desktop 모드에서 데몬은 고정 포트 대신 OS가 할당하는 빈 포트를 사용한다.

**문제:** 데몬이 고정 포트(config.toml `[daemon].port`)를 사용하면 (1) Desktop 앱 다중 인스턴스 실행 시 충돌, (2) 다른 서비스와 포트 충돌이 발생할 수 있다.

**프로토콜:**

1. Desktop 모드에서 데몬은 `--port=0` (또는 config.toml `port = 0`)으로 시작
2. 데몬이 Node.js `net.createServer()`에서 TCP `bind(0)`으로 OS가 빈 포트를 할당
3. 할당된 포트를 두 가지 채널로 Sidecar Manager에 전달:
   - **stdout (primary):** 데몬 시작 시 `WAIAAS_PORT={port}` 형식의 한 줄을 stdout에 출력. Sidecar Manager가 stdout 파이프를 파싱
   - **tempfile (fallback):** `{data_dir}/daemon.port` 파일에 포트 번호 기록. stdout 파싱 실패 시 폴백으로 사용
4. Sidecar Manager가 포트를 수신하면 WebView URL을 `http://127.0.0.1:{port}/admin`으로 설정
...
sequenceDiagram
    ...
    SM->>D: spawn with --port=0
    D->>OS: TCP bind(0)
    OS-->>D: assigned port N
    D->>D: stdout: "WAIAAS_PORT=N"
    D->>D: write {data_dir}/daemon.port

    SM->>SM: parse stdout for WAIAAS_PORT=
    alt stdout 파싱 성공
        SM->>SM: port = N (from stdout)
    else stdout 파싱 실패
        SM->>SM: read {data_dir}/daemon.port
        SM->>SM: port = N (from file)
    end

    SM->>WV: navigate to http://127.0.0.1:N/admin
    SM->>SM: DaemonStatus.port = N
```

**SidecarManager.start() 포트 파싱 단계:**

기존 `start()` 메서드(섹션 4.3)의 step 2-3 사이에 포트 파싱 단계를 추가한다:

```
// 2. externalBin "waiaas-daemon" sidecar spawn (--port=0)
// 2.5. stdout 이벤트에서 WAIAAS_PORT={port} 파싱
//      - 정규식: /^WAIAAS_PORT=(\d+)$/
//      - 파싱 성공 시 self.port에 저장
//      - 10초 내 파싱 실패 시 {data_dir}/daemon.port 파일에서 읽기
// 3. stdout/stderr 이벤트 리스너 등록
```

**DaemonStatus.port 필드:** 기존의 `port: u16` 필드(섹션 4.3)가 동적으로 설정된다. `--port=0`으로 시작 시 실제 할당된 포트 번호로 업데이트된다.

**데몬 측 구현 (Node.js):**

```typescript
// 데몬 startup 시 포트 할당
const server = Hono.serve({ port: config.port }); // port=0 -> OS 할당
const actualPort = (server.address() as AddressInfo).port;
console.log(`WAIAAS_PORT=${actualPort}`); // stdout으로 Sidecar에 전달
writeFileSync(join(dataDir, 'daemon.port'), String(actualPort)); // tempfile fallback
```
...
```typescript
// packages/admin/src/desktop/hooks/useDaemon.ts
import { startDaemon, stopDaemon, restartDaemon, getDaemonStatus, getDaemonLogs } from '../utils/tauri-bridge'
...
fn main() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // 2. 자동 시작 (config에 따라, --port=0 동적 할당)
            let app_handle = app.handle().clone();
            tauri::async_runtime::spawn(async move {
                let manager = app_handle.state::<sidecar::SidecarManager>();
                let _ = manager.start(&app_handle).await;
                // 4. WebView URL 설정: http://127.0.0.1:{port}/admin
                //    포트는 SidecarManager에서 동적 할당 (섹션 4.2.1)
            });
...
| B. Daemon 직접 | WebView -> `localhost:{port}/admin` | X | 프로덕션과 동일 경로 | 코드 변경 시 Daemon 재시작 필요 |
...
Terminal 2: pnpm --filter @waiaas/daemon dev          # Daemon dev server (동적 포트, 섹션 4.2.1)
...
```typescript
// packages/admin/src/desktop/lib/wallet-connect.ts
...
export async function initializeWalletConnect(projectId: string) {
  ...
  appKitInstance = createAppKit({
    ...
    metadata: {
      ...
      url: `http://127.0.0.1:${await getDaemonPort()}`, // 동적 포트 (섹션 4.2.1)
      icons: ['https://waiaas.dev/icon.png'],
    },
    features: {
...
```typescript
// packages/admin/src/desktop/hooks/useWalletConnect.ts
...
export function useWalletConnect() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const port = await getDaemonPort() // IPC로 동적 포트 조회 (섹션 4.2.1)
      const res = await fetch(`http://127.0.0.1:${port}/v1/owner/settings`, {
...
```typescript
// packages/admin/src/desktop/hooks/useOwnerApi.ts
...
export function useOwnerApi() {
  ...
  async function ownerRequest(
    ...
    // 1. nonce 발급
    const port = await getDaemonPort() // IPC로 동적 포트 조회 (섹션 4.2.1)
    const nonceRes = await fetch(`http://127.0.0.1:${port}/v1/nonce`)
...
```typescript
// packages/admin/src/desktop/lib/notifications.ts
import {
  isPermissionGranted,
  requestPermission,
  sendNotification,
...
|-----------|-------------------|-----------|
| Approval Request | `/approvals` | Tauri 윈도우 포커스 + location.hash 변경 (Admin Web UI router) |
| Kill Switch | `/kill-switch` | Tauri 윈도우 포커스 + location.hash 변경 (Admin Web UI router) |
| Agent Stopped | `/agents` | Tauri 윈도우 포커스 + location.hash 변경 (Admin Web UI router) |
| Session Expiring | `/sessions` | Tauri 윈도우 포커스 + location.hash 변경 (Admin Web UI router) |
| Daemon Crashed | Dashboard (`/`) | Tauri 윈도우 포커스 |

**참고:** Tauri 2.x의 notification 플러그인은 클릭 이벤트를 지원한다. Rust 측에서 알림 클릭 이벤트를 수신하여 WebView에 이벤트를 전달하고, Admin Web UI의 라우터가 해당 페이지로 이동한다.
...
```typescript
// packages/admin/src/desktop/lib/updater.ts
...
```json
{
  "app": {
    "security": {
      "csp": "default-src 'self'; connect-src 'self' http://127.0.0.1:* wss://relay.walletconnect.com https://rpc.walletconnect.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:"
    }
  }
}
```
...
| `connect-src` | `'self' http://127.0.0.1:* wss://relay.walletconnect.com https://rpc.walletconnect.com` | 데몬 API (동적 포트, 섹션 4.2.1), WalletConnect Relay, Reown RPC |
...
async function killSwitch(password: string, reason: string): Promise<void> {
  try {
    const port = await getDaemonPort() // IPC로 동적 포트 조회 (섹션 4.2.1)
    await fetch(`http://127.0.0.1:${port}/v1/admin/kill-switch`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
| **DESK-02** | DONE | 7 (기존 Admin Web UI 19페이지, 상세는 설계 문서 67 참조) | 기존 Admin Web UI 19페이지 재사용 + Desktop 전용 확장 3개 (Setup Wizard, Sidecar Status, WalletConnect QR) |

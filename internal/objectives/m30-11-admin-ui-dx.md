...
- **R2-5.** 비활성 시 하위 UI 동작: 토글 아래에 비활성 안내 메시지를 표시하되, Registered Actions 테이블은 **읽기 전용으로 항상 표시** (운영자가 액션 목록·설명·tier를 미리 확인 가능). 탭(Identity/Registration File/Reputation)과 관련 관리 UI는 숨김. 활성화 시 전체 UI 렌더링 + 데이터 로드.

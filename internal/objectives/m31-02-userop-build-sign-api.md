...
- **R3-9.** DB에 트랜잭션 기록: `type='SIGN'`, `status='SIGNED'` (sign-only 파이프라인과 동일 — 원본 트랜잭션 타입이 아닌 `SIGN` 고정, callData에서 원본 의도를 파싱할 수 있으나 UserOp 서명의 특성상 sign-only와 일관되게 처리)
...
- **R4-4.** 만료된 build 레코드 주기적 정리: `WorkerScheduler`에 `userop-build-cleanup` 워커 등록 (기존 `session-cleanup` 1분 주기 패턴과 동일, `lifecycle/workers.ts`)
...
- **R6-2.** `UserOpBuildResponseSchema` — sender (`0x` hex), nonce (`0x` hex), callData (`0x` hex), factory? (`0x` hex, 미배포 시), factoryData? (`0x` hex, 미배포 시), entryPoint (`0x` hex), buildId (UUID v7)
...
- **R8-1.** v45 마이그레이션: `userop_builds` 테이블 생성 (현재 최신 v44)
...
- **R12-1.** MCP 도구: `build_userop` — 파라미터는 R2-2 `TransactionRequest` + `walletId` + `network`, 응답은 R2-3 `UserOpBuildResponse`
- **R12-2.** MCP 도구: `sign_userop` — 파라미터는 R3-2 `UserOpSignRequest` + `walletId`, 응답은 R3-3 `UserOpSignResponse`
- **R12-3.** SDK 메서드: `buildUserOp(walletId, request)` → `UserOpBuildResponse`, `signUserOp(walletId, request)` → `UserOpSignResponse` (REST API 1:1 래핑)

...
> **리서치 필수**: Across Protocol은 Intent 기반 브릿지로, 일반 lock-and-mint 브릿지와 메커니즘이 다르다. 구현 착수 전에 반드시 Across API 사양, SpokePool 컨트랙트 인터페이스(depositV3 파라미터, fillDeadline, exclusivityDeadline), Relayer 구조, 수수료 모델, 지원 토큰/체인 매트릭스, 상태 추적 API 등을 충분히 리서치하여 설계 문서에 반영해야 한다.
...
- **멀티체인**: Ethereum, Arbitrum, Optimism, Base, Polygon — WAIaaS 지원 체인과 대부분 겹침
...
### Phase 1: Across Protocol 리서치 및 설계

Across Protocol API와 컨트랙트를 심층 리서치하고 WAIaaS 아키텍처에 통합하는 설계 문서를 작성한다.
...
- fillDeadline / exclusivityDeadline 파라미터 전략
...
- 브릿지 상태 추적 통합 (폴링 or WebSocket)
- LI.FI Bridge와의 공존 전략 (사용자 선택 or 자동 라우팅)
- MCP 도구 / SDK 메서드 설계
- 정책 엔진 통합 (크로스체인 전송 한도)

### Phase 2: Across Bridge 구현

Across Protocol 브릿지 기능을 구현한다.

**기능:**
- 브릿지 견적 조회 (quote: 수수료, 예상 도착 시간, Relayer 가용성)
- 지원 라우트 조회 (출발/도착 체인 + 토큰 조합)
- 브릿지 실행 (ERC-20 approve + SpokePool depositV3)
- 네이티브 토큰 브릿지 (ETH → ETH 크로스체인)
- 브릿지 상태 추적 (deposit → fill 완료 확인)
- 브릿지 한도 조회 (최소/최대 금액)
- Action Provider 패턴 구현 (AcrossBridgeActionProvider)
- MCP 도구 + SDK 메서드

### Phase 3: 테스트 및 통합

통합 테스트와 기존 시스템과의 연동을 검증한다.

**기능:**
- Mock API 기반 단위 테스트
- SpokePool 컨트랙트 인코딩 검증
- 정책 엔진 연동 (크로스체인 전송 한도, 목적지 체인 화이트리스트)
- Admin UI 브릿지 현황 표시 (선택)
- 에러 핸들링 (liquidity 부족, route 미지원, timeout)
...
2. **상태 추적**: 브릿지 완료는 목적지 체인에서 Relayer fill이 발생해야 확인 가능. Across API 폴링 또는 deposit 이벤트 추적 필요.
...
5. **LI.FI 공존**: 사용자가 브릿지 프로토콜을 명시적으로 선택하거나, 견적 비교 후 자동 선택하는 전략 필요.

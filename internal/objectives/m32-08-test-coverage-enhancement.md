...
| admin | 87.65% | 80.35% | **71.37%** | 905 | functions 전 패키지 최저 — 95%까지 대량 추가 필요 |
| core | 96.59% | 91.24% | 94.62% | 872 | Lines/Branches 이미 달성, Functions +0.38%p |
| solana | 92.99% | 82.87% | 97.05% | 205 | Lines 달성, Branches +2.13%p |
| evm | 95.81% | **76.36%** | 95.91% | 255 | **Branches 최대 갭 (+8.64%p)** |
| cli | 81.45% | 79.34% | 98.07% | 216 | Lines +8.55%p, Branches +5.66%p |
| mcp | 90.71% | 84.82% | 96.66% | 276 | Lines 달성, Branches +0.18%p |
| wallet-sdk | 90.04% | **79.09%** | 100% | 49 | Branches +5.91%p |
| push-relay | 93.14% | 92.50% | 95.45% | 137 | 전 메트릭 이미 달성 — 임계값만 인상 |
| actions | 96.69% | 83.40% | 95.67% | 1,184 | Lines 달성, Branches +1.60%p |
| **shared** | **63.29%** | 100% | 100% | 11 | **임계값 미설정**, Lines 최대 갭 (+26.71%p) |
...
|--------|----------|-----------------|-------------------|-----------------|
| **Lines 90%** | core, evm, actions, push-relay, solana, mcp, wallet-sdk | — | admin(+2.35%p), sdk(+4.38%p), daemon(+4.83%p), cli(+8.55%p) | shared(+26.71%p) |
| **Branches 85%** | sdk, shared, push-relay, core | mcp(+0.18%p), actions(+1.60%p), solana(+2.13%p) | daemon(+3.92%p), admin(+4.65%p), cli(+5.66%p), wallet-sdk(+5.91%p) | evm(+8.64%p) |
| **Functions 95%** | wallet-sdk, cli, solana, mcp, evm, actions, push-relay | core(+0.38%p), daemon(+0.36%p) | sdk(+9.29%p) | admin(+23.63%p) |
...
shared는 신규 패키지(@waiaas/shared)로 상수/유틸 모듈이나 임계값이 미설정 상태. Lines 63.29%→90%는 대량 테스트 추가 필요.
...
|--------|------------------|--------------------|--------------------|----------|
| core | 96.59% → 유지 | 91.24% → 유지 | 94.62% → **95%** | 소 — Functions +0.38%p |
| actions | 96.69% → 유지 | 83.40% → **85%** | 95.67% → 유지 | 소 — Branches +1.60%p |
| evm | 95.81% → 유지 | 76.36% → **85%** | 95.91% → 유지 | **대** — Branches +8.64%p |
| push-relay | 93.14% → 유지 | 92.50% → 유지 | 95.45% → 유지 | 없음 — 임계값만 인상 |
| solana | 92.99% → 유지 | 82.87% → **85%** | 97.05% → 유지 | 소 — Branches +2.13%p |
| mcp | 90.71% → 유지 | 84.82% → **85%** | 96.66% → 유지 | 소 — Branches +0.18%p |
| wallet-sdk | 90.04% → 유지 | 79.09% → **85%** | 100% → 유지 | 중 — Branches +5.91%p |
| admin | 87.65% → **90%** | 80.35% → **85%** | 71.37% → **95%** | **대** — 전 메트릭 인상, Functions 최대 |
| sdk | 85.62% → **90%** | 95.27% → 유지 | 85.71% → **95%** | 중 — Lines +4.38%p, Functions +9.29%p |
| daemon | 85.17% → **90%** | 81.08% → **85%** | 94.64% → **95%** | **대** — 57,925 LOC, ~2,800줄 추가 커버 |
| cli | 81.45% → **90%** | 79.34% → **85%** | 98.07% → 유지 | 중 — Lines +8.55%p, Branches +5.66%p |
| shared | 63.29% → **90%** | 100% → 유지 | 100% → 유지 | 중 — Lines +26.71%p (LOC 소규모) |
...
evm Branches 76.36%→85%(최대 갭), wallet-sdk Branches 79.09%→85% 집중.
...
admin Functions 71.37%→95%(최대 난이도), cli Lines 81.45%→90%, cli Branches 79.34%→85%.
...
| shared 패키지 Lines | 유틸리티 함수, 상수 모듈 테스트 추가 | 15-20개 |
| solana/actions/mcp Branches | 나머지 소량 갭 해소 (각 0.18~2.13%p) | 10-15개 |
...
| 주요 작업 | daemon Lines 90%(+2,800줄), evm Branches 85%(+8.64%p), admin Functions 95%(+23.63%p) |
| 임계값 변경 | 전 패키지 Lines→90, Branches→85, Functions→95 통일 (기존 초과분 유지) |

---

*생성일: 2026-02-27*
*갱신일: 2026-03-16 — 통일 커버리지 기준(Lines 90%/Branches 85%/Functions 95%)으로 전면 개정. 5 Phase 체계로 확장, admin Functions/evm Branches 미커버 분석 추가, 임계값 전 패키지 동기화 목표 반영.*

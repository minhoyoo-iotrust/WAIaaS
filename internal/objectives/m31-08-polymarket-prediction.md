...
Polymarket CLOB 주문 시스템을 구현한다. Polygon 네트워크에서만 동작하도록 검증한다.
...
5. **Proxy Wallet**: Polymarket은 EOA와 별도의 Proxy Wallet을 사용 — WAIaaS 지갑과의 매핑 관계를 Phase 1에서 명확히 한다.
6. **Polygon 전용**: 모든 Polymarket 액션에서 Polygon 네트워크를 검증하고, 비-Polygon 네트워크 요청은 명확한 에러로 거부한다.
7. **API Key 관리**: Admin Settings에 Polymarket API 자격증명을 저장하여 런타임 변경 가능하게 한다. 기존 Admin Settings 패턴(Hyperliquid 등) 활용.
8. **마켓 해결 대기**: 시장이 해결될 때까지 포지션이 락인됨 — 상태 추적/알림 통합 고려.
9. **규제 고려**: 일부 지역에서 Polymarket 접근이 제한될 수 있음 — 사용자 책임 고지.

...
| WalletNotificationChannel | `waiaas-notify-` 하드코딩 + 지갑명 | O (`waiaas-notify-{name}`) | 표시 없음 |
| NtfySigningChannel | `signing_sdk.ntfy_request_topic_prefix` + 지갑명 | O (`waiaas-sign-{name}`) | 표시 없음 |
...
현재:
```typescript
// prefix + walletName 동적 조합 (sign-request-builder.ts:163)
...
현재:
```typescript
// 하드코딩 prefix + walletName 조합 (wallet-notification-channel.ts:133)

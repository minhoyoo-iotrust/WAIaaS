...
m29-02+ (포지션):  resolve() → ContractCallRequest → 실행 → 포지션 생성 → 모니터링 → 상환/청산
```
...
| 4 | Lending 프레임워크 과설계 | 첫 구현에서 과도한 추상화로 복잡도 증가 | Aave 구현과 함께 프레임워크를 만들어 실용성 검증. Kamino(m29-04) 통합 시 인터페이스 조정 허용 |

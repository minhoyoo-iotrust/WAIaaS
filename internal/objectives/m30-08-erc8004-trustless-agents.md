...
ERC-8004은 2026년 1월 이더리움 메인넷에 배포된 표준으로, AI 에이전트 간 사전 신뢰 없이 발견·상호작용·평가할 수 있는 최소 신뢰 레이어를 정의한다. MetaMask(Marco De Rossi), Ethereum Foundation(Davide Crapis), Google(Jordan Ellis), Coinbase(Erik Reppel) 공동 저작.
...
**connect-info 연동:**
등록 파일의 endpoint가 connect-info의 baseUrl과 일치하도록 자동 동기화. `buildConnectInfoPrompt()`에 ERC-8004 에이전트 ID 정보 포함.
...
기존 12개 PolicyType에 13번째 타입을 추가한다.
...
Stage 3 Policy 평가 순서:
  ...
  10. APPROVE_TIER_OVERRIDE  — 관리자 수동 티어 오버라이드
```

REPUTATION_THRESHOLD가 CONTRACT_WHITELIST 이후, SPENDING_LIMIT 이전에 위치하는 이유: 허용되지 않은 컨트랙트는 평판과 무관하게 차단해야 하고, 평판 기반 티어 결정은 지출 한도 평가 전에 이뤄져야 한다.
...
```sql
-- Migration v24: ERC-8004 agent identity tracking + approval_type 확장
-- LATEST_SCHEMA_VERSION: 23 → 24
...
-- 4. schema_version 기록
INSERT INTO schema_version (version, applied_at) VALUES (24, unixepoch());
```
...
| 버전 | 내용 | 의존 |
|------|------|------|
| v24 | agent_identities + reputation_cache 테이블 생성 + pending_approvals.approval_type 추가 | v23 (현재 최신: DeFi async tracking) |

LATEST_SCHEMA_VERSION을 23 → 24로 변경. REPUTATION_THRESHOLD PolicyType은 Zod SSoT(`PolicyTypeEnum`)에서 관리된다. **구현 시 확인 필요:** policies 테이블의 `type` 컬럼에 DB-level CHECK 제약이 존재하는 경우, v24 마이그레이션에서 해당 CHECK를 갱신해야 한다. SQLite는 ALTER TABLE로 CHECK 변경이 불가하므로 테이블 재생성(CREATE → INSERT → DROP → RENAME) 패턴이 필요할 수 있다. CHECK가 없으면 Zod SSoT만으로 충분.
...
packages/daemon/src/__tests__/
  ...
  infrastructure/database/
    migration-v24.test.ts              # DB 마이그레이션 테스트
...
| E18 | DB 마이그레이션 v23 → v24 무손실 | DB | L0 | O |
...
- **`skills/admin.skill.md`** — ERC-8004 설정 키 8개 + Admin UI 페이지 추가
...
현재 47개 → **52개**로 확장. `@waiaas/core` enums의 `NOTIFICATION_EVENT_TYPES` 배열에 추가.
...
| D4 | 정책 타입 추가 | 13번째 REPUTATION_THRESHOLD | 기존 12-type 구조에 최소 침습 추가 |
...
| D9 | PolicyType CHECK 제약 | Zod SSoT로 관리 (DB ALTER 불필요) | POLICY_TYPES 배열이 SSoT, DB CHECK는 Zod enum에서 파생 |
| D10 | 마이그레이션 버전 | v24 (v23은 DeFi async tracking에 사용됨) | LATEST_SCHEMA_VERSION=23 → 24 |
...
| ERC8004-07 | DB 마이그레이션 v24 | migrate.ts, schema.ts |

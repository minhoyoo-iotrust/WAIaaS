...
m30에서 7가지 기능의 인터페이스, 데이터 모델, 테스트 시나리오가 설계 수준에서 정의되었다. m30-01은 이 설계를 코드로 구현한다.
...
m30 설계 문서의 기술 결정을 그대로 따른다. 구현 시 추가 결정이 필요한 항목은 phase research에서 확정한다.

---

## E2E 검증 시나리오

m30에서 정의한 18개 핵심 검증 시나리오(T-01~T-18)와 6개 보안 시나리오(S-01~S-06)를 구현하여 자동화 테스트로 검증한다.
...
| 의존 대상 | 이유 |
|----------|------|
| m30 (운영 기능 확장 설계) | 7가지 기능의 인터페이스, 스키마, 테스트 시나리오 정의 |
| m20 (릴리스) | 코어 인프라 안정화 상태에서 운영 기능 추가 |

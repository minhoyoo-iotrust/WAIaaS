...
Validation Registry를 정책 엔진과 통합하여 고액 트랜잭션에 자동 검증을 요청하는 패턴. 이는 **별도 구현 마일스톤**으로 분리 가능하며, 기본 m28-06에서는 수동 `request_validation` 액션만 제공한다.
...
**설계 결정 — 자동 검증은 m28-06 범위 밖:**
자동 검증은 파이프라인 Stage 3과 Stage 4 사이에 새 단계를 삽입해야 하므로 복잡도가 높다. m28-06에서는 에이전트가 명시적으로 `request_validation` 액션을 호출하는 수동 방식만 구현하고, 자동 검증은 후속 마일스톤에서 다룬다.
...
```sql
-- Migration v23: ERC-8004 agent identity tracking
-- LATEST_SCHEMA_VERSION: 22 → 23
...
-- 3. schema_version 기록
INSERT INTO schema_version (version, applied_at) VALUES (23, unixepoch());
```
...
| 버전 | 내용 | 의존 |
|------|------|------|
| v23 | agent_identities + reputation_cache 테이블 생성 | v22 (현재 최신) |

LATEST_SCHEMA_VERSION을 22 → 23으로 변경. policies 테이블의 type CHECK 제약조건에 `'REPUTATION_THRESHOLD'` 추가 필요 (v23 마이그레이션에 포함).
...
packages/daemon/src/__tests__/
  ...
  infrastructure/database/
    migration-v23.test.ts              # DB 마이그레이션 테스트
...
| E18 | DB 마이그레이션 v22 → v23 무손실 | DB | L0 | O |
...
| D3 | setAgentWallet 서명 | 기존 ApprovalWorkflow 확장 | 알림 채널 재사용, approval_type 컬럼 추가로 EIP-712 지원 |
...
| D6 | 자동 검증 | m28-06 범위 밖 (수동만) | Stage 3.5 파이프라인 삽입은 복잡도 높음, 후속 마일스톤으로 분리 |
...
| ERC8004-07 | DB 마이그레이션 v23 | migrate.ts, schema.ts |

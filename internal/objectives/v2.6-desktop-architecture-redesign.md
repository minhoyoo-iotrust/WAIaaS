# 마일스톤 v2.6: Desktop App 아키텍처 재설계
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 1 | 설계 문서 39의 변경 섹션이 v2.6.1 objectives와 일치 | 문서 39의 아키텍처 → v2.6.1 컴포넌트 테이블 교차 검증 | [REVIEW] |
...
> **이연 사유**: Desktop App은 배포 채널이지 핵심 기능이 아님. 모든 코어 기능(월렛, 트랜잭션, 정책, 세션, 인증)은 CLI + REST API + Admin Web UI + MCP로 동작. v2.0은 npm/Docker/CLI 중심으로 릴리스하고, Desktop은 이후 진행.

---

*생성일: 2026-02-14*
*v2.0 이후로 이연: 2026-02-16 — Desktop은 배포 채널이므로 v2.0 릴리스 후 진행*
*선행: v1.3.2 (Admin Web UI)*
*후행: v2.6.1 (Tauri Desktop App 구현)*

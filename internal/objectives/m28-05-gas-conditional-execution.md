...
기존 8-state 머신에 GAS_WAITING을 추가하여 9-state로 확장:

```
PENDING → POLICY_EVALUATED → [GAS_WAITING] → QUEUED → SIGNED → SUBMITTED → CONFIRMED
                                                                            → FAILED
                                              → CANCELLED (타임아웃)
```

GAS_WAITING은 **Stage 2(정책 평가)와 Stage 3(가스 추정) 사이**에 위치한다. 정책 평가를 통과한 트랜잭션만 가스 대기에 진입하며, 가스 조건 충족 시 Stage 3부터 파이프라인을 재개한다.

| 상태 | 설명 |
|------|------|
| `GAS_WAITING` | 가스 조건 미충족으로 대기 중 (신규, Stage 2→3 사이) |

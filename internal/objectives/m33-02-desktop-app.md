...
| 신규 파일 | 12-18개 (Rust 5-7개, Admin Web UI 확장 5-8개, CI 워크플로우 1-2개, Tauri 설정 1개) |
| 테스트 | 20-30개 |
...
*최종 업데이트: 2026-03-28. GitHub Releases CI(tauri-action 3 플랫폼 빌드 매트릭스) + Tauri 자동 업데이트 범위 추가. E2E #12-15 추가(CI 빌드, 아티팩트 업로드, 자동 업데이트, 서명 검증). 기술 결정 #7-9 추가. waiaas.ai 다운로드 페이지 + Homebrew는 m33-03으로 분리.*
*이전: 2026-02-16. m20 이후로 이연 — Desktop은 배포 채널이므로 m20 릴리스 후 진행. Admin Web UI 재사용 아키텍처로 변경. 설계 문서 39 변경은 m31에서 수행.*

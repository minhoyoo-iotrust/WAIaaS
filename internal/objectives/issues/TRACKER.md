...
| 255 | ENHANCEMENT | MEDIUM | Pushwoosh API base URL 설정 가능하도록 개선 — 기본값 api.pushwoosh.com 변경 + config.toml api_url 옵션 | — | OPEN | — |
| 256 | ENHANCEMENT | HIGH | Smart Account 멀티체인 Factory 전환 (permissionless.js) — Solady factory 2개 체인 제한 해소 + 기존 AA 지갑 deprecation | v31.3 | OPEN | — |
| 257 | BUG | MEDIUM | Admin UI Drift Perp 활성화 상태가 항상 Inactive — BUILTIN_PROVIDERS key `drift_perp` vs DB key `drift` 불일치 | — | OPEN | — |
| 258 | BUG | HIGH | Admin 세션 Reissue가 무제한 세션을 만료로 판정 — expiresAt=0 가드 누락 + 헬퍼 함수 도입으로 재발 방지 | — | OPEN | — |

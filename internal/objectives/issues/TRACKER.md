...
| 353 | BUG | MEDIUM | Nightly 테스트 배지 ANSI 파싱 실패 — vitest ANSI 코드 미제거로 Gist 미갱신 | — | OPEN | — |
| 354 | BUG | HIGH | 기본 비활성 빌트인 프로바이더(Hyperliquid, Polymarket)가 Admin UI Actions 페이지에 미표시 — 활성화 불가 | — | OPEN | — |
| 355 | ENHANCEMENT | MEDIUM | Admin UI Actions 페이지 카테고리별 탭 분리 — 프로바이더 증가에 따른 UX 개선 | — | OPEN | — |
| 356 | ENHANCEMENT | LOW | 지갑 상세 Overview 탭 Owner Protection 섹션을 Balances 위로 이동 | — | OPEN | — |
| 357 | ENHANCEMENT | LOW | 대시보드 DeFi Positions 섹션이 포지션 없으면 숨겨짐 — 항상 표시로 변경 | — | OPEN | — |
| 358 | ENHANCEMENT | LOW | Hyperliquid, Polymarket, Across Bridge 기본 활성화로 변경 | — | OPEN | — |

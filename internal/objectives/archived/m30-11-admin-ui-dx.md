...
- **R1-5.** 라우트 리다이렉트 불필요 — Admin UI는 내부 전용이며 외부 링크·북마크 시나리오가 없으므로 구 라우트(`#/actions`, `#/erc8004`) 리다이렉트는 구현하지 않음
...
- **R2-5.** 비활성 시 하위 UI 동작: 토글 아래에 비활성 안내 메시지를 표시하되, Registered Actions 테이블은 **읽기 전용으로 항상 표시** (운영자가 액션 목록·설명·tier를 미리 확인 가능). Tier 드롭다운은 `disabled` 상태로 렌더링하여 변경 불가. 탭(Identity/Registration File/Reputation)과 관련 관리 UI는 숨김. 활성화 시 전체 UI 렌더링 + 데이터 로드.
...
- **R3-5.** DB 마이그레이션(v42) — DB에 10개 `_enabled` 설정이 **존재하지 않는 경우에만** `true`로 삽입 (`INSERT OR IGNORE`). 운영자가 의도적으로 `false`로 설정한 값은 존중한다. SettingsService는 DB 값 > config.toml > 코드 기본값 순으로 조회하므로, 코드 기본값만 변경하면 DB에 값이 없는 기존 사용자에게 적용되지 않음. 신규 설치는 코드 기본값(`true`)으로 동작.
...
키가 길어질 수 있으나(`actions.drift_perp_open_perp_position_tier` 등) 기능적 제약 없음.
...
| 파일 | 변경 내용 |
|------|----------|
| `packages/admin/src/app.tsx` | 해시 라우트 정의 변경 (`#/actions` → `#/defi`, `#/erc8004` → `#/agent-identity`) |
| `packages/admin/src/components/layout.tsx` | 메뉴 라벨·순서 변경 (Actions→DeFi, ERC-8004→Agent Identity, 인접 배치) |
...
| `packages/daemon/src/infrastructure/db/migrate.ts` | v42: 신규 사용자용 10개 `_enabled` 설정 INSERT OR IGNORE 마이그레이션 |
...
| `packages/admin/tests/` | 메뉴 변경·토글 통합·tier 드롭다운 UI 테스트 |
| `packages/daemon/tests/` | 기본값 변경에 따른 feature gate·등록·파이프라인 tier 오버라이드 테스트 |

...
| 4 | CLI 지원 | v2.5.2에서는 Admin UI만 | CLI에서의 패스워드 변경은 데몬 실행 중에만 가능하므로 Admin UI와 동일한 REST API 호출. 필요 시 추후 CLI 명령어 추가 |
...
| v2.1.2 (지갑 앱 알림 채널) | 패스워드 변경 후 지갑 앱 알림 인프라 활용 가능 |

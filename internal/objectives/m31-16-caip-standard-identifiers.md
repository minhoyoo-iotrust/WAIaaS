...
> **관련**: v31.15에서 추가된 `amountFormatted`/`symbol`/`decimals` 응답 필드와 동일 스키마에 `chainId`/`assetId` 병렬 추가. 공유 응답 스키마: `TxDetailResponseSchema` (`packages/core/src/schemas/transaction.schema.ts`), `BalanceResponseSchema`, `AssetResponseSchema`, `ActionResultSchema`
...
- **R2-1.** `assetId`가 제공되면 `address`, `decimals`, `symbol`을 optional로 전환. 구현 방식: `.superRefine()`으로 cross-field validation — `assetId` 존재 시 `address`/`decimals`/`symbol` optional, 미존재 시 기존 required 규칙 유지. `discriminatedUnion`은 discriminant 필드가 없으므로 부적합
- **R2-2.** `assetId`만 제공 시, 파이프라인 진입 전에 토큰 레지스트리에서 `address`, `decimals`, `symbol` 자동 resolve
- **R2-3.** 레지스트리에 없는 `assetId`는 CAIP-19에서 `chainId` + `address`를 파싱하여 네트워크와 주소를 자동 추출 (사용자가 `address`를 별도 제공할 필요 없음). `decimals`/`symbol`만 필수로 요구 (on-chain 조회는 범위 밖). **참고**: 트랜잭션 실행 컨텍스트(R2)에서는 정확한 단위 변환을 위해 `decimals`/`symbol` 필수이지만, 조회 전용 컨텍스트(R6 `resolve_asset`)에서는 메타데이터가 없으면 `null`을 반환한다 — 목적이 다르므로 의도된 차이
...
- **R2-6.** `assetId`가 제공되고 `network`가 미제공이면 `assetId`의 CAIP-2 chainId에서 네트워크 자동 추론. 추론은 **파이프라인 진입 전 preprocessing 단계**(route handler의 request parsing 시점)에서 수행하며, 추론된 네트워크는 요청 객체의 `network` 필드에 주입되어 파이프라인(`resolveWalletId` 3단계 우선순위, 정책 엔진 네트워크 매칭 등)에서 명시적 `network` 지정과 동일하게 동작
- **R2-7.** `assetId`에서 네트워크 자동 추론이 가능한 엔드포인트에서는 `network`를 conditional optional로 전환. 구현 방식: 기존 `network` required 스키마를 `.superRefine()`으로 교체하여 `assetId` 존재 시 `network` 생략 허용, `assetId` 미존재 시 기존 required 규칙 유지. 적용 대상: `POST /transactions` (TOKEN_TRANSFER, APPROVE), `POST /actions/:provider/:action` 등 `TokenInfoSchema`를 사용하는 엔드포인트

### R3. 응답 — CAIP 필드 항상 포함

모든 API 응답에서 네트워크와 자산 식별자에 CAIP 필드를 항상 포함한다.

- **R3-1.** 네트워크를 포함하는 모든 응답에 `chainId` (CAIP-2) 필드 추가. 대상 응답: 잔액(`GET /balance`), 자산 목록(`GET /assets`), 트랜잭션(`GET /transactions`, `POST /transactions`), NFT(`GET /nfts`), DeFi 포지션(`GET /defi/positions`), 수신 트랜잭션(`GET /incoming`), Action 실행 결과. `connect-info`의 CAIP-2는 R3-6(`supportedChainIds[]`)에서 별도 처리 — 지갑 전체 정보이므로 단일 `chainId`가 아닌 배열 형태. 알림 이벤트 payload는 범위 밖 (내부 소비 전용, CAIP 변환은 소비자 측에서 필요 시 수행)
- **R3-2.** 토큰/자산을 포함하는 모든 응답에 `assetId` (CAIP-19) 항상 포함 — optional에서 항상 생성으로 변경. **구현 방식**: v31.15의 `amountFormatted`와 동일하게 **런타임 동적 생성** — DB에 저장하지 않고 응답 직렬화 시점에 `networkToCaip2()`/`tokenAssetId()` 호출로 생성. 기존 DB 레코드에 대해서도 동일하게 동작하므로 DB 마이그레이션 불필요
...
- **R4-2.** `sendToken()` 등 토큰 관련 메서드에서 `{ assetId }` 단독 전달 지원. 구현 방식: 토큰 파라미터 타입을 union으로 확장 — `{ address, decimals, symbol }` (기존) | `{ assetId }` (신규). `assetId` 제공 시 `address`/`decimals`/`symbol`은 optional이 됨 (`Partial` 적용)
- **R4-3.** 응답 타입에 `chainId` (CAIP-2), `assetId` (CAIP-19) 필드 추가
- **R4-4.** 기존 메서드 시그니처는 그대로 유지 — 추가 union 타입으로 확장 (overload보다 union 선호: 파라미터 조합이 2개이므로 union이 더 간결)
...
### R6. MCP `resolve_asset` 도구 신규 생성

새 MCP 도구를 추가하여 AI 에이전트가 CAIP-19 식별자로 토큰 메타데이터를 조회할 수 있게 한다. **신규 파일: `packages/mcp/src/tools/resolve-asset.ts`**

- **R6-1.** 새 MCP 도구 `resolve_asset`: CAIP-19 → 토큰 메타데이터(address, decimals, symbol, name) 조회
- **R6-2.** 입력: `assetId` (CAIP-19 문자열). 출력: `{ address, decimals, symbol, name, network, chainId, isNative, isRegistered }`
- **R6-3.** 내부 구현은 기존 `GET /v1/tokens/resolve` 엔드포인트(on-chain ERC-20 메타데이터 조회, `network` + `address` 파라미터)의 CAIP-19 기반 래퍼 — assetId에서 network/address를 파싱하여 기존 엔드포인트에 위임. 토큰 레지스트리에 등록된 토큰은 on-chain 조회 없이 레지스트리에서 즉시 반환
- **R6-4.** 미등록 자산의 경우 CAIP-19에서 network + address를 파싱하여 `isRegistered: false`로 반환, decimals/symbol은 `null`

### R7. OpenAPI 스펙 업데이트

- **R7-1.** 네트워크 파라미터 스키마에 CAIP-2 예시 추가
- **R7-2.** TokenInfo 스키마에 `assetId`-only 사용 패턴 문서화
- **R7-3.** 응답 스키마에 `chainId`, `assetId` 필드 추가
- **R7-4.** CAIP-2/CAIP-19 형식 설명 및 예시를 API 문서 상단에 기재

### R8. Skill 파일 동기화

- **R8-1.** `skills/*.skill.md` 파일에 CAIP-2/19 사용법 추가
- **R8-2.** 네트워크 지정 예시에 CAIP-2 형식 병기
- **R8-3.** 토큰 전송 예시에 `assetId` 단독 사용 패턴 추가

### R9. 테스트

- **R9-1.** `normalizeNetworkInput` CAIP-2 변환 테스트: 15개 네트워크 매핑 전수 검증
- **R9-2.** CAIP-2 + plain string + legacy 혼용 입력 테스트 (우선순위 검증)
- **R9-3.** `TokenInfo` assetId-only 입력 → 레지스트리 resolve 테스트
- **R9-4.** `assetId`에서 네트워크 자동 추론 테스트
- **R9-5.** `assetId` vs `network` 불일치 validation error 테스트
- **R9-6.** 미등록 assetId + address/decimals/symbol 폴백 테스트
- **R9-7.** 모든 응답 스키마에 `chainId`/`assetId` 포함 검증 테스트
- **R9-8.** SDK CAIP-2 네트워크 입력 테스트
- **R9-9.** MCP 도구 CAIP-2 네트워크 입력 + assetId 단독 전달 테스트
- **R9-10.** `resolve_asset` MCP 도구 테스트
...
| `packages/daemon/src/api/routes/balance.ts` | 잔액 응답에 `chainId` (CAIP-2) 필드 추가 (R3-1) |
| `packages/daemon/src/api/routes/connect-info.ts` | `supportedChainIds` 배열 추가 (R3-6) |
...
| `packages/mcp/src/tools/resolve-asset.ts` | 신규 — `resolve_asset` MCP 도구 (R6) |

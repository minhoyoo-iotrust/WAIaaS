...
기존 PositionTracker 인프라와 Admin Dashboard DeFi 섹션을 활용하여, **미구현 상태인 5개 DeFi 프로바이더의 `getPositions()` 로직을 구현**하고 PositionTracker에 연결한다. Admin Dashboard에서 스테이킹, 렌딩, Yield, Perp 등 모든 DeFi 예치 포지션을 카테고리별·프로바이더별로 조회할 수 있도록 완성한다.
...
v28.4~v31.4에 걸쳐 12개 DeFi ActionProvider를 구현했으나, 포지션 추적(`getPositions()`)은 Kamino(Solana Lending)와 Drift(Solana Perp)만 완성되어 있다. 나머지 프로바이더는 빈 배열을 반환하거나 IPositionProvider를 구현하지 않았다.
...
| Drift | PERP | ✅ 구현 완료 (SDK 래핑) | ✅ 연결됨 |
| Aave V3 | LENDING | ❌ 빈 배열 반환 | ✅ 연결됨 (duck-type) |
| Pendle | YIELD | ❌ 빈 배열 반환 | ✅ 연결됨 (duck-type) |
| Lido | STAKING | ❌ IPositionProvider 미구현 | ❌ 미연결 |
| Jito | STAKING | ❌ IPositionProvider 미구현 | ❌ 미연결 |
| Hyperliquid | PERP/SPOT | ❌ IPositionProvider 미구현 | ❌ 미연결 |
...
- **R1-4.** PositionTracker에 자동 등록 (duck-type 감지: `getPositions` + `getSupportedCategories` + `getProviderName` 메서드 존재 시)
...
- **R2-4.** PositionTracker에 자동 등록 (duck-type 감지)
...
### R5. Hyperliquid Perp/Spot 포지션 구현

- **R5-1.** `HyperliquidPerpProvider`에 `IPositionProvider` 인터페이스 신규 구현
- **R5-2.** `getPositions()` 신규 작성: Hyperliquid Info API로 오픈 포지션 조회
...
- **R5-3.** `HyperliquidSpotProvider`에 `IPositionProvider` 인터페이스 신규 구현 (spot 잔액 포지션)
- **R5-4.** PositionTracker에 자동 등록 (duck-type 감지)

### R6. Admin Dashboard UX 개선

- **R6-1.** 카테고리별 필터 탭: STAKING / LENDING / YIELD / PERP / ALL
- **R6-2.** 프로바이더별 그룹핑: 같은 프로바이더 포지션을 묶어서 표시 (예: Aave V3 → Supply 3건, Borrow 1건)
- **R6-3.** 포지션 상세 정보 표시: metadata 기반으로 카테고리별 맞춤 UI
...
- **R6-4.** Health Factor 글로벌 경고: 전체 지갑 중 worst health factor가 임계값 미만일 때 대시보드 상단 경고 배너
- **R6-5.** 지갑별 필터: 특정 지갑의 DeFi 포지션만 조회
- **R6-6.** 자동 새로고침: 30초 주기 (기존 대시보드 패턴 유지)

### R7. 테스트

- **R7-1.** 각 프로바이더 `getPositions()` 단위 테스트 (모킹된 RPC/API 응답 기반)
- **R7-2.** PositionTracker 통합 테스트: 신규 프로바이더 등록 후 동기화 확인
- **R7-3.** Admin API 테스트: 카테고리별 필터링, 집계 정확성
- **R7-4.** Admin UI 컴포넌트 테스트: 필터 탭, 그룹핑, 경고 배너
...
- Hyperliquid: 거래소 마켓 데이터 (Info API)

별도의 통합 가격 서비스 구축은 범위 밖.

### D3. Hyperliquid 포지션은 완전 신규 구현

Drift와 달리 Hyperliquid는 `getPositions()` 메서드가 존재하지 않는다. Hyperliquid Info API를 직접 호출하여 오픈 포지션과 spot 잔액을 조회하는 로직을 신규 작성한다.
...
### D5. PositionProvider 등록은 duck-type 자동 감지

daemon 부팅 시 ActionProviderRegistry의 모든 프로바이더를 순회하며 `getPositions` + `getSupportedCategories` + `getProviderName` 메서드가 존재하면 자동으로 PositionTracker에 등록한다. 별도의 수동 등록 로직 불필요.
...
| 파일/영역 | 변경 내용 |
|----------|----------|
| `packages/actions/src/providers/lido-staking/index.ts` | IPositionProvider 구현, getPositions() 추가 |
| `packages/actions/src/providers/jito-staking/index.ts` | IPositionProvider 구현, getPositions() 추가 |
...
| `packages/actions/src/providers/hyperliquid/perp-provider.ts` | IPositionProvider 신규 구현, getPositions() 추가 |
| `packages/actions/src/providers/hyperliquid/spot-provider.ts` | IPositionProvider 신규 구현, getPositions() 추가 |
...
| Drift 포지션 구현 | 이미 getPositions() 구현 완료 + PositionTracker 연결됨 | 테스트 보강은 R7에 포함 |

...
```
AI 에이전트: "ETH를 USDC로 스왑해줘. 단, 가스비가 30 gwei 이하일 때"

1. 트랜잭션 제출 (gasCondition: { maxGasPrice: "30000000000" })
2. 파이프라인 Stage 3(정책 평가) 통과 후, 가스 조건 평가에서 대기
3. 가스 조건 충족 시 → Stage 4(대기/승인) → Stage 5(실행) 진행
...
현재 11-state 머신에 이미 포함된 GAS_WAITING 상태를 활용 (v23 마이그레이션에서 추가됨):

```
PENDING → EXECUTING → [GAS_WAITING] → QUEUED → SIGNED → SUBMITTED → CONFIRMED
                                                                      → FAILED
                                       → CANCELLED (타임아웃)
```

GAS_WAITING은 **Stage 3(정책 평가)과 Stage 4(대기/승인) 사이**에 위치한다. 정책 평가를 통과한 트랜잭션만 가스 대기에 진입하며, 가스 조건 충족 시 Stage 4부터 파이프라인을 재개한다.

| 상태 | 설명 |
|------|------|
| `GAS_WAITING` | 가스 조건 미충족으로 대기 중 (기존 상태 활용, Stage 3→4 사이) |
...
packages/core/src/
  ...
  enums/
    transaction.ts                # GAS_WAITING 상태 이미 존재 (v23 마이그레이션)
...
| # | 결정 항목 | 선택지 | 결정 근거 |
|---|----------|--------|----------|
| 1 | 가스 조건 평가 시점 | 정책 평가(Stage 3) 통과 후 | 정책 위반 트랜잭션은 가스 대기 없이 즉시 거부. 불필요한 대기 방지 |
...
| 수정 파일 | 8-12개 |
| 테스트 | 12-16개 |
| DB 마이그레이션 | 없음 — GAS_WAITING 상태는 m28-03 통합 마이그레이션에서 이미 추가됨 (m28-00 DEFI-04 설계에 따라 bridge_status + GAS_WAITING을 단일 마이그레이션으로 실행) |

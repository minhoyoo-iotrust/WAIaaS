...
지갑으로 들어오는 수신 트랜잭션을 실시간 감지하여 DB에 기록하고, AI 에이전트가 입금 이벤트에 자율적으로 반응할 수 있는 인프라를 **설계 수준에서** 정의한다.

## 배경

### 현재 상태

WAIaaS는 AI 에이전트가 **보내는 트랜잭션만** 관리한다:

```
AI 에이전트 요청 → 파이프라인 6-stage → 온체인 전송 → transactions 테이블 기록
```

외부에서 지갑으로 들어오는 입금은 WAIaaS를 거치지 않으므로 DB에 기록되지 않는다. 잔액 조회(`getBalance()`)는 체인 RPC 실시간 호출이라 입금이 반영되지만, **언제 누가 얼마를 보냈는지**는 추적할 수 없다.
...
| AI 에이전트 반응 | 입금에 반응하려면 잔액 반복 조회 필요 | 이벤트 기반 자율 행동 불가 |
| 보안 | 송신 패턴만 감시 | 의심스러운 입금(dust attack 등) 미감지 |

### 설계 원칙

1. **Opt-in** — 기본 비활성. 지갑별로 활성화하며, 비활성 지갑은 RPC 비용 0
...
지갑 주소의 수신 트랜잭션을 실시간 감지하는 핵심 서비스를 설계한다.
...
| 활성화 | 지갑별 opt-in (`monitor_incoming` 설정) |
| 감지 대상 | 네이티브 토큰 입금 (SOL, ETH), SPL/ERC-20 토큰 입금 |
| 저장 | `incoming_transactions` 테이블 (기존 `transactions`와 별도) |
| 이벤트 | `INCOMING_TX_DETECTED` 이벤트 발생 → 알림 + AI 에이전트 반응 가능 |
| 라이프사이클 | 데몬 시작 시 활성 지갑의 구독 등록, 지갑 추가/삭제 시 동적 구독 관리 |
...
| 연결 공유 | 같은 체인의 여러 지갑이 하나의 WebSocket 연결 공유 |
| 배치 조회 | 폴링 모드에서 여러 지갑 주소를 한 번에 조회 |
...
AI 에이전트가 발생시킨 TX        외부에서 들어온 TX
정책 평가, 승인 흐름 포함        정책 평가 없음
wallet_id = 송신 지갑            wallet_id = 수신 지갑
```
...
- 인덱스 설계 (wallet_id, chain, confirmed_at, from_address)
...
| 인증 | sessionAuth (세션에 연결된 지갑의 수신 이력만 조회) |
...
- cursor pagination (기존 트랜잭션 이력 조회와 동일 패턴)
- SDK 메서드: `client.listIncomingTransactions(filters)`
- MCP 도구: `list_incoming_transactions` (21번째 도구, 현재 20개)
...
입금 감지 시 기존 알림 인프라와 연동하여 Owner에게 통보하고, AI 에이전트가 이벤트에 반응할 수 있는 구조를 설계한다.
...
기존 28개 NotificationEventType에 2개 추가 → 30개.
...
| INCOMING_TX_SUSPICIOUS | `*Suspicious Incoming Transaction*` | `*의심스러운 수신 트랜잭션*` |
...
- 알림 메시지 템플릿 (en/ko)
- WalletNotificationChannel 카테고리 확장 (수신 TX 카테고리 추가)
...
POST /v1/wallets
{
  "name": "trader-bot",
  "chain": "solana",
  "monitor_incoming": true    // 이 지갑만 수신 모니터링 활성화
}
```

전역 `incoming_enabled = true` + 지갑별 `monitor_incoming = true` 두 조건 모두 충족해야 해당 지갑의 수신을 모니터링한다.

#### 5.3 설계 산출물

- config.toml `[incoming]` 섹션 키 정의 (6개, 기존 10개 섹션에 1개 추가 → 11개)
- 지갑 생성/수정 API 필드 확장 (`monitor_incoming`)
- `wallets` 테이블 컬럼 추가 (`monitor_incoming` BOOLEAN DEFAULT 0, 기존 15개 → 16개)
...
| 문서 | 영향 범위 | 변경 규모 |
|------|----------|----------|
| 25 (sqlite) | `incoming_transactions` 테이블 추가, `wallets` 컬럼 추가 | 중 |
| 27 (chain-adapter) | IChainSubscriber 인터페이스 추가 (IChainAdapter 22메서드 확장 아님, 별도 계층) | 중 |
...
| 35 (notification) | INCOMING_TX_DETECTED, INCOMING_TX_SUSPICIOUS 이벤트 2개 추가 (28 → 30) | 소 |
| 37 (rest-api) | 엔드포인트 2개 추가 (incoming, incoming/summary) | 소 |
| 38 (sdk-mcp) | `listIncomingTransactions()` 메서드 + MCP 도구 1개 추가 (20 → 21) | 소 |
| 75 (notification-push-relay) | WalletNotificationChannel 수신 TX 카테고리 추가 | 소 |
...
| ID | 산출물 | 설명 |
|----|--------|------|
| ITM-01 | IncomingTransactionMonitor 설계 스펙 | IChainSubscriber, WebSocket/폴링 전략, 연결 관리, 지갑별 구독 |
...
| ITM-05 | 설정 구조 설계 | config.toml `[incoming]` 섹션, 지갑별 opt-in, 환경변수 매핑 |
...
| T-05 | 구독 | 지갑 동적 추가/삭제 | 런타임 지갑 생성 → 구독 추가, 삭제 → 구독 해제 | Integration |
...
| T-16 | 설정 | 지갑별 활성화 | 2개 지갑 중 1개만 활성 → 1개만 구독 등록 | Integration |
| T-17 | 비용 | 연결 공유 | 같은 체인 지갑 3개 → WebSocket 연결 1개 | Unit |
...
| S-04 | API | 타 지갑 이력 조회 | sessionAuth 지갑 A가 지갑 B의 수신 이력 접근 불가 |
...
──────────                        ──────────────────────────
IChainAdapter 22메서드 (27)   →   IChainSubscriber 별도 인터페이스
...
v2.7 (지갑 앱 알림 채널)          m27 (이 문서)
──────────────────────            ──────────────────
WalletNotificationChannel     →   수신 TX 카테고리 추가
Push Relay (75)               →   수신 알림 릴레이 활용
28개 NotificationEvent        →   2개 이벤트 추가 (→ 30개)
MCP 20개 도구                 →   list_incoming_transactions 추가 (→ 21개)
```
...
6. WebSocket 연결 공유 전략이 지갑 수 증가에 따른 비용 선형 증가를 방지함
...
*작성: 2026-02-09*
*갱신: 2026-02-21 — v2.7 기준 용어(agent→wallet), 수치, 선행 참조 갱신*

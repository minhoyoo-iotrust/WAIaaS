...
| EVM Calldata 인코딩 | ABI + 함수명 + 파라미터를 받아 인코딩된 calldata hex를 반환하는 유틸리티. REST API(`POST /v1/utils/encode-calldata`), TS SDK(`encodeCalldata()`), Python SDK(`encode_calldata()`), MCP 도구(`encode_calldata`)로 제공. 내부적으로 viem `encodeFunctionData()` 사용. AI 에이전트가 ABI를 알고 있으면 CONTRACT_CALL에 필요한 calldata를 직접 생성 가능 |
| MCP 스킬 리소스 | MCP `resources/list` + `resources/read`로 `skills/*.skill.md` 파일을 노출. 에이전트가 필요 시 온디맨드로 API 레퍼런스를 조회할 수 있다. 리소스 URI: `waiaas://skills/{name}` (예: `waiaas://skills/quickstart`) |
...
packages/daemon/src/
  ...
  routes/
    ...
    utils.ts                     # POST /v1/utils/encode-calldata 엔드포인트
  utils/
    encode-calldata.ts           # viem encodeFunctionData() 래퍼
...
packages/sdk/src/
  client.ts                      # signTransaction(), encodeCalldata() 메서드 추가

packages/mcp/src/
  tools.ts                       # sign_transaction, encode_calldata 도구 추가
  resources.ts                   # skills 리소스 핸들러 (resources/list, resources/read)

python-sdk/src/waiaas/
  client.py                      # sign_transaction(), encode_calldata() 메서드 추가
```
...
| POST | /v1/utils/encode-calldata | sessionAuth | ABI + 함수명 + 인자로 EVM calldata 인코딩 |
...
EVM 전용. ABI와 함수명, 인자를 받아 인코딩된 calldata hex를 반환한다. 내부적으로 viem `encodeFunctionData()`를 사용한다.
...
```json
{
  ...
  "args": ["0x1234567890abcdef1234567890abcdef12345678", "1000000000000000000"]
}
```

- `abi`: ABI JSON 배열 (전체 ABI 또는 해당 함수만)
- `functionName`: 호출할 함수명
- `args`: 함수 인자 배열 (Solidity 타입에 맞는 문자열/숫자)

##### 응답 (성공)

```json
{
  "calldata": "0xa9059cbb0000000000000000000000001234567890abcdef1234567890abcdef1234567800000000000000000000000000000000000000000000000000000000000f4240",
  "selector": "0xa9059cbb",
  "functionName": "transfer"
}
```
...
```json
{
  ...
  "message": "Function 'transfer' not found in provided ABI"
}
```
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 34 | ERC-20 transfer calldata 인코딩 | POST /v1/utils/encode-calldata { abi: ERC20_ABI, functionName: 'transfer', args: [to, amount] } → calldata 시작 0xa9059cbb assert | [L0] |
| 35 | ERC-20 approve calldata 인코딩 | POST /v1/utils/encode-calldata { abi: ERC20_ABI, functionName: 'approve', args: [spender, amount] } → calldata 시작 0x095ea7b3 assert | [L0] |
| 36 | 복합 ABI 함수 인코딩 (tuple 파라미터) | POST /v1/utils/encode-calldata { abi: COMPLEX_ABI, functionName: 'execute', args: [...] } → valid hex calldata 반환 assert | [L0] |
| 37 | 존재하지 않는 함수명 → ABI_ENCODING_FAILED | POST /v1/utils/encode-calldata { abi: ERC20_ABI, functionName: 'nonexistent' } → ENCODE_ERROR assert | [L0] |
| 38 | 인자 타입 불일치 → ABI_ENCODING_FAILED | POST /v1/utils/encode-calldata { abi: ERC20_ABI, functionName: 'transfer', args: ['not-address'] } → ENCODE_ERROR assert | [L0] |
| 39 | TS SDK encodeCalldata() → 데몬 호출 + 결과 반환 | WAIaaSClient.encodeCalldata({...}) → POST /v1/utils/encode-calldata 호출 + calldata 반환 assert | [L0] |
| 40 | Python SDK encode_calldata() → 데몬 호출 + 결과 반환 | client.encode_calldata({...}) → POST /v1/utils/encode-calldata 호출 assert | [L0] |
| 41 | MCP encode_calldata 도구 → 도구 목록 + 호출 성공 | MCP tools/list → encode_calldata 포함 assert + callTool 성공 assert | [L0] |
| 42 | encode → CONTRACT_CALL 연쇄 테스트 | encode_calldata로 calldata 생성 → POST /v1/transactions/send (type: CONTRACT_CALL, data: calldata) → 정상 처리 assert | [L0] |

### MCP 스킬 리소스

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 43 | resources/list에 스킬 목록 포함 | MCP resources/list → waiaas://skills/quickstart 등 5개 URI 포함 assert | [L0] |
| 44 | resources/read로 스킬 내용 조회 | MCP resources/read { uri: 'waiaas://skills/transactions' } → 마크다운 내용 반환 assert | [L0] |
| 45 | 존재하지 않는 리소스 → 에러 | MCP resources/read { uri: 'waiaas://skills/nonexistent' } → ResourceNotFound 에러 assert | [L0] |

### 에러 처리

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 46 | 잘못된 base64/hex → 파싱 에러 | POST /v1/transactions/sign { transaction: "invalid" } → INVALID_TRANSACTION 에러 assert | [L0] |
| 47 | 월렛이 서명자에 포함되지 않음 → 에러 | Solana tx의 feePayer가 다른 pubkey → WALLET_NOT_SIGNER 에러 assert | [L0] |
| 48 | 세션 없이 요청 → 401 | sessionAuth 없이 요청 → 401 assert | [L0] |
| 49 | 잘못된 chain 파라미터 → 400 | chain='bitcoin' → UNSUPPORTED_CHAIN 에러 assert | [L0] |
| 50 | rawTx 크기 초과 → 413 | Solana MTU 초과 또는 EVM 상한 초과 → TRANSACTION_TOO_LARGE assert | [L0] |

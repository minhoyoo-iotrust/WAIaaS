...
2. **REST API / MCP / SDK 확장** → SIG-02 (connect-info capabilities 포함)
...
**리서치에 의해 변경 가능한 설계 항목:**

| 설계 항목 | 현재 가정 | 리서치 항목 | 변경 시 영향 |
|-----------|----------|------------|-------------|
| keyid 형식 (ENG-02, SIG-04) | `erc8128:<chainId>:<address>` | #4 | keyid.ts, 서명 플로우, 검증 유틸리티 |
| alg 식별자 (ENG-05) | `eip191-secp256k1` | #6 | constants.ts, Signature-Input 빌더 |
| Nonce 필수 여부 (ENG-04) | 기본 활성 + 선택적 비활성화 | #5 | SignatureParamsSchema, Admin Settings |
| Covered Components 명칭 | RFC 9421 §2 기반 | #2 | 프리셋 정의, 빌더 |

> 리서치 결과가 현재 가정과 일치하면 설계를 그대로 유지하고, 상충 시 리서치 결과를 반영하여 본 문서를 업데이트한 후 구현에 진입한다.
...
| 422 | `EVM_NETWORK_REQUIRED` | 지정된 network가 EVM 체인이 아님 (walletId + network 조합에서 EVM 네트워크 필요) |

#### 2.2 API-02: 요청·응답 시퀀스 및 파이프라인 경로

**설계 결정 — 전용 라우트 + 파이프라인 바이패스:**
`POST /v1/erc8128/sign`은 기존 6-stage 파이프라인을 거치지 않는 **전용 라우트**이다. 이유: (1) ERC-8128은 트랜잭션이 아니라 HTTP 메시지 서명이므로 Stage 1(파싱)~Stage 6(제출) 흐름에 맞지 않음, (2) SIGN 파이프라인은 `signMessage(arbitrary_data)` 용도이며 RFC 9421 서명 구성 로직이 필요한 ERC-8128과는 처리 흐름이 다름. 대신 **정책 평가(ERC8128_ALLOWED_DOMAINS)는 라우트 핸들러에서 직접 호출**하여 동일한 보안 수준을 보장한다. D12의 "SIGN 타입 재사용"은 discriminatedUnion에 새 타입을 추가하지 않는다는 의미이며, 파이프라인 경유를 의미하지 않는다.
...
AI 에이전트                    WAIaaS 데몬                    외부 API 서버
    ...
    |                              |                              |
    |              ┌─ 전용 라우트 핸들러 ─┐                        |
    |              │ 1. erc8128.enabled 확인                       |
    |              │ 2. 지갑 EVM 체인 검증                         |
    |              │ 3. 정책 직접 평가                              |
    |              │    ERC8128_ALLOWED_DOMAINS                     |
    |              │ 4. Content-Digest 생성                        |
    |              │ 5. Signature-Input 구성                       |
    |              │ 6. Signature Base 구성                        |
    |              │ 7. signMessage(signatureBase)                 |
    |              │ 8. 응답 헤더 조립                             |
    |              └──────────────────────┘                        |
    |                              |                              |
    |<-- { headers, keyid, ... } --|                              |
...
export const POLICY_TYPES = [
  // ... 기존 18개 (SPENDING_LIMIT ~ REPUTATION_THRESHOLD) ...
  'ERC8128_ALLOWED_DOMAINS',   // 신규 추가 (19번째)
] as const;
```
...
const Erc8128AllowedDomainsRulesSchema = z.object({
  ...
  // ⚠️ Rate limit 카운터는 인메모리 (Map<domain, {count, windowStart}>)로 관리.
  // 데몬 재시작 시 카운터가 리셋되며, 이는 의도된 동작 (보안보다 DX 편의 목적).
  // DB 영속화는 오버엔지니어링으로 판단 — ERC-8128은 금전 리스크 없는 인증 서명.
...
#### 3.2 POL-02: 정책 평가 위치 — 전용 라우트 핸들러

**설계 결정 — DatabasePolicyEngine 밖에서 평가:**
ERC8128_ALLOWED_DOMAINS는 `DatabasePolicyEngine.evaluate()` 순서에 포함되지 않는다. X402_ALLOWED_DOMAINS가 `x402-domain-policy.ts`에서 라우트 핸들러 레벨로 독립 평가되는 것과 동일한 패턴을 따른다.

```
ERC-8128 전용 라우트 핸들러 (POST /v1/erc8128/sign):
  1. erc8128.enabled 확인
  2. 지갑 EVM 체인 검증
  3. ERC8128_ALLOWED_DOMAINS 정책 직접 평가  ← DatabasePolicyEngine 밖
     - DB에서 해당 지갑의 ERC8128_ALLOWED_DOMAINS 정책 로드
     - 대상 URL 도메인 추출 → allowed_domains 매칭
     - default-deny / rate_limit 적용
  4. 서명 생성 (Content-Digest → Signature-Input → Signature Base → signMessage)
```

> 참고: X402_ALLOWED_DOMAINS도 동일하게 `x402-domain-policy.ts`에서 라우트 핸들러 레벨로 평가된다. 도메인 기반 정책은 트랜잭션 파이프라인의 Stage 3가 아닌, 각 기능의 전용 라우트에서 직접 평가하는 것이 WAIaaS 패턴이다.
...
### 5.4 SIG-02 보충: connect-info capabilities 확장

SIG-02 범위에 포함. connect-info 응답의 `capabilities` 객체에 ERC-8128 지원 여부를 노출한다.

```typescript
// GET /v1/connect-info 응답 capabilities 확장
{
  capabilities: {
    // ... 기존 capabilities ...
    erc8128Support: true,  // erc8128.enabled Admin Setting에 연동
  }
}
```

에이전트는 connect-info를 통해 데몬의 ERC-8128 지원 여부를 자기 발견하며, `erc8128.enabled = false`이면 `erc8128Support` 필드를 생략하거나 `false`로 반환한다.
...
| D12 | 파이프라인 경로 | 전용 라우트 (6-stage 바이패스) | discriminatedUnion에 새 타입 미추가, 정책은 라우트 핸들러에서 직접 평가 |
| D13 | Rate limit 카운터 | 인메모리 (재시작 시 리셋) | 금전 리스크 없는 인증 서명, DB 영속화 불필요 |
| D14 | connect-info | capabilities.erc8128Support 노출 | 에이전트 자기 발견, erc8128.enabled 연동 |
...
| S10 | POST /v1/erc8128/sign 비-EVM 네트워크 거부 (Solana network 지정 시) | API | L1 | O |
...
| S21 | connect-info에 erc8128Support capabilities 노출 | API | L1 | O |
| S22 | rate_limit_per_minute 초과 시 서명 요청 차단 | 정책 | L0 | O |
...
**커버리지 목표:** L0+L1 ≥ 48 테스트, 신규 코드 statement coverage ≥ 80%
...
8. connect-info의 `capabilities.erc8128Support`가 설정에 따라 올바르게 노출됨
9. 22개 테스트 시나리오 (S1~S22) 중 L0+L1 ≥ 48 테스트 PASS

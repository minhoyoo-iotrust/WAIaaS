...
| 수신 트랜잭션 | `IncomingSettings`는 System/Settings에 존재하지만 수신 TX 뷰어 없음 | v27.1에서 구현한 수신 모니터링의 감지 결과를 UI에서 조회 불가 |
...
의존: 크로스 지갑 트랜잭션 조회 admin API 신규 추가 필요 (`GET /v1/admin/transactions` — 현재 미존재, `/admin/wallets/{id}/transactions`만 존재).
...
의존: 크로스 지갑 수신 트랜잭션 조회 admin API 신규 추가 필요 (`GET /v1/admin/incoming` — 현재 `GET /v1/wallet/incoming`은 sessionAuth 스코프).
...
packages/daemon/src/
  api/routes/
    admin.ts                   # 수정: 크로스 지갑 트랜잭션 admin API 추가 (GET /v1/admin/transactions)
    incoming.ts                # 수정: 크로스 지갑 수신 TX admin API 추가 (GET /v1/admin/incoming)
```
...
| 3 | 수신 TX 뷰어 위치 | 독립 `/incoming` 페이지 | 크로스 지갑 조회 필요 확인 → 독립 페이지로 확정 |
| 4 | 익스플로러 URL 매핑 | `packages/core` 기존 모듈 재사용 | #148(v27.2 FIXED)에서 13-네트워크 매핑 구현 완료. Admin 컴포넌트만 신규 |
| 5 | 크로스 지갑 admin API | **신규 API 2개 추가** | `GET /v1/admin/transactions` + `GET /v1/admin/incoming` 신규 필요 (현재 미존재 확인됨) |
...
| 의존 대상 | 상태 | 이유 |
|----------|------|------|
| ~~#148 블록 익스플로러 링크~~ | v27.2 FIXED | 13-네트워크 익스플로러 URL 매핑 구현 완료. 기존 모듈 재사용 |
| v27.1 수신 TX 모니터링 | SHIPPED | 수신 TX 뷰어가 `GET /v1/wallet/incoming` API에 의존. 구현 완료됨 |
...
| # | 리스크 | 영향 | 대응 방안 |
|---|--------|------|----------|
| 1 | 크로스 지갑 트랜잭션 admin API 부재 | 트랜잭션 페이지 구현 불가 | **확인됨**: 미존재. Phase 1에서 `GET /v1/admin/transactions` 신규 추가 |
| 2 | 크로스 지갑 수신 TX admin API 부재 | 수신 TX 뷰어 구현 불가 | **확인됨**: `GET /v1/wallet/incoming`은 sessionAuth 스코프. Phase 1에서 `GET /v1/admin/incoming` 신규 추가 |
...
| 페이즈 | 내용 | 구현 대상 |
|--------|------|----------|
| Phase 1 | 공용 컴포넌트 + Admin API 신규 | 익스플로러 링크 컴포넌트(core 모듈 재사용), 필터 바, 검색 입력, `GET /v1/admin/transactions` + `GET /v1/admin/incoming` 신규 추가 |
...
*생성일: 2026-02-22*
*검토일: 2026-02-22 — IncomingSettings 상태 정정, #148 해결 확인, 미결 기술 결정 확정*
*선행: ~~#148~~ (v27.2 해결), v27.1 (수신 TX, SHIPPED)*

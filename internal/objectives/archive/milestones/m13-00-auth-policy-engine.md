...
| 52 | AUTH-REDESIGN | auth-model-redesign | masterAuth(Argon2id 검증, 암묵적/명시적 이중 모드, X-Master-Password 헤더), ownerAuth(SIWS/SIWE per-request 서명 검증, 2곳 한정: 거래 승인 + KS 복구), sessionAuth(JWT HS256 2-stage 검증, SessionConstraints DB 조회), authRouter 디스패치(41 엔드포인트 인증 맵), 3-tier 인증 책임 분리 | 전체 |
...
| 29 (api-framework) | OpenAPI 3.0 전체 스펙 자동 생성, 41 엔드포인트 전체 라우트 | v1.3 |
...
| 4 | **authRouter** | 41 엔드포인트 인증 맵 디스패치(masterAuth implicit 20개, sessionAuth 12개, None 4개, masterAuth explicit 3개, ownerAuth 1개, dualAuth 1개), Hono 미들웨어 체인 분기. v0.5 기준 31개에서 v0.6(+5 action), v0.8(+1 withdraw), v1.1(+2 agents/tx detail) 추가로 41개 |
...
| 12 | **X-Agent-Id → sessionAuth 전환** | v1.1에서 에이전트 식별에 사용한 `X-Agent-Id` 헤더 패턴을 sessionAuth JWT의 `aid` claim으로 전환. wallet/transactions/agents 라우트의 `c.req.header('X-Agent-Id')` 호출을 `c.get('agentId')` (sessionAuth 미들웨어가 설정)로 교체. masterAuth implicit 엔드포인트(세션 생성 등)는 기존 패턴 유지 |
| 13 | **transactions 테이블 마이그레이션** | TOCTOU 방지를 위한 `reserved_amount TEXT` 컬럼이 v1.1 스키마에 미포함. v1.2에서 `ALTER TABLE transactions ADD COLUMN reserved_amount TEXT` 마이그레이션 추가 필요. 기본값 NULL, DELAY/APPROVAL 진입 시 설정, CONFIRMED/FAILED/CANCELLED/EXPIRED 시 해제 |
...
class DatabasePolicyEngine implements IPolicyEngine {
  async evaluate(
    agentId: string,
    transaction: { type: string; amount: string; toAddress: string; chain: string },
  ): Promise<PolicyEvaluation>
    // 11단계: 에이전트 로드 → 정책 로드 → 타입별 순차 평가 →
    //         TOCTOU 검증 → 티어 결정 → reserved_amount 업데이트
    // Note: v1.1 IPolicyEngine 시그니처(agentId + transaction 2인자) 준수
}
...
- 모노레포 4-pkg 구조(core, daemon, adapters/solana, cli), pnpm workspace, Turborepo
...
- Hono HTTP 서버 + 미들웨어 5개(requestId, hostGuard, killSwitchGuard, requestLogger, errorHandler). zodValidator는 별도 미들웨어가 아닌 라우트별 Zod `.parse()` 직접 호출로 구현됨
...
*최종 업데이트: 2026-02-10 v1.1 구현 완료 후 7건 반영 (U-01~U-07: 모노레포 4-pkg, 미들웨어 5개, 엔드포인트 41개, X-Agent-Id 전환, reserved_amount 마이그레이션, IPolicyEngine 시그니처)*
*이전: 2026-02-09 v1.0 Phase 45 Plan 01 산출물*

...
| 4 | Docker image 크기 (native addon 포함) | sodium-native + better-sqlite3 native 바이너리로 이미지 크기 증가 가능(예상 300-500MB) | Multi-stage 빌드: builder에서 devDependencies 설치/빌드, runner에서 production만 복사. node:22-slim으로 기본 이미지 최소화. .dockerignore로 불필요 파일 제외 |

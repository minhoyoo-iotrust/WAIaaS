...
Kill Switch/AutoStop으로 긴급 제어, Telegram Bot으로 원격 관리, Docker로 원클릭 배포, 잔액 모니터링으로 가스비 부족 사전 알림이 동작하는 상태. Desktop App은 v1.6.1에서 구현한다.
...
| (신규) | balance-monitoring | 잔액 모니터링 스케줄러 + LOW_BALANCE NotificationEventType 추가 + 가스비 부족 사전 알림. config.toml 임계값 설정, 에이전트별 주기적 잔액 체크, 임계값 이하 시 알림 발송 | 전체 |
...
| 잔액 모니터링 | BalanceMonitorService: 주기적 잔액 체크 스케줄러. config.toml `[monitoring]` 섹션(check_interval_sec=300 기본 5분, low_balance_threshold_sol=0.01, low_balance_threshold_eth=0.005). 에이전트별 네이티브 토큰 잔액 조회(getBalance), 임계값 이하 시 `LOW_BALANCE` 알림 발송(NotificationService.notify). 중복 알림 방지(동일 에이전트 24시간 내 1회). AutoStopService와 동일 EventEmitter 기반 스케줄러 패턴. NotificationEventType에 `LOW_BALANCE` 추가(22번째 이벤트). 메시지 템플릿 en/ko 추가("Agent {agentId} balance low: {balance} {currency}. Threshold: {threshold}") |
...
Telegram Bot과 잔액 모니터링 알림은 사람이 직접 보는 인터페이스이므로, config.toml `locale` 설정에 따라 영문/한글을 지원한다. Desktop UI i18n은 v1.6.1에서 적용한다.
...
| 잔액 모니터링 | LOW_BALANCE_BODY | Agent {agentId} balance low: {balance} {currency}. Threshold: {threshold}. | 에이전트 {agentId} 잔액 부족: {balance} {currency}. 임계값: {threshold}. |
...
| 4 | AutoStop 규칙 저장 | config.toml [autostop] 섹션 | 규칙 임계값은 정적 설정(daily_spending_limit_usd, consecutive_failures_max 등). 런타임 변경 빈도 낮음. policies 테이블은 에이전트별 트랜잭션 정책 전용이므로 혼용 방지 |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 16 | 잔액 임계값 이하 -> LOW_BALANCE 알림 발송 | low_balance_threshold_sol=0.01 설정 + 에이전트 잔액 0.005 SOL -> LOW_BALANCE 알림 mock 수신 assert | [L0] |
| 17 | 잔액 임계값 이상 -> 알림 미발송 | 에이전트 잔액 1.0 SOL -> 알림 미발송 assert | [L0] |
| 18 | 중복 알림 방지 -> 24시간 내 동일 에이전트 1회만 발송 | LOW_BALANCE 알림 1회 발송 후 -> 다음 체크 주기에 동일 에이전트 알림 미발송 assert | [L0] |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 20 | /status -> 데몬 상태 + 에이전트 요약 반환 | mock Telegram Bot API -> /status 명령 전송 -> sendMessage 호출 파라미터에 데몬 상태 + 에이전트 수 포함 assert | [L0] |
...
| 26 | /newsession -> 에이전트 선택 인라인 키보드 -> 세션 발급 | /newsession 명령 -> 에이전트 목록 인라인 키보드 -> 에이전트 선택 -> 세션 토큰 발급 + 응답 assert | [L0] |
...
| 3 | Kill Switch CAS 패턴 SQLite 성능 | BEGIN IMMEDIATE 잠금 범위가 6-step cascade 전체(세션 무효화+거래 중단+에이전트 정지)를 포함하면 write lock 장시간 점유 | cascade 6-step은 별도 트랜잭션으로 분리: 상태 전이(CAS)만 단일 트랜잭션, cascade 단계는 순차 실행(각각 독립 트랜잭션). 상태 전이 성공 후 cascade 일부 실패 시 상태는 유지하되 재시도 |

---

*최종 업데이트: 2026-02-11 v1.6/v1.6.1 분리 — Desktop App을 v1.6.1로 이동, 잔액 모니터링(BalanceMonitorService, LOW_BALANCE) 추가. v0.10 설계 결정(CONC-03 Kill Switch 4전이 CAS ACID) 반영, 설계 문서 40/36 참조*

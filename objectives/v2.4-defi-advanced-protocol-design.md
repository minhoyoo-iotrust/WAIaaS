# 마일스톤 v2.4: 고급 DeFi 프로토콜 설계 (Lending/Yield/Perp 프레임워크)

## 목표

상태를 가진 DeFi 포지션(담보/차입, 수익률 거래, 레버리지 트레이딩)을 관리하기 위한 3개 프레임워크(ILendingProvider, IYieldProvider, IPerpProvider)와 공통 인프라(PositionTracker, HealthFactorMonitor, MarginMonitor)를 설계 수준에서 정의한다. 7개 프로토콜(Aave, Kamino, Pendle, Drift, Morpho, Marinade, CoW)의 공통 설계를 확정하여 v2.4.1~v2.4.7 구현 마일스톤의 입력을 생산한다.

---

## 배경

v2.3.x에서 구현되는 Action Provider(Swap/Bridge/Staking)는 모두 **단발성 트랜잭션**(요청 → 실행 → 완료)이다. v2.4.x의 프로토콜은 **상태를 가진 포지션**(예치 → 이자 누적 → 상환/청산)으로, 지속적인 모니터링과 리스크 관리가 필요하다.

### v2.3.x vs v2.4.x 패턴 비교

```
v2.3.x (단발성):  resolve() → ContractCallRequest → 실행 → 완료
v2.4.x (포지션):  resolve() → ContractCallRequest → 실행 → 포지션 생성 → 모니터링 → 상환/청산
```
...
| 마일스톤 | 프로토콜 | 패턴 |
|---------|----------|------|
| v2.4.6 | Marinade | v2.3.4 Staking 패턴 재사용 |
| v2.4.7 | CoW Protocol | Intent/EIP-712 서명 신규 패턴 |
...
| 대상 구현체 | Aave V3 (v2.4.1), Kamino (v2.4.2), Morpho (v2.4.5) |
...
1. 3개 프레임워크의 인터페이스가 확정되어 v2.4.1에서 바로 구현 가능
...
*생성일: 2026-02-15*
*범위: 설계 마일스톤 — 코드 구현은 v2.4.1~v2.4.7에서 수행*
*선행: v2.3 (기본 DeFi 프로토콜 설계), v1.5 (Action Provider 프레임워크)*

# 마일스톤 v2.0.5: DX(Developer Experience) 품질 개선
...
| 항목 | 이유 | 대상 마일스톤 |
|------|------|-------------|
| Admin UI 온보딩 (지갑 0개 상태 CTA) | Admin UI 메뉴 재구성과 함께 처리 | v2.0.3 |
| `waiaas start --background` 데몬 모드 | 프로세스 관리 아키텍처 변경 필요 (별도 설계) | TBD |
| SDK `createSession()` 메서드 추가 | SDK 인터페이스 확장 (별도 설계) | TBD |
| Python SDK PyPI 게시 | npm Trusted Publishing (v2.0.4)와 병행 검토 | v2.0.4 |
| `--password` CLI 인자 보안 경고 | 기능 변경이 아닌 문서화로 충분 | v2.0.1 (CONTRIBUTING.md 또는 docs/) |
...
| 5 | Python SDK PyPI 게시 시점 | v2.0.4에서 npm과 함께 | Trusted Publishing 인프라를 npm + PyPI 동시에 구축하면 효율적 |
...
| 의존 대상 | 이유 |
|----------|------|
| v2.0.1 (오픈소스 거버넌스) | README 재작성(issue #067)이 v2.0.1에서 완료. 본 마일스톤에서는 코드 예시만 수정 |
| v2.0 (릴리스) | npm 패키지 게시, Docker 이미지 배포가 완료된 상태에서 DX 검증 |

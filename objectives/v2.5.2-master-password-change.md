...
| 4 | CLI 지원 | v2.1.2에서는 Admin UI만 | CLI에서의 패스워드 변경은 데몬 실행 중에만 가능하므로 Admin UI와 동일한 REST API 호출. 필요 시 추후 CLI 명령어 추가 |
...
| 의존 대상 | 이유 |
|----------|------|
| v2.0 (오픈소스 릴리스) | Admin UI + masterAuth + 키스토어 인프라가 안정화된 상태에서 진행 |
| v2.0.1 (거버넌스) | 보안 관련 변경이므로 거버넌스 체계 수립 후 진행 |
| v2.0.3 (지갑 앱 알림 채널) | 패스워드 변경 후 지갑 앱 알림 인프라 활용 가능 |

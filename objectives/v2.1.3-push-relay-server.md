...
v2.1.1에서 WAIaaS는 ntfy 토픽을 통해 서명 요청을 전달한다. 지갑 앱이 ntfy SSE/WebSocket을 직접 구독하면 서버 없이 동작하지만, 실제 지갑 개발사 환경에서는 한계가 있다:
...
| 의존 대상 | 이유 |
|----------|------|
| v2.1.1 (Signing SDK) | ntfy 서명 토픽 구조, SignRequest 스키마 |
| v2.1.2 (알림 채널) | ntfy 알림 토픽 구조, WalletNotification 스키마 |
...
*생성일: 2026-02-15*
*선행: v2.1.1 (Signing SDK), v2.1.2 (알림 채널)*

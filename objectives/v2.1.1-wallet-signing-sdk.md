# 마일스톤 v2.0.2: WAIaaS Wallet Signing SDK
...
v2.0.2 완료 시 Owner는 5가지 승인 채널을 선택할 수 있다:

| 우선순위 | 채널 | 마일스톤 | 특징 |
|---------|------|---------|------|
| 1 | **WAIaaS SDK + ntfy** | v2.0.2 | 메신저 불필요, 지갑 앱만으로 동작 |
| 2 | **WAIaaS SDK + Telegram** | v2.0.2 | 메신저 중계, 파트너 지갑 전용 |
...
| 값 | 설명 | 연동 마일스톤 |
|----|------|-------------|
| `sdk_ntfy` | WAIaaS SDK + ntfy 직접 푸시 (메신저 불필요) | v2.0.2 |
| `sdk_telegram` | WAIaaS SDK + Telegram 메신저 중계 | v2.0.2 |
...
| 항목 | 설명 | 시기 |
|------|------|------|
| Slack 양방향 | Slack Socket Mode 구현으로 서명 응답 수신 | v2.0.2 이후 |
| Discord 양방향 | Discord Gateway Bot 구현으로 서명 응답 수신 | v2.0.2 이후 |

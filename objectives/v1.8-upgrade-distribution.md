...
설치된 WAIaaS가 새 버전 출시를 감지하여 사용자에게 알리고, 안전하게 업그레이드할 수 있는 체계를 **설계**하는 상태. npm/Docker/Desktop 각 배포 채널별 업그레이드 경로, DB 마이그레이션 연동, 호환성 매트릭스, 롤백 전략이 설계 문서로 확정되어 v1.9에서 즉시 구현할 수 있다.
...
#### 6. Tauri Desktop 자동 업데이트

| 항목 | 설계 범위 |
|------|----------|
| 소스 | GitHub Releases (`latest.json`) |
| 검증 | Tauri 내장 Ed25519 서명 검증 |
| UX | 시스템 트레이 알림 → 다운로드 → 재시작 |
| Sidecar | 데몬 바이너리 포함 업데이트 |
...
| 39 | tauri-desktop-architecture | Tauri Auto Updater 기존 계획 |
...
| 6 | Desktop 자동 업데이트 | Tauri 내장 Updater | 안정적 내장 기능, 서명 검증 포함 |
...
> v1.8은 설계 마일스톤이므로 E2E 시나리오는 **v1.9 구현 시 검증 기준**으로 정의한다.
...
| 19 | Tauri Updater → latest.json 조회 → 업데이트 감지 | [L0] |
| 20 | 업데이트 설치 → 서명 검증 → Sidecar 교체 | [L1] |
...
| v1.6 (Desktop + Docker) | Tauri Auto Updater, Docker 이미지 태깅이 v1.6에서 구현됨 |
...
| 5 | Tauri 서명 키 분실 | Desktop 자동 업데이트 불가 | CI Secrets + 오프라인 백업 |
| 6 | Docker latest 태그 breaking change | Watchtower 자동 업데이트 시 호환성 문제 | major 태그(`waiaas/daemon:2`) 사용 권장 |

---

*최종 업데이트: 2026-02-10 — 초안 작성 (설계 전용 마일스톤, 구현은 v1.9)*

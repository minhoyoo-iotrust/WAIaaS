...
> **이연 사유**: Desktop App은 배포 채널이지 핵심 기능이 아님. 모든 코어 기능은 CLI + REST API + Admin Web UI + MCP로 동작. v2.0은 npm/Docker/CLI 중심으로 릴리스하고, Desktop은 이후 진행.

---

*최종 업데이트: 2026-02-16. v2.0 이후로 이연 — Desktop은 배포 채널이므로 v2.0 릴리스 후 진행. Admin Web UI 재사용 아키텍처로 변경 — React 18 재구현 대신 기존 Preact Admin Web UI를 WebView에서 로드. Desktop 전용 기능(Sidecar, 트레이, Wizard, WalletConnect)만 추가. 설계 문서 39 변경은 v2.6에서 수행.*

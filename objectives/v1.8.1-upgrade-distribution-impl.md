...
v1.8에서 설계한 업그레이드/배포 인프라가 모두 구현되어, 사용자가 새 버전 알림을 받고 `waiaas upgrade`로 안전하게 업그레이드할 수 있는 상태. npm/Docker 2개 채널 업그레이드 경로가 동작하며, DB 마이그레이션과 호환성 검증이 자동으로 실행된다. Desktop 채널은 v2.6.1로 이연.
...
| 문서 | 이름 | 구현 범위 | 전체/부분 |
|------|------|----------|----------|
| 66 | upgrade-distribution-spec | VersionCheckService, CLI 알림, `waiaas upgrade` 명령, Health 확장, Docker 업그레이드 경로, 호환성 매트릭스, CHANGELOG 자동 생성 | 부분 (Tauri Auto Updater는 v2.6.1로 이연) |
...
- Tauri Desktop 앱 + Sidecar Manager (v2.6.1로 이연)
- Docker Multi-stage Dockerfile + docker-compose (v1.6에서 구현)
- GitHub Actions release.yml (v1.7에서 구현). release-please는 v1.8.1에서 신규 추가
- 설계 문서 66 (v1.8에서 작성)
...
| ~~Tauri Updater 설정~~ | — | v2.6.1로 이연 (Desktop App 자체가 v2.6.1로 이연) |
...
> v1.8에서 정의한 24건 기준을 구체화하고 검증 방법을 명시한다.
...
| v1.6 (Docker) | Docker Dockerfile이 v1.6에서 구현됨. v1.8.1에서 라벨 추가. Tauri Updater는 v2.6.1로 이연 |
| v1.7 (CI/CD) | release.yml이 v1.7에서 구축됨. release-please는 v1.8.1에서 신규 추가하여 CHANGELOG 자동 생성 연동 |
...
| 5 | Tauri 서명 키 분실 | v2.6.1로 이연 (Desktop 자체가 v2.6.1로 이연) | — |
...
*최종 업데이트: 2026-02-17 — (1) 의존 section 수정: release-please는 v1.7이 아닌 v1.8.1에서 신규 추가 (2) Health schemaVersion 예시값 16으로 수정 (3) 2-게이트 릴리스 모델 E2E 6건 추가 (총 24건). Tauri Desktop은 v2.6.1로 이연*

# 마일스톤 v2.1: Wallet SDK 설계

## 목표

지갑 개발사(D'CENT 등)가 WAIaaS와 통합하기 위한 Wallet Signing SDK와 개방형 서명 프로토콜, 지갑 앱 알림 채널, Push Relay Server의 공통 설계를 확정한다. v2.1.1(Signing SDK 구현), v2.1.2(알림 채널 구현), v2.1.3(Push Relay Server)의 입력을 생산한다.
...
| 축 | 마일스톤 | 내용 |
|----|---------|------|
| 서명/승인 | v2.1.1 | Signing Protocol v1 + WAIaaS Wallet SDK + ntfy/Telegram 채널 |
| 알림 수신 | v2.1.2 | 지갑 앱으로 모든 알림 수신 + 승인/거부 완결 UX |
| 푸시 중계 | v2.1.3 | Push Relay Server — ntfy → 지갑사 푸시 서비스(Pushwoosh/FCM/APNs) 변환 |
...
| 토픽 분리 | 서명 요청 토픽 (v2.1.1) + 일반 알림 토픽 (v2.1.2) |
...
1. Signing Protocol v1 스키마가 확정되어 v2.1.1에서 바로 구현 가능
...
*생성일: 2026-02-15*
*범위: 설계 마일스톤 — 코드 구현은 v2.1.1~v2.1.3에서 수행*
*선행: v2.0 (릴리스)*

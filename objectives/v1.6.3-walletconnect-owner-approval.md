# 마일스톤 v1.6.3: WalletConnect Owner 승인
...
```toml
[walletconnect]
project_id = ""                          # Reown Cloud Project ID (필수)
auto_extend = true                       # 세션 자동 연장 (기본: true)
extend_before_expiry_hours = 24          # 만료 N시간 전에 extend (기본: 24)
request_timeout_sec = 300                # 서명 요청 타임아웃 (기본: 5분)
```
...
| 4 | Project ID 관리 | Admin Settings에서 설정 (기존 walletconnect.project_id 필드 활용) | Reown Cloud에서 무료 생성. 기존 설정 필드가 이미 존재하므로 추가 UI 작업 최소화 |

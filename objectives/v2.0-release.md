...
| # | 결정 항목 | 선택지 | 결정 근거 |
|---|----------|--------|----------|
| 1 | npm publish 범위 | 전체 7 패키지 공개 | Self-hosted 아키텍처이므로 모든 패키지가 npm에서 직접 설치 가능해야 함. 비공개 패키지는 관리 복잡도만 증가 |
...
| 6 | npm publish --dry-run 성공 (공개 7 패키지) | 각 패키지 `npm publish --dry-run` exit 0 | [L1] |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 12 | README 영문 버전 내용 검토 + 품질 확인 | README.md 영문 버전 읽기 -> 프로젝트 소개/Quick Start/아키텍처 개요가 정확하고 이해하기 쉬운지 판단 | [HUMAN] |
| 13 | CHANGELOG.md 최종 검토 + 누락 확인 | CHANGELOG.md에 v1.1~v2.0 모든 주요 변경 사항이 누락 없이 기재되었는지 확인 | [HUMAN] |
| 14 | 설계 부채 이연 판단 (0건이 아닌 경우) | design-debt.md에 미해결 항목이 있을 경우, v2.1 이연이 적절한지 판단. 보안 관련 부채는 이연 불가 | [HUMAN] |
...
*최종 업데이트: 2026-02-10 예제 에이전트(examples/simple-agent) 산출물 추가, 릴리스 체크리스트 15건(자동 12 + [HUMAN] 3)*

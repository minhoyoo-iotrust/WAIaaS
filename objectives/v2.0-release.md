...
v2.0은 새로운 설계 문서를 구현하는 것이 아니라, **v1.1~v1.7에서 구현한 설계 문서(29개)의 최종 검증과 릴리스 준비**를 수행한다. 설계 문서 39(Tauri Desktop)는 v2.6.1로 이연.
...
| CHANGELOG | v1.1~v2.0 전체 변경 이력 | release-please 자동 생성 (v1.8에서 설계, v1.8.1에서 구현) + 수동 보완 |
...
| 3 | GitHub Release 자동화 | release-please (v1.8에서 설계, 2-게이트 모델) | 게이트 1: Release PR 머지(릴리스 의사 결정) → 게이트 2: deploy job 수동 승인(배포 실행). v2.0에서 deploy job의 dry-run 제거로 실제 배포 활성화 |
...
*최종 업데이트: 2026-02-16 Desktop(Tauri)을 v2.6/v2.6.1로 이연 — doc 39 검증 제거, 5플랫폼 바이너리 제거, 플랫폼 테스트 84건으로 축소. npm/Docker/CLI 중심 릴리스. @waiaas/skills npm 패키지 배포 추가 (공개 8패키지)*

...
v0.1~v0.4 설계 문서 전수 분석에서 도출된 29건의 구현 장애 요소(CRITICAL 8 + HIGH 11 + MEDIUM 10)를 해소한다. 코드 작성 전에 "설계대로 구현하면 동작하지 않는 부분"을 제거하여, 구현 첫날부터 차단 없이 진행할 수 있는 상태를 만든다.
...
| **v0.7 (본 마일스톤)** | **"코드로 옮길 때 부딪히는 기술적 문제 29건"** | — |

### 문제의 성격

v0.3에서 해소한 37건은 **문서 간 용어/값/경로 불일치**였다. 본 마일스톤의 29건은 성격이 다르다:
...
| 등급 | 건수 | 성격 |
|------|------|------|
| CRITICAL | 8 | 구현 차단 또는 런타임 장애 |
| HIGH | 11 | 보안 취약점 또는 통합 실패 |
| MEDIUM | 10 | 추가 조사 필요 또는 향후 확장 시 문제 |
| **합계** | **29** | |
...
### 1. 설계 문서를 직접 수정한다
- 새 문서를 만들지 않고, 기존 설계 문서(24~40번)의 해당 섹션을 보완한다
...
- 대규모 재설계가 필요한 항목은 별도 마일스톤(objectives/05, 06)으로 분리되어 있다
...
**문제:** rateLimiter(#7) → sessionAuth(#8) 순서로, 인증 전에 IP 기반 rate limit만 적용. 공격자가 토큰 없이 100 req/min 소진 시 정상 사용자도 차단.

**관련 문서:** `29-api-framework-design` §2.1

**해결:**
- **2단계 Rate Limiter** 도입:
  - Stage 1 (미들웨어 #3.5, shutdownGuard 직후): IP 기반 글로벌 rate limit — 1000 req/min (DoS 방어용, 느슨한 제한)
  - Stage 2 (미들웨어 #9, sessionAuth 직후): 세션/에이전트 기반 세분화 rate limit — 기존 3-level 유지 (global 100/session 300/tx 10 req/min)
- 미들웨어 순서 변경:
...
→ #8 sessionAuth/ownerAuth → #9 sessionRateLimit(session, 300/min; tx, 10/min)
```
...
**해결:**
- killSwitchGuard는 **인증 전**(#7)에 위치하는 것이 맞음. 근거:
  - Kill Switch ACTIVATED 시 시스템이 잠긴 상태 — 인증 자체가 불필요한 요청(health, recover)만 허용
  - 인증 후에 배치하면 잠긴 상태에서도 세션 검증 오버헤드 발생
...
### D-1. MCP 도구/리소스 완전 스펙 [CRITICAL C-6]

**문제:** "6 tools + 3 resources"라고만 기술. 도구명, 파라미터 스키마, REST 엔드포인트 매핑 없음. DELAY/APPROVAL 티어 폴링 처리 방법도 미정의.

**관련 문서:** `38-sdk-mcp-interface` §5
...
| Tool | REST 매핑 | 입력 | 출력 |
|------|----------|------|------|
| `send_token` | `POST /v1/transactions/send` | `{ agentId, to, amount, memo? }` | `{ txId, status, hash? }` |
| `get_balance` | `GET /v1/wallet/balance` | `{ agentId }` | `{ balance, decimals, formatted }` |
| `list_transactions` | `GET /v1/transactions` | `{ agentId?, status?, cursor?, limit? }` | `{ items[], nextCursor? }` |
| `get_transaction` | `GET /v1/transactions/:id` | `{ txId }` | `{ tx object }` |
| `list_agents` | `GET /v1/agents` | `{}` | `{ agents[] }` |
| `get_agent` | `GET /v1/agents/:id` | `{ agentId }` | `{ agent object }` |

- 3개 MCP Resource:

| Resource | URI | 설명 |
|----------|-----|------|
| `waiaas://agents` | 전체 에이전트 목록 | 갱신 주기: 요청 시 |
| `waiaas://agents/{id}/balance` | 에이전트 잔액 | 갱신 주기: 요청 시 |
| `waiaas://system/status` | 데몬 상태 (health, kill switch) | 갱신 주기: 요청 시 |

- DELAY/APPROVAL 티어 처리:
  - `send_token` 반환 시 `status: "QUEUED"` 포함
  - AI 에이전트는 `get_transaction`으로 폴링하여 최종 상태 확인
  - MCP content에 `hint: "Transaction queued for approval. Check status with get_transaction."` 포함

**수정 대상:** `38-sdk-mcp-interface` §5

### D-2. Tauri 종료 타임아웃 정합 [CRITICAL C-7]
...
### D-3. Tauri CORS Origin 검증 [HIGH H-3]
...
| 순서 | 동작 | 근거 |
|------|------|------|
| 1 | 해당 Owner의 APPROVAL 대기 트랜잭션 → EXPIRED 처리 | 승인자 부재 |
| 2 | 해당 Owner의 DELAY 대기 트랜잭션 → 유지 (타이머 계속) | DELAY는 Owner 개입 불필요 |
| 3 | wallet_connections에서 WalletConnect 세션 정리 | push 서명 비활성화 |
| 4 | agents.owner_address는 유지 | 주소는 에이전트 속성, 변경은 별도 API |
| 5 | audit_log에 OWNER_DISCONNECTED 이벤트 기록 | 감사 추적 |

- 새 Owner 등록: 즉시 가능 (cooldown 없음). `PUT /v1/agents/:id/owner` (masterAuth + 기존 ownerAuth 조건부)

**수정 대상:** `34-owner-wallet-connection` §6~7, `37-rest-api-complete-spec` §8

### D-5. Transaction Status 응답값 명확화 [HIGH H-6]

**문제:** INSTANT 티어 동기 응답에서 status가 `PENDING`인지 `QUEUED`인지 미명시.
...
### D-6. Setup Wizard vs CLI init 순서 확정 [HIGH H-7]
...
### D-7. 세션 생성 API 인증 방식 확정 [HIGH H-11]

**문제:** `POST /v1/sessions`의 인증이 "Owner Signature (body 내 서명) 커스텀 검증"인지 "Authorization 헤더 기반 ownerAuth"인지 충돌.

**관련 문서:** `37-rest-api-complete-spec` §3.4, `34-owner-wallet-connection` §5.3

**해결:**
- `POST /v1/sessions`는 **masterAuth** (X-Master-Password 헤더)로 확정
- 근거: objectives/05-auth-restructure-dx.md에서 이미 결정됨 — 세션 발급은 "데몬 운영" 행위이므로 ownerAuth 불필요
- v0.2 설계의 ownerAuth 기반 세션 발급은 objectives/05 마일스톤에서 masterAuth로 전환 예정
- 본 마일스톤에서는 `37-rest-api-complete-spec` §3.4의 인증 맵에 **현재 상태(ownerAuth)와 향후 변경(masterAuth, objectives/05 참조)** 모두 주석으로 기록

**수정 대상:** `37-rest-api-complete-spec` §3.4

### D-8. /v1/nonce 경로 통일 잔존 처리 [MEDIUM M-1]

**문제:** `37-rest-api-complete-spec`에서 `/v1/nonce`로 단순화했으나, `30-session-token-protocol`과 `32-transaction-pipeline-api`에 `/v1/auth/nonce` 잔존 가능.

**관련 문서:** `30-session-token-protocol`, `32-transaction-pipeline-api`, `37-rest-api-complete-spec` §5.3

**해결:**
- 모든 문서에서 `/v1/auth/nonce` → `/v1/nonce`로 일괄 치환
- `37-rest-api-complete-spec`이 SSoT (이미 결정됨)
- 검색 대상: `30-session-token-protocol`, `32-transaction-pipeline-api`, `38-sdk-mcp-interface`

**수정 대상:** `30-session-token-protocol`, `32-transaction-pipeline-api`, `38-sdk-mcp-interface`

### D-9. Python SDK snake_case 매핑 규칙 [MEDIUM M-2]
...
### D-10. Zod refinement 규칙 SDK 노출 [MEDIUM M-8]
...
### D-11. Approval 만료 시 Owner 알림 [MEDIUM M-10]

**문제:** APPROVAL 만료 시 EXPIRED로 전환되지만 Owner에게 알림이 없음. 조용한 실패.

**관련 문서:** `34-owner-wallet-connection` §6.2, `35-notification-architecture`

**해결:**
- `NotificationEventType` enum에 `TX_APPROVAL_EXPIRED` 추가:

```typescript
// 45-enum-unified-mapping.md의 NotificationEventType에 추가
TX_APPROVAL_EXPIRED  // 승인 대기 시간 초과
```

- `ApprovalTimeoutWorker`가 EXPIRED 처리 시 알림 발송:
  - 만료 10분 전: `TX_APPROVAL_REMINDER` (사전 경고)
  - 만료 시: `TX_APPROVAL_EXPIRED` (최종 알림)
- audit_log에도 `TX_APPROVAL_EXPIRED` 이벤트 기록

**수정 대상:** `35-notification-architecture`, `34-owner-wallet-connection` §6.2, `45-enum-unified-mapping`
...
**문제:** `chain`, `network` 컬럼에 CHECK 제약조건이 없어 `'solanna'` 같은 오타가 DB에 저장될 수 있음.
...
| `27-chain-adapter-interface` | A | 중 | getCurrentNonce, resetNonceTracker, UnsignedTransaction.nonce |
| `28-daemon-lifecycle-cli` | B, D | 중 | flock 잠금, JWT rotate CLI, Setup Wizard 연동, 비정상 종료 복구 |
| `29-api-framework-design` | B | 중 | 2단계 Rate Limiter, 미들웨어 순서 변경, CORS Origin 추가 |
| `30-session-token-protocol` | B, C, D | 중 | JWT dual-key 로테이션, SIWE viem 전환, /v1/nonce 통일, nonce 저장 옵션 |
| `31-solana-adapter-detail` | A | 소 | blockhash freshness guard, priority fee TTL 근거 |
| `32-transaction-pipeline-api` | A, D | 소 | blockhash 갱신 스펙, /v1/nonce 통일, status 응답값 확정 |
| `33-time-lock-approval-mechanism` | — | — | 변경 없음 (패턴 참조만) |
| `34-owner-wallet-connection` | B, D | 중 | Master Password 통일, disconnect cascade, approval 만료 알림 |
| `35-notification-architecture` | D, E | 소 | TX_APPROVAL_EXPIRED 이벤트, 채널 삭제 트랜잭션 |
| `36-killswitch-autostop-evm.md` | A, B | 소 | EVM nonce 메서드, killSwitchGuard 허용 목록 |
| `37-rest-api-complete-spec` | B, D | 중 | Rate Limiter, killSwitchGuard, status 응답값, 인증 맵, disconnect |
| `38-sdk-mcp-interface` | C, D | 중 | MCP 6 tools 완전 스펙, Zod export, Python 매핑, /v1/nonce |
...
| `45-enum-unified-mapping` | D, E | 소 | TX_APPROVAL_EXPIRED 이벤트, ChainType/NetworkType CHECK 값 |
...
| ID | 산출물 | 설명 |
|----|--------|------|
| FIX-A | 체인 어댑터 보완 | blockhash guard, EVM nonce 인터페이스, keystore math 정정, fee TTL 근거 |
| FIX-B | 데몬 보안 보완 | JWT rotation, flock 잠금, 2단계 Rate Limiter, killSwitch 허용 목록, 패스워드 통일, 단일 인스턴스 |
| FIX-C | 의존성 해소 | SIWE viem 전환, sidecar 크로스 컴파일 전략 |
| FIX-D | API 통합 완성 | MCP 6 tools, Tauri 타임아웃/CORS, disconnect cascade, status 응답, init 순서, 인증 맵, nonce 경로, Python 매핑, Zod export, approval 알림 |
...
- [ ] IChainAdapter에 getCurrentNonce, resetNonceTracker가 추가되고 UnsignedTransaction.nonce가 명시적 필드로 승격됨
- [ ] SIWE 검증이 viem/siwe로 전환되어 ethers 의존성이 제거됨
- [ ] JWT Secret dual-key rotation 메커니즘이 5분 전환 윈도우로 정의됨
- [ ] 데몬 인스턴스 잠금이 flock 기반으로 전환됨
- [ ] MCP 6 tools + 3 resources의 완전한 입출력 스키마와 REST 매핑이 정의됨
- [ ] Tauri sidecar 종료 타임아웃이 35초로 변경되고 비정상 종료 복구 로직이 추가됨
- [ ] Keystore nonce 충돌 확률 계산이 정정됨

### HIGH 해소 (11/11)
- [ ] Rate Limiter가 2단계(globalRateLimit + sessionRateLimit)로 분리됨
- [ ] killSwitchGuard 허용 엔드포인트 목록이 4개로 확정됨
- [ ] Tauri CORS Origin에 Windows용 `https://tauri.localhost`가 추가됨
- [ ] Owner disconnect cascade 5단계가 정의됨
- [ ] Master Password 인증이 Argon2id로 통일됨
- [ ] 티어별 Transaction Status HTTP 응답값이 확정됨
...
- [ ] POST /v1/sessions 인증 방식이 현재/향후 모두 문서화됨

### MEDIUM 해소 (10/10)
- [ ] /v1/nonce 경로가 모든 문서에서 통일됨
- [ ] Python SDK snake_case 변환 규칙과 Pydantic ConfigDict 패턴이 정의됨
...
- [ ] Approval 만료 시 TX_APPROVAL_EXPIRED 알림이 정의됨

### 통합 검증
- [ ] 29건 전체에 대해 설계 문서가 수정되고 `[v0.7 보완]` 태그로 추적 가능
...
- 실제 코드 구현 (설계 보완 마일스톤)
- v0.2 아키텍처 대규모 변경 (objectives/05 인증 재설계, objectives/06 블록체인 확장에서 처리)
- 성능 최적화 (프로파일링은 구현 후)
- 새로운 기능 추가
...
──────────────────                  ─────────────────────
IChainAdapter 13 methods       →   +2 methods (nonce), blockhash guard
...
─────────────────────               ─────────────────────
9개 Enum SSoT                  →   TX_APPROVAL_EXPIRED 추가, CHECK 제약조건
/v1/nonce 경로 통일            →   잔존 불일치 일괄 치환

v0.4 (테스트 전략)                  v0.7 (구현 장애 해소)
──────────────                      ─────────────────────
MCP 도구 테스트 계획            →   MCP 도구 스펙 완성 (테스트 대상 확정)
보안 시나리오 25개             →   경쟁 조건/TOCTOU 시나리오 구체화

v0.7 (구현 장애 해소)              v0.5/v0.6 (향후 마일스톤)
─────────────────────               ────────────────────────
인증 맵 현재/향후 문서화       →   v0.5에서 masterAuth 전환 실행
IChainAdapter nonce 인터페이스 →   v0.6에서 EVM 본 구현 시 활용
```

---

*작성: 2026-02-06*
*기반 분석: v0.1~v0.4 설계 문서 45건 전수 분석, 4개 병렬 에이전트 구현 시뮬레이션*
*상태: 초안*

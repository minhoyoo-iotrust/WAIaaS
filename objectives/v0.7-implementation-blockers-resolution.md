...
- 대규모 재설계가 필요한 항목은 별도 마일스톤(objectives/v0.5, 06)으로 분리되어 있다
...
- 근거: objectives/v0.5-auth-restructure-dx.md에서 이미 결정됨 — 세션 발급은 "데몬 운영" 행위이므로 ownerAuth 불필요
- v0.2 설계의 ownerAuth 기반 세션 발급은 objectives/v0.5 마일스톤에서 masterAuth로 전환 예정
- 본 마일스톤에서는 `37-rest-api-complete-spec` §3.4의 인증 맵에 **현재 상태(ownerAuth)와 향후 변경(masterAuth, objectives/v0.5 참조)** 모두 주석으로 기록
...
- 실제 코드 구현 (설계 보완 마일스톤)
- v0.2 아키텍처 대규모 변경 (objectives/v0.5 인증 재설계, objectives/v0.6 블록체인 확장에서 처리)
- 성능 최적화 (프로파일링은 구현 후)
- 새로운 기능 추가

...
| 11 | 설계 문서 통합 완료 | `internal/design/` 디렉토리에 60+개 파일 존재 + `.planning/deliverables/` 및 `docs-internal/` 미존재 assert | [L0] |
| 12 | objectives 이동 완료 | `internal/objectives/` 디렉토리에 TRACKER.md 존재 + 루트 `objectives/` 미존재 assert | [L0] |
| 13 | 완료 문서 아카이브 | `internal/objectives/archive/milestones/`에 v0.1~v2.0 문서 존재 + `internal/objectives/`에 v2.0.1~ 활성 문서만 존재 assert | [L0] |
| 14 | docs/ 영문 전용 검증 | `docs/` 내 모든 .md 파일이 한글 제목/헤더 미포함 assert | [L0] |
...
*최종 업데이트: 2026-02-17 — 내부 문서 구조 정리 항목 추가 (산출물 4, 검증 11-13)*

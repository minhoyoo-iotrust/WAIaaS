...
v2.3.1에서 Jupiter(Solana DEX)가 첫 번째 ActionProvider로 구현된다. v2.3.2는 **동일한 REST API -> calldata 패턴**을 EVM 체인에 적용하여, WAIaaS가 Solana와 EVM 양쪽에서 DEX 스왑을 지원하는 상태를 만든다.
...
| v2.3.1 (Jupiter Swap) | 첫 번째 ActionProvider 구현으로 패턴 확립. packages/actions/ 패키지 구조, 내장 프로바이더 로딩 로직 재사용 |

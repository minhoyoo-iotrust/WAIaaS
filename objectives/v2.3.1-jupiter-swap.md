# 마일스톤 v2.3.1: Jupiter Swap (첫 번째 Action Provider 구현체)
...
v1.5에서 IActionProvider 인터페이스, ActionProviderRegistry, MCP Tool 자동 변환이 구현된다. v2.3.1은 이 프레임워크의 **첫 번째 실 구현체**로서 Jupiter Aggregator를 연동한다.
...
| 4 | Solana 토큰만 지원 | EVM DEX(Uniswap 등)는 별도 프로바이더 필요 | Jupiter는 Solana 전용으로 명확히 범위 한정. EVM DEX는 별도 마일스톤(v2.3.2+)에서 UniswapActionProvider로 추가 |

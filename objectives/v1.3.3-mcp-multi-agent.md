...
| # | 결정 항목 | 결정 | 근거 |
|---|----------|------|------|
| 1 | 토큰 파일 경로 | `DATA_DIR/mcp-tokens/<agentId>` | 에이전트별 격리. 기존 `mcp-token` 단일 파일에서 `mcp-tokens/` 디렉토리로 전환 |
...
MCP 서버 시작 시 `WAIAAS_AGENT_ID`가 설정되어 있으면, 토큰의 JWT `agt` 클레임에서 에이전트 ID를 확인할 수 있다. 에이전트 **이름**은 토큰에 포함되어 있지 않으므로, 서버 이름과 도구 description에는 두 가지 전략이 있다:

| 전략 | 장점 | 단점 |
|------|------|------|
| A. 시작 시 `GET /v1/agents/{id}` 호출 | 정확한 이름 사용 | sessionAuth 토큰으로는 다른 에이전트 정보 조회 불가 (자기 에이전트만 가능하므로 OK) |
| B. 환경변수 `WAIAAS_AGENT_NAME` 추가 | API 호출 불필요 | 환경변수 추가 부담 |

**채택: A 전략** — MCP 서버 시작 시 `GET /v1/wallet/address` 호출로 에이전트 이름을 가져오지 않고, **CLI `mcp setup` 단계에서 에이전트 이름을 config 스니펫의 키 이름에 반영**한다. MCP 서버 자체는 `WAIAAS_AGENT_ID`만으로 토큰 파일을 찾고, 서버 이름에는 agentId 앞 8자를 사용하되, **별도 `WAIAAS_AGENT_NAME` 환경변수를 CLI가 자동으로 config 스니펫에 포함**시킨다.
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 10 | `WAIAAS_AGENT_NAME` 설정 시 MCP 서버 이름이 `waiaas-{name}` | createMcpServer → server.name assert | [L0] |
| 11 | `WAIAAS_AGENT_NAME` 미설정 시 서버 이름 `waiaas-wallet` (하위 호환) | createMcpServer → server.name === 'waiaas-wallet' assert | [L0] |
| 12 | 도구 description에 에이전트 이름 포함 | `WAIAAS_AGENT_NAME=trading` → send_token description에 "trading" 포함 assert | [L0] |

### 갱신 (renewal) 연동 (2건)

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 13 | agentId 설정 시 갱신된 토큰이 `mcp-tokens/<agentId>` 경로에 저장 | renewal mock → 새 파일 경로 assert | [L0] |
| 14 | recovery loop에서 agentId별 토큰 파일 감지 | 파일 교체 → recoveryPoll → 새 토큰 로드 assert | [L0] |
...
| v1.3.2 (Admin Web UI 구현) | v1.3.2 완료 후 v1.3.3 시작 (현재 진행 중 마일스톤) |
...
| 5 | 에이전트 이름에 특수문자 | config 키 이름이 유효하지 않을 수 있음 | 에이전트 이름을 slug화: 영소문자 + 하이픈만 허용, 나머지는 하이픈으로 치환 |
...
| 수정 LOC | ~150 |
| 신규 테스트 | ~14건 |
| 페이즈 수 | 2 (토큰 파일 + SessionManager 수정 → CLI + MCP 서버 수정) |

---

*최종 업데이트: 2026-02-11 — 초안 작성*

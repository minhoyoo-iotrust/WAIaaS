...
```
1. 클라이언트 → 서버에 리소스 요청
2. 서버 → HTTP 402 + PaymentRequirements JSON 응답
3. 클라이언트 → 결제 서명 생성 → PAYMENT-SIGNATURE 헤더와 함께 재요청
4. 서버 → facilitator를 통해 결제 검증/정산 → 리소스 제공
```
...
| 컴포넌트 | 내용 |
|----------|------|
| x402 핸들러 | HTTP 402 응답 파싱 → PaymentRequirements 해석 → 지원 가능한 (scheme, network) 쌍 선택 → 결제 서명 생성. exact 스킴 지원: Solana(SPL TransferChecked 부분 서명 트랜잭션) + EVM(EIP-3009 transferWithAuthorization 서명) |
...
| facilitator 클라이언트 | x402 facilitator 서버와 통신. 기본값: `https://x402.org/facilitator` (Coinbase 공개 facilitator). config.toml에서 자체 facilitator URL 오버라이드 가능 |
...
packages/daemon/src/services/
  x402/
    x402-handler.ts                # 402 응답 파싱 + 결제 서명 생성 + 재요청
    facilitator-client.ts          # facilitator /verify, /settle, /supported 호출
...
facilitator_url = "https://x402.org/facilitator"    # facilitator 서버 URL
request_timeout = 30                                # 외부 API 요청 타임아웃 (초)
```

도메인 제어는 config.toml이 아닌 DB 정책(X402_ALLOWED_DOMAINS)으로 관리한다. 결제 한도는 기존 SPENDING_LIMIT 정책을 그대로 사용한다.
...
| 4 | facilitator 의존성 | Coinbase 공개 facilitator 기본 사용, Self-Hosted facilitator 선택적 지원 | x402 스펙상 facilitator는 결제 검증/정산만 담당. 개인키를 facilitator에 노출하지 않으므로 보안 위험 낮음 |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 21 | facilitator /verify 성공 → /settle 실행 → 결제 완료 | mock facilitator(/verify isValid=true → /settle success=true) → 정상 결제 흐름 assert | [L0] |
| 22 | facilitator /verify 실패 → 결제 중단 + 에러 반환 | mock facilitator(/verify isValid=false) → X402_PAYMENT_INVALID 에러 assert | [L0] |
| 23 | facilitator 장애 (타임아웃) → 에러 반환 | mock facilitator 응답 지연 > request_timeout → X402_FACILITATOR_ERROR assert | [L0] |
...
| 2 | facilitator 신뢰 | Coinbase 공개 facilitator는 결제 정산을 대행하므로 일정 수준의 신뢰 필요 | facilitator는 결제 검증/정산만 수행, 개인키 접근 불가. Self-Hosted facilitator 옵션 제공 |

# 마일스톤 v1.5.2: Admin UI 정책 폼 UX 개선

## 목표

Admin UI에서 정책을 JSON 직접 입력 없이 구조화된 폼으로 생성/수정할 수 있는 상태. 10개 정책 타입 각각에 맞는 전용 입력 UI를 제공하여 운영자 DX를 개선한다.
...
- SPENDING_LIMIT만 목록에서 시각화(tier bars)가 되고 나머지 9개는 JSON 요약만 표시
...
|-----------|------------|------------------|
| SPENDING_LIMIT | `{"instant_max":"1000000", ...}` | 4개 금액 입력 필드 (Instant/Notify/Delay/Approval) + 지연 시간(초) + 승인 타임아웃(초) |
...
| 패턴 | 설명 |
|------|------|
| 동적 행 목록 | WHITELIST, ALLOWED_TOKENS, CONTRACT_WHITELIST, APPROVED_SPENDERS에 사용. [+ 추가] 버튼으로 행 추가, 각 행에 [×] 삭제 버튼 |
...
| 컴포넌트 | 내용 |
|----------|------|
| PolicyFormRouter | 정책 타입에 따라 해당 전용 폼 컴포넌트를 렌더링하는 라우터 |
| SpendingLimitForm | 4-tier 금액 + 지연/타임아웃 입력. 기존 TierVisualization과 스타일 일관성 유지 |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 1 | 타입 선택 시 전용 폼 렌더링 | SPENDING_LIMIT 선택 → 4개 금액 필드 존재 assert | [L0] |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 7 | SPENDING_LIMIT 폼 입력 → API 호출 | 4개 금액 입력 + 생성 → POST /v1/policies body에 올바른 rules 포함 assert | [L0] |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 13 | 기존 정책 수정 시 폼에 현재값 프리필 | SPENDING_LIMIT 수정 클릭 → 기존 금액값이 입력 필드에 표시 assert | [L0] |
| 14 | 수정 후 저장 → API 호출 | 금액 변경 + 저장 → PUT /v1/policies/{id} body 검증 assert | [L0] |

### 유효성 검증

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 15 | SPENDING_LIMIT 금액 비숫자 입력 → 에러 | instant_max에 "abc" 입력 → 필드 에러 메시지 표시 assert | [L0] |
| 16 | ALLOWED_TOKENS address 빈 값 → 에러 | 토큰 행의 address 미입력 + 생성 → "주소 필수" 에러 assert | [L0] |

### 목록 시각화

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 17 | ALLOWED_TOKENS 목록에 토큰 심볼 배지 표시 | tokens: [{symbol:'LINK'}, {symbol:'USDC'}] → 배지 2개 렌더링 assert | [L0] |
| 18 | RATE_LIMIT "100 req / 1h" 형식 표시 | {max_requests:100, window_seconds:3600} → "100 req / 1h" 텍스트 assert | [L0] |
...
기존 10개 정책 타입과 Zod 스키마, Admin UI 컴포넌트 시스템(Preact + signals + FormField)을 그대로 활용한다.
...
| 항목 | 예상 |
|------|------|
| 페이즈 | 2개 (동적 행 컴포넌트 + 4-type 폼 1 / 6-type 폼 + 목록 시각화 1) |
| 신규/수정 파일 | 15-18개 |
| 테스트 | 18-25개 |
| DB 마이그레이션 | 없음 (프론트엔드 전용) |

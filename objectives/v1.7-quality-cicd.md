...
| 49 | enum-config-consistency-verification | 10개 Enum SSoT 빌드타임 검증: as const -> TypeScript 타입 -> Zod enum -> Drizzle text enum -> DB CHECK 4단계 방어. 10개 Enum (TransactionStatus 10값, PolicyTier 4값, TransactionType 7값, KillSwitchState 3값, SessionStatus 3값, WalletStatus 5값, OwnerState 3값, NotificationEventType 24값, PolicyType 12값, NotificationLogStatus 2값). config.toml 3단계 로딩 검증 12건 (CF-01~12: 기본값, TOML 오버라이드, 환경변수 오버라이드, 잘못된 값, 누락 키, 음수, 빈 문자열, 다중 섹션, 중첩 금지, 경로 확장, 환경변수 우선, 최종 통합). NOTE-01~11 매핑 중 4건 테스트 필요 (22건 테스트 케이스) | 전체 |
...
| 10개 Enum 전수 | Enum 값 목록 snapshot + 파싱 round-trip 테스트 | Stage 2 (매 PR) |
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 9 | Enum SSoT 빌드타임 검증: as const <-> DB CHECK 일치 | 10개 Enum 파싱 round-trip + generateCheckConstraint 출력 assert | [L0] |
...
| v1.6.1 (WalletConnect Owner 승인) | WalletConnect 세션 관리, 모바일 월렛 서명 검증, APPROVAL/DELAY 승인 흐름의 테스트가 v1.7 보안/통합 테스트 범위에 포함. WC 관련 에러 코드/알림 이벤트도 커버리지 대상 |
...
*최종 업데이트: 2026-02-16 — Enum 카운트 코드베이스 동기화(10개 Enum, 86 에러 코드), IChainAdapter 22 메서드, JupiterSwapProvider→TestESMPlugin, coverage-report→Vitest 호환, x402 보안 ~12건 추가(총 ~249건), v1.6.1 의존 추가, Admin 커버리지 타겟 추가, release.yml 역할 명확화*

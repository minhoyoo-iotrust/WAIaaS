...
| 49 | enum-config-consistency-verification | 9개 Enum SSoT 빌드타임 검증: as const -> TypeScript 타입 -> Zod enum -> Drizzle text enum -> DB CHECK 4단계 방어. 9개 Enum (TransactionStatus 8값, TransactionTier 4값, TransactionType 5값, KillSwitchState 3값, SessionStatus 4값, AgentStatus 3값, OwnerState 3값, NotificationEventType 17값, PolicyType 10값). config.toml 3단계 로딩 검증 12건 (CF-01~12: 기본값, TOML 오버라이드, 환경변수 오버라이드, 잘못된 값, 누락 키, 음수, 빈 문자열, 다중 섹션, 중첩 금지, 경로 확장, 환경변수 우선, 최종 통합). NOTE-01~11 매핑 중 4건 테스트 필요 (22건 테스트 케이스) | 전체 |
...
| 51 | platform-test-scope | 4개 배포 타겟별 테스트 범위: CLI Daemon 32건 (init 4, start 6, stop 5, status 3, signal 5, Windows 2, exit codes 6, E2E 1), Docker 18건 (build 2, compose 2, volume 2, env 2, hostname 1, grace 2, secrets 2, healthcheck 2, non-root 2, auto-init 1), Tauri Desktop 34건 (자동화 6 + 수동 QA 28), Telegram Bot 34건 (polling 5, commands 10, callbacks 7, auth 4, format 2, callback_data 2, direct approve 2, shutdown 2). 총 118건 (자동 90 + 수동 QA 28) | 전체 |
| 64 | extension-test-strategy | v0.6 확장 기능 테스트 전략: Mock 경계 5->10개 확장 (Jupiter API msw 2.x, 가격 API CoinGecko/Pyth/Chainlink mock, 온체인 오라클 mock, IPriceOracle, IActionProvider), Contract Test 5->7개 확장 (IPriceOracle mock vs CoinGeckoOracle 동작 동일성, IActionProvider mock vs JupiterSwapProvider 동일성), ~148개 신규 테스트 시나리오 (토큰 32, 컨트랙트 28, Approve 24, 배치 22, Oracle 20, Action Provider 16, Swap 12, ChainError 12), ~56개 신규 보안 시나리오 (토큰 정책 우회, 컨트랙트 화이트리스트, 무제한 approve, 배치 분할 우회, 가격 조작, 스왑 슬리피지 등). EVM Stub -> Hardhat/Anvil 전환 | 전체 |

### v1.1~v1.6에서 작성된 테스트와 v1.7 추가 범위 구분

| 구분 | 기존 (v1.1~v1.6) | v1.7에서 추가 |
|------|-------------------|-------------|
| 단위 테스트 | 각 마일스톤에서 모듈별 구현과 동시 작성 (커버리지 Soft 40% -> 60%) | 누락분 보완 + Hard 80% 달성을 위한 추가 테스트 |
...
| 보안 테스트 | 각 마일스톤에서 해당 기능의 보안 시나리오 부분 작성 | 71건(46-47) + 166건(64) = 237건 보안 시나리오 전수 통과 |
| 플랫폼 테스트 | CLI 기본 테스트 (v1.1), Docker 기본 (v1.6) | CLI 32건 + Docker 18건 + Tauri 34건 + Telegram 34건 = 118건 전수 |
| Enum 검증 | tsc --noEmit로 빌드타임 검증 (v1.1~) | SSoT 검증 스크립트 + config.toml 12건 + NOTE 테스트 22건 완성 |
| CI/CD | 로컬 테스트 실행 (v1.1~v1.3), 로컬 + PR (v1.4~v1.6) | 4-stage 파이프라인 4개 YAML + 커버리지 게이트 Hard 전환 |
| 크로스 빌드 | 단일 플랫폼 빌드 (v1.6 Tauri) | 5 플랫폼 (macOS ARM64/x64, Windows x64, Linux x64/ARM64) prebuildify |
...
모듈별 Vitest 테스트 스위트. 커버리지 게이트 단계별 기준:
- v1.1~v1.3: Soft 40% (경고만, CI 통과)
- v1.4~v1.6: Soft 60% (경고만, CI 통과)
- **v1.7: Hard 80% (미달 시 PR 머지 차단)**
...
| M7 | 가격 API | msw handlers (CoinGecko/Pyth/Chainlink) | 64-extension-test |
| M8 | 온체인 오라클 | Mock Pyth/Chainlink 프로그램 | 64-extension-test |
...
|---|----------|------|-----------|----------|
| CT-1 | IChainAdapter | MockChainAdapter | SolanaAdapter | 20 메서드 동작 동일성 |
| CT-2 | IChainAdapter | MockChainAdapter | EvmAdapter | 20 메서드 동작 동일성 (buildBatch -> BATCH_NOT_SUPPORTED) |
...
| CT-6 | IPriceOracle | MockPriceOracle | CoinGeckoOracle | getPrice/getSupportedTokens 동작 동일성 |
| CT-7 | IActionProvider | MockActionProvider | JupiterSwapProvider | resolve/execute/getDefinitions 동작 동일성 |
...
총 237건 공격 시나리오. 71건(46-47) + 166건(64).
...
| Tauri Desktop | 6건 | 28건 | 34건 | 빌드 검증 6건 (자동) + 8 화면 UI/UX 28건 (QA 체크리스트) |
...
└── desktop/__tests__/platform/       # Tauri 6건 + QA-CHECKLIST.md
```
...
| 9개 Enum 전수 | Enum 값 목록 snapshot + 파싱 round-trip 테스트 | Stage 2 (매 PR) |
...
- 5 플랫폼 크로스 빌드: macOS ARM64/x64, Windows x64, Linux x64/ARM64
...
| 타겟 | OS | Arch | 빌드 방식 |
|------|-----|------|----------|
| macOS ARM64 | darwin | arm64 | GitHub Actions macos-latest |
| macOS x64 | darwin | x64 | GitHub Actions macos-13 |
| Windows x64 | win32 | x64 | GitHub Actions windows-latest |
| Linux x64 | linux | x64 | GitHub Actions ubuntu-latest |
| Linux ARM64 | linux | arm64 | GitHub Actions ubuntu-latest (cross-compile) |

native addon (sodium-native, better-sqlite3) prebuildify 전략으로 크로스 빌드.
...
| 3 | 보안 테스트 분류 | 별도 `__tests__/security/` 디렉토리 | 보안 시나리오 237건을 레이어별로 구조화. 모듈별 분산 시 전수 확인 어려움. `turbo run test:security`로 독립 실행 가능 |
...
| 6 | nightly -> 보안 시나리오 237건 전수 통과 (71건 + 166건) | nightly.yml full-suite job의 test:security 전수 통과 | [L0] |
| 7 | release tag -> 5 플랫폼 바이너리 빌드 성공 | release.yml matrix 5 타겟 모두 exit code 0 | [L1] |
| 8 | release tag -> Docker 이미지 빌드 + push | release.yml platform-docker job 통과, health check 200 OK | [L1] |
| 9 | release tag -> npm publish (dry-run) | release.yml npm publish --dry-run exit code 0 | [L1] |

### 테스트 프레임워크 검증

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 10 | Enum SSoT 빌드타임 검증: as const <-> DB CHECK 일치 | 9개 Enum 파싱 round-trip + generateCheckConstraint 출력 assert | [L0] |
| 11 | Contract Test: IChainAdapter mock <-> SolanaAdapter 동작 동일성 | 20 메서드 호출 결과 비교 assert (반환 타입 + 에러 타입 일치) | [L0] |
| 12 | Contract Test: IPriceOracle mock <-> CoinGeckoOracle 동작 동일성 | getPrice/getSupportedTokens 반환 스키마 + 캐시 동작 비교 assert | [L0] |

### 블록체인 통합 검증

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 13 | 블록체인 Level 2: solana-test-validator SOL 전송 + SPL 전송 통합 | Airdrop -> buildTransaction -> simulate -> sign -> submit -> waitForConfirmation CONFIRMED assert | [L1] |
| 14 | 블록체인 Level 2: Anvil ETH 전송 + ERC-20 전송 통합 | Anvil 기동 -> ERC-20 deploy + mint -> transfer -> CONFIRMED assert | [L1] |
| 15 | 블록체인 Level 3: Devnet 실 트랜잭션 (nightly only) | Devnet Airdrop -> SOL 전송 -> 잔액 변경 assert (continue-on-error) | [L1] |

### 플랫폼 테스트 검증

| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 16 | 플랫폼 CLI: 32건 시나리오 전수 통과 | release.yml platform-cli job에서 turbo run test:platform 전수 통과 | [L0] |
| 17 | 플랫폼 Docker: 18건 시나리오 전수 통과 | release.yml platform-docker job에서 build + health check + teardown 전수 통과 | [L1] |
...
| 의존 대상 | 이유 |
|----------|------|
| v1.6 (Desktop + Telegram Bot + Docker) | 테스트 대상 코드가 v1.6까지 구현 완료되어야 테스트 작성 가능. CI/CD 파이프라인도 모든 패키지가 존재해야 의미 있는 커버리지 측정 가능 |
...
| 2 | native addon 크로스 빌드 호환성 | sodium-native, better-sqlite3가 5 플랫폼 prebuildify에서 일부 실패 가능 | v0.7 prebuildify 전략 설계 완료. 빌드 실패 플랫폼은 스파이크 후 대응. Linux ARM64는 cross-compile 별도 검증 |
| 3 | CI 실행 시간 | 보안 237건 + 통합 테스트 전수 실행 시 nightly 30분+ 예상 | 보안 테스트 `--maxWorkers=75%` 병렬화. 블록체인 테스트는 nightly에만 배치. Stage 1-2는 `--affected`로 영향 범위만 실행 |
| 4 | 커버리지 게이트 Hard 80% 달성 난이도 | UI 컴포넌트(Tauri React), 외부 API 의존 코드(Jupiter, CoinGecko)의 커버리지 확보 어려움 | Mock 경계 10개로 외부 의존 격리. Tauri 프론트엔드는 adapter-evm과 함께 Low tier(50%+) 적용 검토. 패키지별 독립 Hard 전환으로 점진적 달성 |
| 5 | GitHub Actions 무료 러너 제한 | macOS 빌드 시간이 Linux 대비 3~10배 소요. 월간 사용량 제한 도달 가능 | 크로스 빌드는 release 시만 실행 (매 커밋 아님). macOS x64는 macos-13 러너(Intel)로 ARM64와 분리. 필요 시 self-hosted macOS 검토 |
...
*최종 업데이트: 2026-02-09 v1.0 구현 계획 기반 생성, 설계 문서 46-51/64 참조, 보안 시나리오 237건(71+166) 전수 범위 포함*

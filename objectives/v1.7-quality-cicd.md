...
**native addon prebuildify:**

native addon (sodium-native, better-sqlite3) prebuildify 전략으로 npm 배포 시 플랫폼별 바이너리 제공.

> Desktop 5플랫폼 크로스 빌드 매트릭스 (macOS ARM64/x64, Windows x64, Linux x64/ARM64)는 v2.6.1로 이연.
...
| # | 시나리오 | 검증 방법 | 태그 |
|---|---------|----------|------|
| 9 | Enum SSoT 빌드타임 검증: as const <-> DB CHECK 일치 | 9개 Enum 파싱 round-trip + generateCheckConstraint 출력 assert | [L0] |
| 10 | Contract Test: IChainAdapter mock <-> SolanaAdapter 동작 동일성 | 20 메서드 호출 결과 비교 assert (반환 타입 + 에러 타입 일치) | [L0] |
| 11 | Contract Test: IPriceOracle mock <-> CoinGeckoOracle 동작 동일성 | getPrice/getSupportedTokens 반환 스키마 + 캐시 동작 비교 assert | [L0] |
...
*최종 업데이트: 2026-02-16 Desktop(Tauri) 테스트/빌드를 v2.6.1로 이연. 플랫폼 테스트 118→84건, 5플랫폼 크로스빌드 제거. 설계 문서 46-51/64 참조, 보안 시나리오 237건(71+166) 전수 범위 포함*

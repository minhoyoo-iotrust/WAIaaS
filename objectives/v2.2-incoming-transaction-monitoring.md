...
- cursor pagination (v2.1 Audit Log와 동일 패턴)
...
입금 감지 → INCOMING_TX_DETECTED 이벤트
  ...
  ├── Webhook → 외부 시스템 통보 (v2.1 Webhook 활용)
...
v1.x (구현) → v2.0 (릴리스) → v2.1 (운영 설계) → v2.2 (이 문서)
```

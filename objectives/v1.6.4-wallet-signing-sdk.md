# 마일스톤 v1.6.4: WAIaaS Wallet Signing SDK
...
v1.6.4 완료 시 Owner는 5가지 승인 채널을 선택할 수 있다:

| 우선순위 | 채널 | 마일스톤 | 특징 |
|---------|------|---------|------|
| 1 | **WAIaaS SDK + ntfy** | v1.6.4 | 메신저 불필요, 지갑 앱만으로 동작 |
| 2 | **WAIaaS SDK + Telegram** | v1.6.4 | 메신저 중계, 파트너 지갑 전용 |
...
| ApprovalChannelRouter | 승인 채널 우선순위 라우팅. config 기반: SDK(ntfy) > SDK(Telegram) > WalletConnect > Telegram Bot > REST. 활성 채널 감지 후 자동 라우팅 |
...
preferred_wallet = "dcent"               # Owner 기본 지갑
...
| 항목 | 설명 | 시기 |
|------|------|------|
| Slack 양방향 | Slack Socket Mode 구현으로 서명 응답 수신 | v1.6.4 이후 |
| Discord 양방향 | Discord Gateway Bot 구현으로 서명 응답 수신 | v1.6.4 이후 |
...
| 수정 파일 | 4-6개 (Telegram Bot 명령어, config loader, 승인 우선순위, package.json) |
| 테스트 | 20-24개 |
| DB 마이그레이션 | 없음 (sign_requests는 인메모리 + TTL 만료) |

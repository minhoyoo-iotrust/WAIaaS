/**
 * SSRF protection for external URL fetch.
 *
 * Defense layers:
 * 1. URL normalization (trailing dot, lowercase, userinfo rejection, port restriction)
 * 2. Protocol enforcement (HTTPS only by default; HTTP allowed via option)
 ...
 * @param options - Optional settings (e.g. allowHttp for RPC test endpoints)
...
export async function validateUrlSafety(
  ...
  // Port validation: only 443 (or default empty) for HTTPS-only mode
  if (!allowHttp && url.port && url.port !== '443') {
    throw new WAIaaSError('X402_SSRF_BLOCKED', {

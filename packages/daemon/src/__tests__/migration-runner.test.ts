...
describe('Migration Runner', () => {
  ...
  // all migration versions [1, 2, 3, ..., 42], so getMaxVersion() returns 42.
  // All test migration versions use 43+ to avoid conflicts with real migrations.
  ...
  it('should return { applied: 0, skipped: 0 } for empty migrations array', () => {
    ...
    expect(getMaxVersion()).toBe(42); // v1~v42 (action provider defaults)
  });
  ...
  it('should execute new migrations sequentially', () => {
    ...
    expect(getMaxVersion()).toBe(44);
    expect(getVersions()).toContain(43);
    expect(getVersions()).toContain(44);
  ...
  it('should rollback failed migration and not execute subsequent ones', () => {
    ...
    expect(() => runMigrations(sqlite, migrations)).toThrow(
      /Migration v43.*failed.*Intentional migration failure/,
    );

    // version 43 should NOT be recorded (max stays at 42 from pushSchema)
    expect(getMaxVersion()).toBe(42);

    // version 43 should NOT have been executed
    const columns = sqlite.prepare("PRAGMA table_info('wallets')").all() as Array<{ name: string }>;
    const colNames = columns.map((c) => c.name);
  ...
  it('should execute migrations in ascending version order regardless of array order', () => {
    ...
    const migrations: Migration[] = [
      {
        version: 45,
        description: 'Forty-fifth',
        up: (db) => {
          executionOrder.push(45);
          db.exec('ALTER TABLE wallets ADD COLUMN order_v45 TEXT');
        },
      },
      {
        version: 43,
      ...
      {
        version: 44,
        description: 'Forty-fourth',
        up: (db) => {
          executionOrder.push(44);
          db.exec('ALTER TABLE wallets ADD COLUMN order_v44 TEXT');
        },
      },
    ];
    ...
    expect(executionOrder).toEqual([43, 44, 45]);
    expect(getVersions()).toContain(43);
    expect(getVersions()).toContain(44);
    expect(getVersions()).toContain(45);
  });

  it('should skip version 1-42 migrations (already applied from pushSchema)', () => {
    const migrations: Migration[] = [
    ...
    const result = runMigrations(sqlite, migrations);
    expect(result).toEqual({ applied: 1, skipped: 42 });
    expect(getMaxVersion()).toBe(43);
  });
  ...
  it('should record description in schema_version for applied migrations', () => {
    ...
    const row = sqlite
      .prepare('SELECT description FROM schema_version WHERE version = 43')
...
describe('managesOwnTransaction migrations', () => {
  it('should manage its own PRAGMA and transaction', () => {
    ...
    // schema_version should record version 43
    expect(getMaxVersion()).toBe(43);
  });
  ...
  it('should still allow retry after failure and restore foreign_keys', () => {
    ...
    expect(() => runMigrations(sqlite, migrations)).toThrow(
      /Migration v43.*failed.*Intentional self-managed failure/,
    );

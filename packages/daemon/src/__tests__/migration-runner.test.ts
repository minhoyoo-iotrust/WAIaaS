...
describe('Migration Runner', () => {
  ...
  // all migration versions [1, 2, 3, ..., 18], so getMaxVersion() returns 18.
  // All test migration versions use 19+ to avoid conflicts with real migrations.
  ...
  it('should return { applied: 0, skipped: 0 } for empty migrations array', () => {
    ...
    expect(getMaxVersion()).toBe(18); // v1~v18 (wallets owner_approval_method)
  });
  ...
  it('should execute new migrations sequentially', () => {
    const migrations: Migration[] = [
      {
        ...
        up: (db) => {
          db.exec('ALTER TABLE wallets ADD COLUMN test_col_v19 TEXT');
        },
      },
      {
        version: 20,
    ...
    expect(colNames).toContain('test_col_v19');
  ...
  it('should rollback failed migration and not execute subsequent ones', () => {
    ...
    expect(() => runMigrations(sqlite, migrations)).toThrow(
      /Migration v19.*failed.*Intentional migration failure/,
    );

    // version 19 should NOT be recorded (max stays at 18 from pushSchema)
    expect(getMaxVersion()).toBe(18);

    // version 19 should NOT have been executed
    const columns = sqlite.prepare("PRAGMA table_info('wallets')").all() as Array<{ name: string }>;
    const colNames = columns.map((c) => c.name);
  ...
  it('should execute migrations in ascending version order regardless of array order', () => {
    ...
    expect(executionOrder).toEqual([19, 20, 21]);
    expect(getVersions()).toContain(19);
    expect(getVersions()).toContain(20);
    expect(getVersions()).toContain(21);
  });

  it('should skip version 1-18 migrations (already applied from pushSchema)', () => {
    const migrations: Migration[] = [
      {
        version: 1,
    ...
    const result = runMigrations(sqlite, migrations);
    expect(result).toEqual({ applied: 1, skipped: 18 });
  ...
  it('should record description in schema_version for applied migrations', () => {
    ...
    const row = sqlite
      .prepare('SELECT description FROM schema_version WHERE version = 19')
...
describe('managesOwnTransaction migrations', () => {
  ...
  it('should still allow retry after failure and restore foreign_keys', () => {
    ...
    // Version 21 should NOT be recorded (max stays at 18 from pushSchema)
    expect(getMaxVersion()).toBe(18);

...
describe('Migration Runner', () => {
  ...
  // all migration versions [1, 2, 3, ..., 61], so getMaxVersion() returns 61.
  // All test migration versions use 62+ to avoid conflicts with real migrations.
  ...
  it('should return { applied: 0, skipped: 0 } for empty migrations array', () => {
    ...
    expect(getMaxVersion()).toBe(61); // v1~v61
  });
  ...
  it('should rollback failed migration and not execute subsequent ones', () => {
    ...
    expect(() => runMigrations(sqlite, migrations)).toThrow(
      /Migration v62.*failed.*Intentional migration failure/,
    );

    // version 62 should NOT be recorded (max stays at 61 from pushSchema)
    expect(getMaxVersion()).toBe(61);

    // version 63 should NOT have been executed
    const columns = sqlite.prepare("PRAGMA table_info('wallets')").all() as Array<{ name: string }>;
    const colNames = columns.map((c) => c.name);
  ...
  it('should execute migrations in ascending version order regardless of array order', () => {
    ...
    expect(executionOrder).toEqual([62, 63, 64]);
    expect(getVersions()).toContain(62);
    expect(getVersions()).toContain(63);
    expect(getVersions()).toContain(64);
  });

  it('should skip version 1-61 migrations (already applied from pushSchema)', () => {
    const migrations: Migration[] = [
    ...
    const result = runMigrations(sqlite, migrations);
    expect(result).toEqual({ applied: 1, skipped: 61 });
  ...
  it('should record description in schema_version for applied migrations', () => {
    ...
    const row = sqlite
      .prepare('SELECT description FROM schema_version WHERE version = 62')
...
describe('managesOwnTransaction migrations', () => {
  it('should manage its own PRAGMA and transaction', () => {
    ...
    // schema_version should record version 62
    expect(getMaxVersion()).toBe(62);
  });
  ...
  it('should still allow retry after failure and restore foreign_keys', () => {
    ...
    expect(() => runMigrations(sqlite, migrations)).toThrow(
      /Migration v62.*failed.*Intentional self-managed failure/,
    );

...
describe('Migration Runner', () => {
  ...
  // All test migration versions use 57+ to avoid conflicts with real migrations.
  ...
  it('should rollback failed migration and not execute subsequent ones', () => {
    ...
    expect(() => runMigrations(sqlite, migrations)).toThrow(
      /Migration v57.*failed.*Intentional migration failure/,
    );
  ...
  it('should execute migrations in ascending version order regardless of array order', () => {
    ...
    expect(executionOrder).toEqual([57, 58, 59]);
    expect(getVersions()).toContain(59);
    expect(getVersions()).toContain(58);
    expect(getVersions()).toContain(59);
  });

  it('should skip version 1-56 migrations (already applied from pushSchema)', () => {
    const migrations: Migration[] = [
    ...
    const result = runMigrations(sqlite, migrations);
    expect(result).toEqual({ applied: 1, skipped: 56 });
  ...
  it('should record description in schema_version for applied migrations', () => {
    ...
    const row = sqlite
      .prepare('SELECT description FROM schema_version WHERE version = 57')
...
describe('managesOwnTransaction migrations', () => {
  it('should manage its own PRAGMA and transaction', () => {
    ...
    // schema_version should record version 57
    expect(getMaxVersion()).toBe(57);
  });
  ...
  it('should still allow retry after failure and restore foreign_keys', () => {
    ...
    expect(() => runMigrations(sqlite, migrations)).toThrow(
      /Migration v57.*failed.*Intentional self-managed failure/,
    );

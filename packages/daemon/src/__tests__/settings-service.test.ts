...
describe('SettingsService', () => {
  ...
  describe('setting-keys', () => {
    it('all definitions have valid categories', () => {
      const validCategories = new Set(['notifications', 'rpc', 'security', 'daemon', 'walletconnect', 'oracle', 'display']);
    ...
    it('has expected number of definitions', () => {
      // 9 notifications + 14 rpc + 11 security + 1 daemon + 1 walletconnect + 2 oracle + 1 display = 39
      expect(SETTING_DEFINITIONS.length).toBe(39);
    });
  });
});

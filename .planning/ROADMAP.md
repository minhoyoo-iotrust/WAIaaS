...
### v28.8 빌트인 지갑 프리셋 자동 설정 (In Progress)
...
**Goal**: wallet_type 컬럼이 DB에 존재하고, 빌트인 프리셋 레지스트리가 등록되어 있으며, Owner 등록 API가 wallet_type 필드를 수용하여 프리셋 기반 설정의 기반이 갖춰진 상태
**Depends on**: Nothing (first phase of v28.8)
**Requirements**: PRST-01, PRST-03, PRST-04, API-01, API-02, API-03, DB-01
**Success Criteria** (what must be TRUE):
  1. wallets 테이블에 wallet_type nullable TEXT 컬럼이 존재하고 기존 데이터가 정상 조회된다 (DB v24 마이그레이션)
  2. PUT /v1/wallets/{walletId}/owner 요청에 wallet_type 필드를 포함하면 해당 값이 DB에 저장되고, wallet_type 없이 요청하면 기존과 동일하게 동작한다
...
**Requirements**: PRST-02, PRST-05, ADUI-01
**Success Criteria** (what must be TRUE):
...
Plans:
- [x] 266-01-PLAN.md -- 4단계 자동 설정 파이프라인 (signing SDK + registry + approval_method + preferred_wallet) + Settings 스냅샷 롤백
- [x] 266-02-PLAN.md -- Admin UI Owner 등록 폼 지갑 종류 드롭다운 + 프리셋 선택 연동
...
**Depends on**: Nothing (independent from Phase 265-266)
**Requirements**: RLAY-01, RLAY-02, RLAY-03, RLAY-04
**Success Criteria** (what must be TRUE):
  1. Push Relay config.toml에 [relay.push.payload] 섹션을 추가하면 Zod 검증을 통과하고 static_fields + category_map이 파싱된다
...
**Execution Order:**
Phases execute in numeric order: 265 -> 266 -> 267
(Phase 267 is independent and can run in parallel with 265-266)
...
*Last updated: 2026-02-25 after v28.8 roadmap creation*

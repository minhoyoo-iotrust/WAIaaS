...
### Phase 285: API 키 저장소 통합
**Goal**: API 키가 단일 저장소(SettingsService)에서 관리되어 Admin UI에서 설정한 키가 프로바이더에 즉시 반영된다
**Depends on**: Nothing (first phase)
**Requirements**: APIKEY-01, APIKEY-02, APIKEY-03, APIKEY-04, APIKEY-05, APIKEY-06, APIKEY-07
**Success Criteria** (what must be TRUE):
  1. Admin UI에서 DeFi 프로바이더 API 키를 저장하면 프로바이더가 해당 키로 외부 API를 호출한다 (401 에러 미발생)
  2. `api_keys` 테이블이 존재하지 않으며, 기존 데이터는 `settings` 테이블로 마이그레이션되어 있다 (DB schema v28)
...
Plans:
- [x] 285-01-PLAN.md -- DB migration v28 + admin route delegation to SettingsService
- [x] 285-02-PLAN.md -- ApiKeyStore removal + hot-reload fix + test conversion
...
**Depends on**: Phase 285 (DB migration v28 must precede v29)
**Requirements**: NETID-01, NETID-02, NETID-03, NETID-04, NETID-05, NETID-06, NETID-07, NETID-08, NETID-09, NETID-10, NETID-11
**Success Criteria** (what must be TRUE):
  1. core 상수, DB, API 응답, Admin UI 표시 등 전 레이어에서 Solana 네트워크가 `solana-mainnet` 형식으로 표시된다
  2. CLI/API에서 레거시 `mainnet` 입력 시 `solana-mainnet`으로 자동 변환되고 deprecation 경고가 출력된다

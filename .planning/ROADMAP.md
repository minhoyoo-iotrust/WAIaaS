...
### Phase 206: 지갑 앱 알림 사이드 채널
**Goal**: 지갑 앱(D'CENT 등)이 Telegram 없이 모든 알림 이벤트를 ntfy 사이드 채널로 수신할 수 있는 상태
**Depends on**: Phase 205 (v2.6.1 WAIaaS Wallet Signing SDK)
**Requirements**: SCHEMA-01, SCHEMA-02, SCHEMA-03, DAEMON-01, DAEMON-02, DAEMON-03, DAEMON-04, DAEMON-05, DAEMON-06, SDK-01, SDK-02, SETTINGS-01, SETTINGS-02, SETTINGS-03, ADMIN-01, SYNC-01
**Success Criteria** (what must be TRUE):
  1. sdk_ntfy 지갑에 트랜잭션 완료/정책 위반/세션 이벤트/Kill Switch/시스템 알림이 waiaas-notify-{walletName} ntfy 토픽으로 도착한다
  2. 지갑 앱이 subscribeToNotifications()로 알림을 실시간 수신하고, parseNotification()으로 NotificationMessage를 디코딩/검증한다

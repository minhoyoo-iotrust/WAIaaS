...
- 📋 **v26.3 Push Relay Server** — Phases 207-209 (planned)
...
### v26.3 Push Relay Server (Planned)
...
**Success Criteria** (what must be TRUE):
  1. NtfySigningChannel이 ntfy message 필드에 base64url(JSON.stringify(SignRequest))를 전송한다
  2. NtfySigningChannel이 ntfy title 필드에 displayMessage 텍스트를 설정한다
  3. wallet-sdk subscribeToRequests()가 변경된 인코딩의 메시지를 정상적으로 파싱하여 유효한 SignRequest를 반환한다
**Plans**: TBD
...
**Requirements**: SUB-01, SUB-02, SUB-03, SUB-04, PUSH-01, PUSH-02, PUSH-03, PUSH-04, PUSH-05, PUSH-06, PUSH-07, REG-01, REG-02, REG-03, REG-04, REG-05, INFRA-05, INFRA-06
**Success Criteria** (what must be TRUE):
  1. Relay가 config.toml에 설정된 wallet_names의 서명+알림 토픽을 SSE로 구독하고, 연결 끊김 시 지수 백오프로 자동 재연결한다
  2. Relay가 ntfy 메시지를 base64url 디코딩 + Zod 검증으로 파싱하고, 토픽 패턴으로 서명 요청과 알림을 구분하여 Push data 필드에 전체 JSON을 포함한 푸시를 전송한다
  3. PushwooshProvider(createMessage API)와 FcmProvider(FCM v1 API) 중 설정된 프로바이더로 푸시를 전송하며, 5xx 시 3회 재시도하고 401/403 시 재시도 없이 로그한다
  4. POST /devices로 디바이스 토큰을 등록(upsert)하고, DELETE /devices/:token으로 해제하며, X-API-Key 없는 요청은 401로 거부하고, GET /health로 ntfy 연결 상태와 Push 프로바이더 상태를 반환한다
  5. SIGTERM/SIGINT 수신 시 ntfy SSE 연결을 종료하고 대기 중인 Push 전송을 완료한 뒤 정상 종료한다
**Plans**: TBD
...
**Success Criteria** (what must be TRUE):
  1. @waiaas/push-relay npm 패키지가 빌드되고 package.json의 files/main/types/exports 설정이 올바르다
  2. packages/push-relay/Dockerfile로 waiaas/push-relay Docker 이미지가 빌드되고 정상 실행된다
  3. release-please-config.json에 push-relay 패키지가 등록되어 Conventional Commits로 독립 버전 관리된다
  4. release.yml에 push-relay npm publish + Docker push job이 추가되어 릴리스 시 자동 배포된다
**Plans**: TBD
...
**Execution Order:**
Phases execute in numeric order: 207 → 208 → 209

...
- 🚧 **v27.3 토큰별 지출 한도 정책** -- Phases 235-238 (in progress)
...
### v27.3 토큰별 지출 한도 정책 (In Progress)

**Milestone Goal:** SPENDING_LIMIT 정책의 네이티브 금액 임계값이 raw 단위(lamports/wei)로 모든 토큰에 동일하게 적용되는 문제를 해결한다. USD 기반 임계값을 주 정책으로 유지하면서, 토큰별 사람 읽기 단위의 금액 한도를 선택적으로 추가할 수 있도록 확장한다.

- [ ] **Phase 235: Schema -- Zod SSoT 확장** - TokenLimitSchema 정의, raw 필드 optional 전환, token_limits CAIP-19 record 추가, superRefine 검증
- [ ] **Phase 236: Policy Engine -- 토큰별 티어 평가** - TransactionParam 확장, evaluateTokenTier CAIP-19 매칭, 3곳 callsite tokenContext 전달, 폴백 로직
- [ ] **Phase 237: Admin UI -- 토큰별 한도 편집 폼** - USD 최상단 배치, 네이티브/CAIP-19 토큰 한도 편집기, 레지스트리 연동, legacy deprecated 표시
- [ ] **Phase 238: Compat + Docs -- 하위 호환 검증 및 문서화** - 기존 정책 동일 동작 확인, 우선순위 검증, 누적 한도 비영향 확인, skill 파일 갱신

## Phase Details

### Phase 235: Schema -- Zod SSoT 확장
**Goal**: 토큰별 사람 읽기 단위 한도를 표현할 수 있는 스키마가 정의되고, 기존 raw 필드가 optional로 전환되어 USD만으로 정책을 생성할 수 있다
**Depends on**: Nothing (first phase)
**Requirements**: SCHM-01, SCHM-02, SCHM-03, SCHM-04, SCHM-05, SCHM-06
**Success Criteria** (what must be TRUE):
  1. TokenLimitSchema로 instant_max/notify_max/delay_max를 사람 읽기 단위 문자열로 정의할 수 있다
  2. raw 필드(instant_max/notify_max/delay_max) 없이 USD 또는 token_limits만으로 SpendingLimitRules를 생성하면 Zod 검증을 통과한다
...
**Requirements**: ENGN-01, ENGN-02, ENGN-03, ENGN-04, ENGN-05, ENGN-06, ENGN-07, ENGN-08, ENGN-09, ENGN-10
**Success Criteria** (what must be TRUE):
  1. TOKEN_TRANSFER 트랜잭션이 token_limits에 해당 CAIP-19 키가 있으면 사람 읽기 단위(decimal 변환)로 비교되어 올바른 티어를 반환한다
  2. TRANSFER 트랜잭션이 token_limits에 "native:{chain}" 또는 "native" 키가 있으면 해당 체인의 decimal 기준으로 평가된다
...
**Success Criteria** (what must be TRUE):
  1. Spending Limit 폼에서 USD Tiers 섹션이 최상단에 위치하고, Token-Specific Limits 섹션에서 네이티브 토큰 한도를 설정할 수 있다
...
**Depends on**: Phase 236, Phase 237
...
**Execution Order:**
Phases execute in numeric order: 235 -> 236 -> 237 -> 238
(Phase 236 and 237 both depend on 235; Phase 238 depends on both 236 and 237)
...
| 235 | v27.3 | 0/1 | Not started | - |
| 236 | v27.3 | 0/3 | Not started | - |
| 237 | v27.3 | 0/2 | Not started | - |

...
- 🚧 **v27.4 Admin UI UX 개선** -- Phases 239-243 (in progress)
...
### 🚧 v27.4 Admin UI UX 개선 (In Progress)
...
- [x] **Phase 243: Wallet List + Wallet Detail** - 지갑 목록 검색/필터/잔액 + 지갑 상세 4탭 구조/페이지네이션/USD 표시 (completed 2026-02-22)

## Phase Details

### Phase 239: Foundation -- Shared Components + Admin API
**Goal**: 후속 페이지가 의존하는 공용 UI 컴포넌트와 크로스 지갑 Admin API가 준비되어 있다
**Depends on**: Nothing (first phase of v27.4)
**Requirements**: COMP-01, COMP-02, COMP-03, API-01, API-02
**Success Criteria** (what must be TRUE):
  1. ExplorerLink 컴포넌트에 13개 네트워크의 txHash를 전달하면 각각 올바른 블록 익스플로러 URL이 렌더링된다
  2. FilterBar 컴포넌트가 다중 필드(드롭다운, 날짜 범위 등)를 받아 URL query params로 동기화하며, 필터 변경 시 콜백이 호출된다
...
**Depends on**: Phase 239 (ExplorerLink, FilterBar, SearchInput, GET /v1/admin/transactions)
**Requirements**: TXN-01, TXN-02, TXN-03, TXN-04, TXN-05, TXN-06, DASH-01, DASH-02, DASH-03, DASH-04
**Success Criteria** (what must be TRUE):
  1. /transactions 라우트에서 전체 지갑의 트랜잭션이 시간/지갑명/타입 배지/수신자/금액(+USD)/네트워크/상태 배지/txHash 익스플로러 링크 컬럼으로 표시된다
  2. 지갑/타입/상태/네트워크/날짜 범위 필터와 txHash/수신자 주소 검색으로 트랜잭션을 좁힐 수 있다
...
**Depends on**: Phase 239 (FilterBar, SearchInput)
**Requirements**: TOKR-01, TOKR-02, TOKR-03, TOKR-04, NLOG-01, NLOG-02
**Success Criteria** (what must be TRUE):
  1. /tokens 페이지에서 네트워크 필터로 빌트인 + 커스텀 토큰 목록이 심볼/이름/주소/decimals/source 배지와 함께 표시된다
...
**Depends on**: Phase 239 (FilterBar, GET /v1/admin/incoming)
**Requirements**: INTX-01, INTX-02, INTX-03, INTX-04
**Success Criteria** (what must be TRUE):
  1. settings.tsx의 수신 TX 설정 필드가 /incoming 독립 페이지의 설정 패널로 추출/이전되어 있고, settings.tsx에서는 제거되어 있다
...
**Depends on**: Phase 239 (ExplorerLink, FilterBar, SearchInput)
**Requirements**: WLST-01, WLST-02, WLST-03, WDET-01, WDET-02, WDET-03, WDET-04
**Success Criteria** (what must be TRUE):
  1. 지갑 목록에서 이름/공개키로 실시간 검색 필터링이 되고, 체인/환경/상태로 목록을 필터링할 수 있다
...
**Execution Order:**
Phases execute in numeric order: 239 -> 240 -> 241 -> 242 -> 243

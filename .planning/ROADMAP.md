...
<details>
<summary>✅ v32.2 보안 패치 (Phases 424-426) — SHIPPED 2026-03-16</summary>

- [x] Phase 424: SSRF Guard + hostGuard Fix (1/1 plan) — completed 2026-03-16
- [x] Phase 425: Rate Limit Middleware (1/1 plan) — completed 2026-03-16
- [x] Phase 426: CORS + Resource Management (1/1 plan) — completed 2026-03-16
...
- [x] **Phase 430: as any 제거** - WC/bundler/hot-reload/daemon 등 프로덕션 as any 55건 제거, JSON.parse Zod 추가 (completed 2026-03-16)
  **Plans:** 3 plans
  Plans:
...
- [x] **Phase 431: SSoT 통합 + 설정 정리** - NATIVE_DECIMALS/formatAmount 등 중복 통합, 팬텀 설정/stale 참조 정리 (completed 2026-03-16)
  **Plans:** 3 plans
  ...
  - [ ] 431-02-PLAN.md — formatDisplayCurrency 동기화 + aggregateStakingBalance 공유 추출 + sweepAll/stage naming/hintedTokens 정리
  - [ ] 431-03-PLAN.md — Settings configPath 감사 + stale phase 주석 정리 + SSoT 불변성 테스트

## Phase Details

### Phase 427: Core Exports + safeJsonParse 유틸리티
...
**Success Criteria** (what must be TRUE):
  ...
  3. Settings configPath 5건이 실제 동작과 일치하고, stale phase 참조 주석 40+건이 제거되었다
  4. pnpm turbo run test:unit이 전체 통과한다

## Progress

**Execution Order:**
Phases execute in numeric order: 427 -> 428 -> 429 -> 430 -> 431
(Phase 428과 429는 427에만 의존하므로 병렬 가능하나, 순차 실행 권장)

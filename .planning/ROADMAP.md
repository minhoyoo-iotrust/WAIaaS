...
### v28.2 0x EVM DEX Swap (In Progress)
...
**Goal**: 운영자가 Admin UI에서 ActionProvider를 관리하고, 프로바이더가 다중 트랜잭션(approve+swap)을 순차 파이프라인으로 실행할 수 있다
**Depends on**: Nothing (first phase of v28.2)
**Requirements**: PINF-01, PINF-02, PINF-03, PINF-04, PINF-05, PINF-06, PINF-07, PINF-08
**Success Criteria** (what must be TRUE):
  1. Admin UI Actions 페이지에서 빌트인 프로바이더(Jupiter, 0x) 목록이 표시되고, 활성화 토글과 API 키 입력이 동작한다
  2. config.toml [actions] 섹션 없이 Admin Settings만으로 프로바이더 설정이 관리되며, 기존 Jupiter 설정이 정상 이관된다
...
**Requirements**: ZXSW-01, ZXSW-02, ZXSW-03, ZXSW-04, ZXSW-05, ZXSW-06, ZXSW-07, ZXSW-08, ZXSW-09, ZXSW-10
**Success Criteria** (what must be TRUE):
  1. ERC-20 토큰 판매 시 approve + swap 두 건의 트랜잭션이 순차 실행되어 스왑이 완료된다
  2. ETH(네이티브) 판매 시 swap 단일 트랜잭션으로 스왑이 완료된다
...
**Success Criteria** (what must be TRUE):
  1. TS SDK에서 client.executeAction('0x-swap', 'swap', params) 호출로 스왑이 실행된다
...
**Execution Order:** 248 -> 249 -> 250

| Phase | Milestone | Plans Complete | Status | Completed |
|-------|-----------|----------------|--------|-----------|
| 248. Provider Infrastructure | v28.2 | 0/3 | Not started | - |

...
must_haves:
  truths:
    ...
    - "Gap recovery after reconnect calls pollAll() on the correct subscriber and routes recovered TXs through the queue"
...
2. Create the gap recovery handler (createGapRecoveryHandler) with a mock subscribers Map containing a subscriber with a mock pollAll(). Call the handler. Verify: pollAll() is called once. This tests the handler factory in isolation to ensure it delegates correctly.
3. Simulate a 50-block gap: create a gap recovery handler with a subscriber whose pollAll() pushes 50 mock IncomingTransactions into a shared IncomingTxQueue (via the onTransaction callback). Call the handler, then flush the queue to a mock DB. Verify: all 50 transactions are in the DB (50 stmt.run calls). This tests gap recovery -> queue -> flush -> DB path end-to-end.
...
1. KillSwitch ACTIVE: Create IncomingTxMonitorService with killSwitch.getState() returning {state:'ACTIVE'}. Push 3 transactions to internal queue. Execute flush handler. Verify: 3 eventBus.emit('transaction:incoming') calls AND 3 notificationService.notify calls (first one only, others may be cooldown-suppressed -- so verify at least 1 notify call).
2. KillSwitch SUSPENDED: Same setup but killSwitch returns {state:'SUSPENDED'}. Push 3 transactions. Execute flush handler. Verify: 3 eventBus.emit('transaction:incoming') calls (events always fire), 0 notificationService.notify calls.
3. KillSwitch LOCKED: Same as above with {state:'LOCKED'}. Verify same behavior: events fire, no notifications.
4. KillSwitch SUSPENDED -> DB writes: Push a suspicious transaction (unknown token, mock token_registry query returns undefined). Execute flush handler. Verify: DB UPDATE is_suspicious=1 is called (DB writes persist), eventBus.emit('transaction:incoming:suspicious') fires, but notificationService.notify is NOT called.
5. KillSwitch transitions: Execute flush with ACTIVE (notification sent), then change mock to SUSPENDED, execute another flush (notification suppressed). Verify the transition is respected within the same service instance.
...
2. Gap recovery tests verify the subscriber -> reconnect -> pollAll -> queue -> DB flow
...
6. ROADMAP success criteria #4 (KillSwitch SUSPENDED -> DB yes, events no) and #5 (50 dust -> <=5 notifications) are directly verified
</success_criteria>

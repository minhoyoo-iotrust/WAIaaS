...
for (const tx of inserted) {
  ...
  if (isSuspicious) {
    eventBus.emit('transaction:incoming:suspicious', { ...tx, reasons: suspiciousReasons, timestamp: tx.detectedAt });
  }
...
try {
  if (this.sqlite && this.adapterPool && this._settingsService) {
    ...
    if (incoming_enabled === 'true') {
      ...
      const subscriberFactory = (chain: string, network: string) => {
        // Dynamic import based on chain type
        // This will be resolved in the service's start() method
        throw new Error('subscriberFactory must be set up with chain-specific imports');
      };
...
Note: The subscriberFactory implementation will import SolanaIncomingSubscriber and EvmIncomingSubscriber dynamically based on the chain parameter. Use `await import()` pattern similar to how WalletConnect and TelegramBot are dynamically imported.
...
**B. HotReloadOrchestrator (hot-reload.ts):**

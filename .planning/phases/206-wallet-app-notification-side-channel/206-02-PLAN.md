...
1. **NotificationService integration (DAEMON-02)**:
   ...
   d) In the `notify()` method, AFTER the existing broadcast/sendWithFallback call and BEFORE the method returns, add side channel delivery. The side channel runs in parallel and independently:
   ```ts
   // Side channel: wallet app notification (independent, never blocks)
   ...
   Note: The side channel call is fire-and-forget. It does not affect the return value or error handling of the existing channels (DAEMON-02, DAEMON-06).

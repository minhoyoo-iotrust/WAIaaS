...
1. `packages/daemon/src/services/signing-sdk/sign-request-builder.ts` 생성:
   ...
   **의존성:**
   - @waiaas/core: SignRequest, SignRequestSchema, encodeSignRequest, buildUniversalLinkUrl
   ...
   **buildRequest(params) 메서드:**
   ...
   로직:
   1. `signing_sdk.enabled` 확인 -> false면 WAIaaSError('SIGNING_SDK_DISABLED', 403) throw
...
2. **테스트** (`sign-request-builder.test.ts`):
  ...
  - signing_sdk.enabled=false 시 에러 반환
...
1. `packages/daemon/src/services/signing-sdk/sign-response-handler.ts` 생성:
   ...
   **의존성:**
   - @waiaas/core: SignResponse, SignResponseSchema, SignRequest
   ...
   **handle(signResponse: SignResponse): Promise<{ action: 'approved' | 'rejected', txId: string }>**
   1. SignResponseSchema.parse(signResponse) -- Zod 검증, 실패 시 WAIaaSError('INVALID_SIGN_RESPONSE', 400)
   2. pendingRequests에서 requestId로 원본 SignRequest 조회 -- 없으면 WAIaaSError('SIGN_REQUEST_NOT_FOUND', 404)
   3. 만료 확인: `new Date() > new Date(request.expiresAt)` → WAIaaSError('SIGN_REQUEST_EXPIRED', 408)
   4. action == 'approve' 분기:
      a. signature 필드 필수 확인 -- 없으면 WAIaaSError('INVALID_SIGN_RESPONSE', 400, 'Missing signature for approve action')
      b. signerAddress가 해당 지갑의 ownerAddress와 일치 확인 -- DB에서 wallets 테이블 조회. 불일치 시 WAIaaSError('SIGNER_ADDRESS_MISMATCH', 403)
      c. 서명 검증:
         ...
         - 검증 실패: WAIaaSError('INVALID_SIGNATURE', 401)
   ...
   6. 이미 처리된 requestId 재수신 시: WAIaaSError('SIGN_REQUEST_ALREADY_PROCESSED', 409)

   **참고:**
   - EVM 서명 검증은 viem의 verifyMessage 사용 (패키지에 이미 viem 의존성 있음)
   - Solana 서명 검증은 기존 ownerAuth 패턴 참조 (@solana/kit 또는 tweetnacl 사용)
   - 실제 트랜잭션 상태 변경 로직은 기존 approve/reject 핸들러의 DB 조작 패턴을 참조하되, 이 클래스에서는 pending_approvals/transactions 테이블의 직접 업데이트 수행 (Drizzle ORM 사용)
...
<verification>
1. `pnpm turbo run typecheck --filter=@waiaas/daemon` -- 타입 체크 통과

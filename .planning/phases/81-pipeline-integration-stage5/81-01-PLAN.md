...
<task type="auto">
  ...
  <action>
  **구현 순서:** stages.ts (Stage 1 -> Stage 3) -> pipeline.ts (타입 확장) -> transactions.ts (route 수정). stages.ts가 핵심이고 나머지 2개는 타입 호환 + 글루 코드.
  ...
  **Stage 3 (stage3Policy) 수정 -- stages.ts:**
  ...
  2. type별 TransactionParam 구성:
     ...
     - `BATCH`: evaluateBatch 전용 경로로 분기 (아래 상세 의사코드 참조)
  3. BATCH 처리 상세 의사코드:
     ```typescript
     if (txType === 'BATCH') {
       const batchReq = ctx.request as BatchRequest;
       // classifyInstruction 로직으로 각 instruction의 type 판별
       // (SolanaAdapter.classifyInstruction과 동일 패턴: spender->APPROVE, token->TOKEN_TRANSFER, programId->CONTRACT_CALL, else->TRANSFER)
       const params: TransactionParam[] = batchReq.instructions.map(instr => {
         ...
         return {
           type: instrType,
           amount: instr.amount ?? '0',
           toAddress: instr.to ?? '',
           chain: ctx.chain,
           tokenAddress: 'token' in instr ? (instr as any).token?.address : undefined,
           contractAddress: instrType === 'CONTRACT_CALL' ? instr.to : undefined,
           selector: 'calldata' in instr ? (instr as any).calldata?.slice(0, 10) : undefined,
           spenderAddress: 'spender' in instr ? (instr as any).spender : undefined,
           approveAmount: instrType === 'APPROVE' ? instr.amount : undefined,
         };
       });
       // evaluateBatch: 2-stage policy (per-instruction + aggregate)
       const evaluation = await policyEngine.evaluateBatch(agentId, params);
       // evaluation 결과를 ctx에 설정 (기존 evaluate 결과와 동일 형태)
     }
     ```
     - DatabasePolicyEngine.evaluateBatch(agentId: string, instructions: TransactionParam[]) 호출
     - evaluateBatch 결과를 evaluation으로 사용 (반환 타입: PolicyEvaluation)

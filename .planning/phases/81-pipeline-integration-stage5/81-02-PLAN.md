...
must_haves:
  truths:
    - "Stage 5가 build->simulate->sign->submit 루프를 실행하고, PERMANENT ChainError는 즉시 FAILED, TRANSIENT는 지수 백오프(1s,2s,4s, max 3회 재시도 = retryCount >= 3 guard), STALE는 buildLoop 복귀(max 1회 = retryCount >= 1 guard) 분기를 수행한다"
    ...
    - "TRANSIENT retryCount >= 3이면 FAILED로 마킹하고 더 이상 재시도하지 않는다 (초기 시도 1회 + 재시도 3회 = 총 4회 시도)"
...
<task type="auto">
  ...
  **ChainError 카테고리별 재시도 테스트 (7개):**
  ...
  3. TRANSIENT ChainError 4번째 시도(retryCount=3)에서 FAILED 마킹 (초기 1회 + 재시도 3회 소진)
...
<task type="auto">
  ...
  **stage5Execute 완전 재작성 -- stages.ts:**
  ...
  2. **stage5Execute에 CONC-01 재시도 루프 추가:**
     - import `ChainError` from `@waiaas/core` (반드시 @waiaas/core에서 import)
     - `let retryCount = 0;`
     - 외부 buildLoop: `while (!controller.signal.aborted)` (CONC-01 의사코드 그대로)
     ...
     - switch(err.category):
       ...
       - TRANSIENT: `if (retryCount >= 3)` -> FAILED + throw. 아니면 `await sleep(1000 * 2**retryCount)`, retryCount++, continue (실패 단계 재시도). 결과: 초기 시도 1회 + 재시도 최대 3회 = 총 4회 시도
       - STALE: `if (retryCount >= 1)` -> FAILED + throw. 아니면 retryCount++, continue buildLoop (Stage 5a 복귀)
     - retryCount는 TRANSIENT과 STALE이 합산됨 (CONC-01 Note 참조)
  ...
  `grep -n "import.*ChainError.*from.*@waiaas/core" packages/daemon/src/pipeline/stages.ts` -- ChainError가 @waiaas/core에서 정상 import 확인
  `grep -n "instanceof ChainError" packages/daemon/src/pipeline/stages.ts` -- stage5Execute에서 ChainError instanceof 체크 사용 확인
  </verify>
  <done>
  Stage 5가 CONC-01 완전 구현:
  - ChainError를 @waiaas/core에서 import하고 instanceof로 에러 분기
  - buildByType로 5-type adapter 메서드 라우팅
  - PERMANENT 즉시 FAILED, TRANSIENT 지수 백오프(max 3회 재시도, retryCount >= 3), STALE 재빌드(max 1회, retryCount >= 1)
...
6. Stage 5: retryCount 합산 + TRANSIENT retryCount >= 3 / STALE retryCount >= 1 guard 확인
...
- TRANSIENT ChainError -> 지수 백오프(1s,2s,4s), retryCount >= 3 guard (초기 1회 + 재시도 3회 = 총 4회 시도)
- STALE ChainError -> buildLoop 복귀, retryCount >= 1 guard (retryCount 합산)

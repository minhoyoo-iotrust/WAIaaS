...
6. Verify AdminRouteDeps interface updated: `grep -A 10 'export interface AdminRouteDeps' packages/daemon/src/api/routes/admin.ts | grep 'adminTimeout'` shows the new field in the interface
  </verify>
  <done>
...
Import serveStatic, CSP middleware, and path utilities:
```ts
...
Compute `ADMIN_STATIC_ROOT` as an absolute path at module level using `fileURLToPath` + `dirname` + `join`. This ensures the path resolves correctly regardless of CWD (the daemon may be started from any directory via CLI):
```ts
...
- `ADMIN_STATIC_ROOT` is an absolute path computed from `import.meta.url`, so it resolves correctly even when CWD differs from the package root (e.g., daemon started via `waiaas start` from a data directory).
- CSP middleware is registered BEFORE serveStatic so the headers are set on the response.
- The Admin UI uses hash routing (`preact-router` + hash history, tech decision #14), so the server-side SPA fallback to index.html is a safety net for direct URL access.
...
5. Verify ADMIN_STATIC_ROOT uses absolute path from import.meta.url: `grep 'ADMIN_STATIC_ROOT' packages/daemon/src/api/server.ts` shows `fileURLToPath` + `dirname` + `join` pattern
  </verify>
  <done>

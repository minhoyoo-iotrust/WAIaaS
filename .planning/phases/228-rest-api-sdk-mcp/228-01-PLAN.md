...
2. **Create packages/daemon/src/api/routes/incoming.ts**:
   ...
   **GET /wallet/incoming handler:**
   ...
   - Return JSON with `data`, `nextCursor`, `hasMore`
   ...
   **GET /wallet/incoming/summary handler:**
   ...
   - SQL query: `SELECT date_col, COUNT(*) as total_count, SUM(CASE WHEN suspicious = 1 THEN 1 ELSE 0 END) as suspicious_count FROM incoming_transactions WHERE wallet_id = ? AND status = 'CONFIRMED' [AND chain = ? AND network = ?] [AND detected_at >= ? AND detected_at <= ?] GROUP BY date_col ORDER BY date_col DESC`
   - **For totalAmountNative:** SQL does NOT SUM amounts. Instead, for each date period, fetch all `amount` values and sum using JavaScript BigInt in app layer to avoid overflow.
     ...
     - Actually, most efficient: single raw SQL query fetching `date_col, amount, suspicious` for all matching rows (no GROUP BY), then aggregate entirely in JS. This is cleanest for BigInt handling.

...
<task type="auto">
  <name>Task 1: Create Zod schemas + Policy CRUD routes + tests</name>
  <files>
    packages/core/src/schemas/policy.schema.ts
    packages/daemon/src/api/routes/policies.ts
    packages/daemon/src/api/routes/index.ts
  ...
  <done>
    POST/GET/PUT/DELETE /v1/policies all functional with masterAuth protection, SPENDING_LIMIT and WHITELIST rules validated, 11 CRUD tests pass, TypeScript compiles clean.
  </done>
</task>

<task type="auto">
  <name>Task 2: Add TOCTOU prevention pattern with reserved_amount</name>
  <files>
    packages/daemon/src/infrastructure/database/schema.ts
    packages/daemon/src/pipeline/database-policy-engine.ts
  ...
  <done>
    evaluateAndReserve uses BEGIN IMMEDIATE to serialize concurrent evaluations, reserved_amount column added to transactions schema, releaseReservation clears reservations, 2+ TOCTOU tests pass, all existing daemon tests pass without regression.
  </done>
</task>

...
**IMPORTANT: Test group isolation.** This plan covers 3 features (sessionId audit, evaluateAndReserve TOCTOU, Owner downgrade). Use `describe()` blocks to isolate each feature so test failures are unambiguous:

```typescript
describe('56-01 Pipeline Integration', () => {
  describe('Feature A: sessionId audit trail', () => { /* tests 1-3 */ });
  describe('Feature B: evaluateAndReserve TOCTOU-safe policy', () => { /* tests 4-6 */ });
  describe('Feature C: downgradeIfNoOwner integration', () => { /* tests 7-8 */ });
});
```

**Test group 1: Feature A - sessionId audit trail**
...
**Test group 2: Feature B - evaluateAndReserve TOCTOU-safe policy**
...
**Test group 3: Feature C - downgradeIfNoOwner integration**
...
Add `sqlite?: SQLiteDatabase` (import `Database as SQLiteDatabase` from `better-sqlite3`) to `TransactionRouteDeps`:
...
export interface TransactionRouteDeps {
  ...
  sqlite?: SQLiteDatabase;   // <-- ADD: raw better-sqlite3 for evaluateAndReserve
}
```

In the POST `/transactions/send` handler, replace the current PipelineContext construction (lines ~107-121) with the full object that includes `sessionId` and `sqlite`:
...
```typescript
    ...
    const ctx: PipelineContext = {
      ...
      request,
      txId,
      // --- NEW: sessionId from Hono context (set by sessionAuth middleware) ---
      sessionId: c.get('sessionId' as never) as string | undefined,
      // --- NEW: raw sqlite for evaluateAndReserve TOCTOU safety ---
      sqlite: deps.sqlite,
    };
```

The `sessionId` value comes from `c.get('sessionId')` which is set by the `createSessionAuth` middleware in `session-auth.ts` (line 66: `c.set('sessionId', payload.sub)`).

Also in the stage1 synchronous INSERT (lines ~86-95), add `sessionId` to the INSERT values so it appears in the audit trail:
...
```typescript
    await deps.db.insert(transactions).values({
      ...
      createdAt: now,
      // --- NEW: session audit trail ---
      sessionId: (c.get('sessionId' as never) as string | undefined) ?? null,
    });
```

Finally, update `server.ts` to pass `sqlite` to `transactionRoutes`. In the `createApp()` function where `transactionRoutes()` is called (lines ~181-194), add `sqlite: deps.sqlite`:
...
```typescript
    app.route(
      ...
      transactionRoutes({
        ...
        sqlite: deps.sqlite,  // <-- ADD: pass raw sqlite for evaluateAndReserve
      }),
    );
```

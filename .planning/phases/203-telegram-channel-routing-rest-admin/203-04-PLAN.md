...
must_haves:
  truths:
    ...
    - "미구성 인프라(예: ntfy 미설정 + sdk_ntfy) 선택 시 경고 메시지를 표시한다"
  artifacts:
...
3. **Fetch settings on mount**: Add a `fetchApprovalSettings` function that calls `apiGet(API.ADMIN_SETTINGS)` and extracts:
   - `signingEnabled`: `settings.signing_sdk?.enabled?.value === 'true'` (check pattern from existing SettingsData usage)
   - `ntfyConfigured`: `settings.notifications?.ntfy_topic?.value` is non-empty
   ...
   const handleApprovalMethodChange = async (method: string | null) => {
     try {
       await apiPut(API.WALLET_OWNER(id), {
         ...
         approval_method: method || undefined,
       });
       await fetchWallet();
...
`pnpm turbo run typecheck --filter=@waiaas/admin` passes.
  ...
  <done>WalletDetail interface includes approvalMethod, settings availability signals loaded, change handler wired to PUT API</done>
</task>
...
const APPROVAL_OPTIONS: Array<{
  ...
  },
  {
    ...
    warning: 'ntfy topic is not configured. Go to Notifications settings to set up ntfy.',
    warningCondition: (s) => !s?.ntfyConfigured,
  },
  {
    value: 'sdk_telegram',
    label: 'SDK via Telegram',
    description: 'Send sign request to Telegram with universal link',
    warning: 'Telegram bot is not enabled or configured. Go to System > Telegram settings.',
...
Note: The `null` value for "Auto" maps to sending `approval_method: undefined` in the PUT request (omitting the field). To clear an existing approval_method back to auto, the handler should send an explicit null or handle it differently. Actually, looking at the backend, if approval_method is not in the body, it won't be changed. So to clear it to null, we need to either:
- Add a separate "clear" action, OR
- Send `approval_method: null` (but the schema uses `.optional()` not `.nullable()`)

**Decision:** Update the backend schema in Plan 02 to accept `null` explicitly: change `ApprovalMethodSchema.optional()` to `ApprovalMethodSchema.nullable().optional()`. If Plan 02 is already executed without this, handle it in the Admin UI by sending a separate request. Actually, simpler approach: define a special value like empty string or simply rely on the existing optional behavior. The cleanest is: in the PUT handler (Plan 02), when approval_method is explicitly `null`, set `owner_approval_method = NULL` in DB. In the Zod schema, use `.nullable().optional()`.

In this plan, send `approval_method: null` for the "Auto" option, which clears the column to NULL.
  </action>
  <verify>
...
Visual check: The Admin wallet detail page shows radio buttons for approval method selection.
  ...
  <done>Admin UI shows 6 radio options (Auto + 5 methods) in wallet detail page, infrastructure warnings appear for unconfigured channels, selection triggers PUT API to save approval_method</done>
</task>
...
4. Infrastructure warning visible when selecting sdk_ntfy without ntfy configured
</verification>
...
- Auto option clears approval_method to null (global fallback)

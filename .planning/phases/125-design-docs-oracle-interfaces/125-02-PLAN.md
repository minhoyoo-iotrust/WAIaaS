...
<feature>
  <name>IPriceOracle Interface + InMemoryPriceCache + classifyPriceAge</name>
  <files>
    packages/core/src/interfaces/price-oracle.types.ts
    packages/core/src/interfaces/index.ts
    packages/core/src/index.ts
  ...
  <behavior>
    ...
    **InMemoryPriceCache (daemon 패키지):**
    - constructor(ttlMs=300000, staleMaxMs=1800000, maxEntries=128)
    - get(key) → CacheEntry | null (TTL 이내이면 stats.hits++, 만료이면 null + stats.misses++)
    - getStale(key) → CacheEntry | null (staleMax 이내이면 stats.staleHits++, 만료 데이터 반환, 초과이면 null)
    - set(key, price) → void (LRU 퇴거: size >= maxEntries && !has(key) → oldest 제거 + stats.evictions++)
    - getOrFetch(key, fetcher) → Promise<PriceInfo> (cache stampede 방지: inflight Promise 공유)
    - getStats() → CacheStats
    - clear() → void
    - buildCacheKey(chain, address) → string (EVM addresses lowercase-normalized: `${chain}:${address.toLowerCase()}` for ethereum, `${chain}:${address}` for solana)
    - LRU touch: get() 시 Map delete+set으로 항목을 최신 위치로 이동

    **classifyPriceAge (daemon 패키지):**
    - classifyPriceAge(fetchedAt, now?) → PriceAge
    - FRESH: age < 5min (300,000ms)
    - AGING: 5min <= age < 30min (1,800,000ms)
    - STALE: age >= 30min
    - now 미지정 시 Date.now() 사용 (테스트에서 주입 가능)
    - PriceAgeEnum: z.enum(['FRESH', 'AGING', 'STALE'])
  ...
  <implementation>
    **Step 1 — RED: 테스트 작성 (core types는 별도 테스트 불필요 — Zod 스키마 자체가 검증)**

    1-a. `packages/daemon/src/__tests__/price-age.test.ts` 작성:
    - `import { classifyPriceAge, PriceAgeEnum, PRICE_AGE_THRESHOLDS } from '../infrastructure/oracle/price-age.js'`
    - 위 behavior의 9개 테스트 케이스를 vitest describe/it으로 작성
    - vi.useFakeTimers()는 사용하지 않음 — classifyPriceAge의 `now` 파라미터로 시간 제어

    1-b. `packages/daemon/src/__tests__/price-cache.test.ts` 작성:
    - `import { InMemoryPriceCache, buildCacheKey } from '../infrastructure/oracle/price-cache.js'`
    - `import type { PriceInfo } from '@waiaas/core'`
    - TTL/LRU/stampede 테스트를 위해 vi.useFakeTimers() 활용 (vi.advanceTimersByTime으로 시간 경과 시뮬레이션)
    - 위 behavior의 15개 테스트 케이스를 vitest describe/it으로 작성
    - 테스트용 PriceInfo 헬퍼: `makePriceInfo(overrides?)` 팩토리 함수를 테스트 파일 상단에 정의

    1-c. 테스트 실행 → 모든 테스트 FAIL 확인 (import 실패)

    **Step 2 — GREEN: 구현**

    2-a. `packages/core/src/interfaces/price-oracle.types.ts` 생성:
    - Research에서 제시한 코드를 기반으로 TokenRefSchema, PriceInfoSchema, CacheStats, IPriceOracle 정의
    - source enum은 반드시 `['pyth', 'coingecko', 'cache']` 3가지만 (v1.5 결정)
    - ChainTypeEnum은 `import { ChainTypeEnum } from '../enums/chain.js'`로 가져옴
    - TokenRefSchema.chain은 ChainTypeEnum 사용

    2-b. `packages/core/src/interfaces/index.ts` 수정:
    ...
    2-e. `packages/daemon/src/infrastructure/oracle/price-cache.ts` 생성:
    - Research에서 제시한 InMemoryPriceCache 코드를 기반으로 구현
    - CacheEntry 내부 인터페이스: { price: PriceInfo, cachedAt: number, expiresAt: number }
    - get(): Map에서 키 조회 → 존재하면 TTL 체크 → 유효하면 LRU touch(delete+set) + stats.hits++ → 만료이면 null + stats.misses++
    - getStale(): TTL 만료 && staleMax 이내이면 데이터 반환 + stats.staleHits++
    - set(): maxEntries 초과 시 Map.keys().next().value로 oldest 제거 + stats.evictions++
    - getOrFetch(): 캐시 히트 → 반환, inflight 있으면 대기, 없으면 새 fetch 시작 → finally에서 inflight 삭제
    - buildCacheKey(): chain === 'ethereum' ? `${chain}:${address.toLowerCase()}` : `${chain}:${address}`
    - 시간 비교는 Date.now()를 내부에서 호출 (테스트에서 vi.useFakeTimers()로 제어)

    2-f. `packages/daemon/src/infrastructure/oracle/index.ts` 생성:
    ```typescript

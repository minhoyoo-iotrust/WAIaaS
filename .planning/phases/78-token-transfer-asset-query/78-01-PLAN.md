...
must_haves:
  truths:
    - "SolanaAdapter.buildTokenTransfer builds SPL transferChecked instruction for Token Program tokens"
    - "SolanaAdapter.buildTokenTransfer automatically detects Token-2022 mint and uses correct program ID"
    - "SolanaAdapter.buildTokenTransfer creates ATA instruction when destination ATA does not exist"
    - "SolanaAdapter.getTokenInfo returns decimals/symbol/name from Solana mint account"
    - "SolanaAdapter.estimateFee returns fee estimate including ATA rent cost when applicable"
    - "SolanaAdapter.getTransactionFee returns estimatedFee from built transaction"
    - "SolanaAdapter.getAssets includes Token-2022 program accounts in addition to Token Program"
    - "ALLOWED_TOKENS policy evaluation denies token transfers when no ALLOWED_TOKENS policy exists"
    - "ALLOWED_TOKENS policy evaluation allows token transfers only for whitelisted token addresses"
  artifacts:
...
```
All tests pass (existing + new). Build succeeds with no TypeScript errors.
  </verify>
  <done>
...
**6. Update daemon test mock adapters**: If any daemon test mock adapters need updates for the new `tokenAddress` field in transaction objects, update them. The mock adapters already have the v1.4 stub methods from Phase 76, so no IChainAdapter changes needed. Only update if the DatabasePolicyEngine evaluate() signature change causes TypeScript errors in test files.
  </action>
  <verify>
```bash
cd /Users/minho.yoo/dev/wallet/WAIaaS

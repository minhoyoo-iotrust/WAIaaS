...
must_haves:
  truths:
    ...
    - "EvmAdapter.getAssets returns native ETH + ERC-20 token balances for provided token addresses via multicall"
    - "EvmAdapter.getAssets accepts optional allowedTokens parameter for ALLOWED_TOKENS-based conservative query"
  artifacts:
...
- The daemon service layer will be responsible for loading ALLOWED_TOKENS from the policy DB and calling `setAllowedTokens()` before `getAssets()` -- this integration happens in Phase 81 (pipeline integration)

...
must_haves:
  truths:
    ...
    - "WalletSchema 변경 설계(network -> environment + defaultNetwork)가 Zod 스키마 수준으로 명시되어 있고, default_network의 NULL 의미 정책(NULL=환경 기본값, NOT NULL=사용자 지정)이 확정되어 있다"
  artifacts:
...
- **default_network NULL 의미 정책 (설계 결정 ENV-07로 확정):**
  - `default_network` 컬럼은 **nullable**로 정의한다
  - NULL 의미: "환경 기본값 사용" -- 런타임에서 `getDefaultNetwork(chain, environment)`로 해결
  - NOT NULL 의미: "사용자 지정 네트워크" -- DB에 저장된 값을 그대로 사용
  - 마이그레이션 시: 기존 `wallets.network` 값을 `default_network`에 복사하므로 마이그레이션 직후에는 NOT NULL (기존 데이터 보존)
  - API에서 향후 `PATCH /v1/wallets/:id { defaultNetwork: null }`로 환경 기본값으로 리셋 허용
  - Zod 검증: `NetworkTypeEnum.nullable()` -- NULL 또는 유효한 NetworkType만 허용
  - DB CHECK: `CHECK (default_network IS NULL OR default_network IN ({NETWORK_TYPES}))` -- nullable이므로 IS NULL 분기 필수
  - 이 결정은 Plan 105-02의 v6b wallets_new CREATE TABLE DDL과 INSERT CASE 분기에 직접 영향:
    - DDL: `default_network TEXT CHECK (default_network IS NULL OR default_network IN (...))` (NOT NULL 제약 없음)
    - INSERT: `network AS default_network` (기존 값 그대로 보존, 마이그레이션 직후 NULL 없음)
...
- default_network의 NULL 의미 정책(NULL=환경 기본값 getDefaultNetwork() 런타임 해결, NOT NULL=사용자 지정)이 섹션 4에 명시되어 있다
- default_network nullable 결정이 DB CHECK 제약(IS NULL OR IN ...)과 Zod 검증(NetworkTypeEnum.nullable())으로 구체화되어 있다

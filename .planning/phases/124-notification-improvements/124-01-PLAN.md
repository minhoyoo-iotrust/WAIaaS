...
(e) Delivery Log 테이블 행 클릭 시 메시지 표시 (NOTF-03 UI):
- `selectedLog` 시그널 추가: `useSignal<NotificationLog | null>(null)` — 클릭한 로그 항목
- `NotificationLog` 인터페이스에 `message: string | null` 필드 추가 (Plan 02에서 백엔드 추가)
- Delivery Log Table의 행에 `onRowClick={(log) => { selectedLog.value = selectedLog.value?.id === log.id ? null : log; }}` 핸들러 추가
- selectedLog가 있으면 해당 행 아래에 `<div class="log-message-detail">` 확장 패널 표시:
  - message가 있으면: `<pre style="white-space: pre-wrap; ...">{selectedLog.value.message}</pre>` 형식으로 메시지 원문 표시
  - message가 null이면: "(No message recorded)" 텍스트 표시
- Table 컴포넌트에 onRowClick이 없으면, 행 렌더링 후에 조건부로 확장 행을 렌더링하거나, 기존 Table 대신 직접 `<table>` 렌더링으로 전환하여 row click 핸들러 부착

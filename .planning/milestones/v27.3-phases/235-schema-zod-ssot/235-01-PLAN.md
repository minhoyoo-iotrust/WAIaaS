...
must_haves:
  truths:
    ...
    - "daemon validateSpendingLimitRules() allows USD-only or token_limits-only policies (raw fields not required)"
    - "daemon-local SpendingLimitRules interface has raw fields (instant_max/notify_max/delay_max) as optional"
  artifacts:
    ...
    - path: "packages/daemon/src/api/routes/policies.ts"
      provides: "Updated validateSpendingLimitRules accepting USD-only/token_limits-only policies"
      contains: "validateSpendingLimitRules"
    - path: "packages/daemon/src/pipeline/database-policy-engine.ts"
      provides: "SpendingLimitRules interface with optional raw fields"
      contains: "instant_max?"
  key_links:
    - from: "packages/core/src/schemas/policy.schema.ts"
      to: "packages/core/src/caip/caip19.ts"
      via: "CAIP-19 regex duplicated inline (no import, avoids circular dep)"
      pattern: "CAIP19_REGEX"
    - from: "packages/core/src/schemas/policy.schema.ts"
...
<task type="auto">
  <name>Task 3: Unblock USD-only policy creation at daemon API level</name>
  <files>packages/daemon/src/api/routes/policies.ts, packages/daemon/src/pipeline/database-policy-engine.ts</files>
  <action>
Two daemon files independently require raw fields that block USD-only or token_limits-only policy creation. Update both to be consistent with the Zod schema changes from Task 2.

**Step 1: Update `validateSpendingLimitRules()` in `packages/daemon/src/api/routes/policies.ts` (lines 52-71)**

The current function unconditionally requires `instant_max`, `notify_max`, `delay_max` as digit strings. Replace it with a version that requires **at least one** of: raw fields, USD fields, or token_limits — matching the Zod superRefine logic.
...
Key change: raw fields are validated only when present. At least one limit source must exist.

**Step 2: Update daemon-local `SpendingLimitRules` interface in `packages/daemon/src/pipeline/database-policy-engine.ts` (line 50-62)**

Make raw fields optional to align with the Zod-inferred type:

```typescript
interface SpendingLimitRules {
  instant_max?: string;   // was required, now optional
  notify_max?: string;    // was required, now optional
  delay_max?: string;     // was required, now optional
...
**Important scope note:** Do NOT update the runtime code in `evaluateNativeTier()` or other methods that call `BigInt(rules.instant_max)` — those runtime guards are explicitly Phase 236 scope. Only the interface type and the API validator are updated here.

Run daemon typecheck after changes: `cd /Users/minho.yoo/dev/wallet/WAIaaS && pnpm --filter @waiaas/daemon exec tsc --noEmit`
If TypeScript errors appear from code accessing `rules.instant_max` without optional checks, add `!` non-null assertion or `?.` guard as minimal fix — but only where needed for compilation, not for runtime safety (Phase 236).
  </action>
  <verify>
Run `cd /Users/minho.yoo/dev/wallet/WAIaaS && pnpm --filter @waiaas/daemon exec tsc --noEmit`

Expected: TypeScript compiles clean. If the interface change causes type errors in `evaluateNativeTier` etc., minimal assertion fixes applied.

Manual check: Confirm `validateSpendingLimitRules` no longer throws for `{ instant_max_usd: 100, notify_max_usd: 500, delay_max_usd: 5000 }`.
  </verify>
  <done>validateSpendingLimitRules accepts USD-only and token_limits-only policies. Daemon-local SpendingLimitRules interface has raw fields as optional. Daemon typecheck passes.</done>
</task>
...
2. Run daemon typecheck to verify downstream compatibility: `cd /Users/minho.yoo/dev/wallet/WAIaaS && pnpm --filter @waiaas/daemon exec tsc --noEmit`
   ...
   - **Note:** Daemon runtime BigInt guards (e.g., `BigInt(rules.instant_max)` in `database-policy-engine.ts` which will crash on `undefined` for USD-only policies) are OUT OF SCOPE for this phase. These guards are explicitly covered by Phase 236 (`evaluateNativeTier undefined 방어`). This phase only ensures TypeScript compilation passes; runtime safety for optional raw fields is Phase 236's responsibility.
...
4. Verify daemon API accepts USD-only policy: `validateSpendingLimitRules({ instant_max_usd: 100, notify_max_usd: 500, delay_max_usd: 5000 })` does not throw
</verification>
...
- daemon `validateSpendingLimitRules()` accepts USD-only and token_limits-only policies (SCHM-02 API unblocked)
- daemon-local `SpendingLimitRules` interface raw fields are optional (aligned with Zod SSoT)

...
NOTE: ToastContainer is NOT added here — it will be added in Task 2 after toast.tsx is created.
  </action>
  <verify>
...
6. Verify app.tsx integrates Layout: `grep 'Layout' packages/admin/src/app.tsx` (ToastContainer NOT yet present — added in Task 2)
  </verify>
  <done>Layout component renders sidebar with 5 nav links, header with page title and logout button, content area with hash-based page routing. All 5 page placeholders exist. Error messages map 68 server + 2 client error codes. Format utilities for uptime/date/address work correctly. App.tsx integrates Layout when authenticated (ToastContainer deferred to Task 2).</done>
</task>

<task type="auto">
  <name>Task 2: Create Reusable UI Components (Table, Form, Modal, Toast, CopyButton, EmptyState) + Wire ToastContainer</name>
  <files>
    packages/admin/src/components/table.tsx
    packages/admin/src/components/form.tsx
    packages/admin/src/components/modal.tsx
...
**IMPORTANT**: Use `useSignal` (not `signal`) for per-instance state so each CopyButton instance has independent copied state:
...
**8. Update `packages/admin/src/app.tsx` to add ToastContainer:**

Now that toast.tsx exists, add the ToastContainer to the App component. Update the import and render:

```tsx
import { ToastContainer } from './components/toast';
```

Update the authenticated return in App() from:
```tsx
return <Layout />;
```
to:
...
10. Verify app.tsx now has ToastContainer: `grep 'ToastContainer' packages/admin/src/app.tsx`
11. Build size: `ls -la packages/admin/dist/assets/` — JS bundle should be small (~15-20KB max)
  </verify>
  <done>Seven reusable components created: Table (generic with columns/data/loading/empty), FormField (6 input types with validation error display), Button (4 variants), Badge (5 variants), Modal (overlay with confirm/cancel), Toast (signal-based auto-dismiss), CopyButton (clipboard with useSignal per-instance state), EmptyState (centered message with optional action). ToastContainer wired into app.tsx. All CSS styles appended to global.css. Build succeeds with all components bundled.</done>
</task>

...
2. Create `packages/daemon/src/pipeline/stages.ts`:
   ...
   **Stage 1: Validate + DB INSERT**
   ...
   - INSERT into transactions table: { id: txId, agentId, chain, type: 'TRANSFER', status: 'PENDING', amount: request.amount, toAddress: request.to, createdAt: now, updatedAt: now }
   - Set ctx.txId

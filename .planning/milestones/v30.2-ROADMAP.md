...
**Goal**: AI 에이전트와 관리자가 트랜잭션을 실제 전송 없이 시뮬레이션하여 정책 평가 결과, 예상 수수료, 잔액 변동, 경고를 사전 확인할 수 있다
**Depends on**: Nothing (first phase, no DB migration needed)
**Requirements**: SIM-01, SIM-02, SIM-03, SIM-04, SIM-05
**Success Criteria** (what must be TRUE):
  1. POST /v1/transactions/simulate 호출 시 DryRunSimulationResult(policy/fee/balanceChanges/warnings/simulation/meta)가 반환되며, DB/서명/알림/이벤트 부수 효과가 전혀 발생하지 않는다
...
**Success Criteria** (what must be TRUE):
  ...
  2. waiaas backup CLI 명령으로 백업을 생성하고, waiaas backup list / backup inspect로 오프라인에서 백업을 조회/검사할 수 있다
...
**Success Criteria** (what must be TRUE):
  1. POST /v1/webhooks로 webhook을 등록하면 64-char hex secret이 일회성으로 반환되고, GET /v1/webhooks로 목록 조회(secret 미노출), DELETE로 삭제(CASCADE logs)할 수 있다
...
**Success Criteria** (what must be TRUE):
  1. GET /v1/admin/stats로 7개 카테고리(transactions/sessions/wallets/rpc/autostop/notifications/system) 운영 통계 JSON이 반환된다
  2. IMetricsCounter 인메모리 카운터가 RPC 호출/에러/지연시간, AutoStop 트리거, 트랜잭션 제출/실패를 실시간으로 집계한다
...
### Phase 313.1: Gap Closure (감사 갭 수정)
**Goal**: v30.2 마일스톤 감사에서 발견된 STAT-02/STAT-04 미와이어링, 테스트 assertion 오류, skill 파일 미동기화를 수정한다
...
**Execution Order:**
Phases execute in numeric order: 309 -> 310 -> 311 -> 312 -> 313 -> 313.1

...
- TF-01~TF-05: 토큰 파일 사양 (getMcpTokenPath/writeMcpToken/readMcpToken, write-then-rename, readFileSync, Windows EPERM 재시도, Last-Writer-Wins)
- NOTI-01~NOTI-05: 알림 확장 (데몬 자동 판단, notification_log 중복 방지, OR 논리 트리거, 보완 발송, 순수 함수)
- SM-01~SM-14: SessionManager (단일 클래스, 4 public 메서드, 9 내부 상태, safeSetTimeout, 파일-우선 쓰기, 5종 에러 분기)
- SMGI-D01~D04: MCP 통합 (getState 4번째 메서드, 50ms 대기, 60초 polling, console.error 통일)
- CLI-01~CLI-06: CLI 커맨드 (mcp 서브커맨드 그룹, setup 7단계, refresh-token 8단계, 에이전트 자동 선택)
- TG-01~TG-06: Telegram (9번째 명령어, 인라인 키보드, 2-level constraints, resolveDefaultConstraints)

**Issues Resolved:**

- MCP 세션 토큰 자동 갱신 메커니즘 미설계 → SessionManager + safeSetTimeout + 5종 에러 분기
- CLI/Telegram MCP 토큰 관리 수단 부재 → mcp setup/refresh-token + /newsession
- 세션 만료 임박 알림 미구현 → SESSION_EXPIRING_SOON 이벤트 + OR 논리 + notification_log
- 토큰 파일 동시 쓰기 충돌 → write-then-rename 원자적 패턴 + Last-Writer-Wins
- Claude Desktop 에러 시 연결 해제 → isError 회피 + 안내 메시지 전략

**Issues Deferred:**

- EXT-01: MCP Streamable HTTP transport 세션 관리 (v1.x)
- EXT-02: 다중 MCP 클라이언트 동시 접속 시 토큰 격리 (v1.x)
- EXT-03: agents.default_constraints DB 컬럼 (v1.x)
...
*Archived: 2026-02-09 as part of v0.9 milestone completion*

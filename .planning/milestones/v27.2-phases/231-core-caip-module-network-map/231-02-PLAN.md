...
must_haves:
  truths:
    ...
    - "x402-types.test.ts parseCaip2 error assertions updated to match new Zod-based error message and all tests pass after re-export refactor"
...
**1b. Update `packages/core/src/__tests__/x402-types.test.ts` (lines 81-84):**

The `parseCaip2` error contract changes after the refactor: the old local implementation threw `Error('Invalid CAIP-2 identifier')`, but the new Zod-based `parseCaip2` from `caip/caip2.ts` throws a `ZodError` with message `'Invalid CAIP-2 chain ID format (expected namespace:reference)'`. Update the two assertions at lines 82-83:
...
This uses a substring match so it works with both the Zod error wrapper and the embedded message string. No other tests in x402-types.test.ts need changes -- `CAIP2_TO_NETWORK`, `NETWORK_TO_CAIP2`, `resolveX402Network`, and schema tests all remain identical because the runtime values are unchanged (only the source location moved).
...
4. Verify `x402-types.test.ts` passes after parseCaip2 error message update (backward compatibility)
...
- All tests pass: caip.test.ts (~65 tests), x402-types.test.ts (error assertions updated), package-exports.test.ts (extended)

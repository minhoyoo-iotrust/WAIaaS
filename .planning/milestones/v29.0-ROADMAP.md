...
- [ ] **v29.0 고급 DeFi 프로토콜 설계** -- Phases 268-273 (in progress)
...
### v29.0 고급 DeFi 프로토콜 설계 (In Progress)
...
- [x] **Phase 273: Intent 서명 패턴 설계** - SignableOrder 타입, ActionProviderRegistry 확장, EIP-712 파이프라인, 보안 (completed 2026-02-26)

## Phase Details

### Phase 268: 포지션 인프라 설계
**Goal**: 모든 DeFi 프로토콜이 공유하는 positions 통합 테이블과 PositionTracker 동기화 서비스가 설계되어, 후속 프레임워크가 인프라 수정 없이 포지션을 저장하고 조회할 수 있다
**Depends on**: Nothing (first phase)
**Requirements**: POS-01, POS-02, POS-03, POS-04, POS-05, POS-06
**Success Criteria** (what must be TRUE):
  1. positions 테이블 스키마가 LENDING/YIELD/PERP/STAKING 4개 카테고리를 discriminatedUnion으로 수용하며, DB v25 마이그레이션 SQL이 명세된다
...
**Success Criteria** (what must be TRUE):
  1. IDeFiMonitor 공통 인터페이스가 3개 모니터(HealthFactor/Maturity/Margin)의 공통 패턴(evaluate, getInterval, emitAlert)을 정의한다
...
**Depends on**: Phase 268, Phase 269
**Requirements**: LEND-01, LEND-02, LEND-03, LEND-04, LEND-05, LEND-06, LEND-07, LEND-08, LEND-09
**Success Criteria** (what must be TRUE):
  1. ILendingProvider가 supply/borrow/repay/withdraw 4개 표준 액션과 getPosition/getHealthFactor/getMarkets 3개 조회 메서드를 정의하며, IActionProvider를 확장한다
  2. LendingPosition Zod 스키마(type SUPPLY/BORROW, provider, asset, amount, apy)와 HealthFactor 타입(담보/차입 비율, 임계값 1.2)이 명세된다
...
**Depends on**: Phase 268, Phase 269
**Requirements**: YIELD-01, YIELD-02, YIELD-03, YIELD-04, YIELD-05, YIELD-06
**Success Criteria** (what must be TRUE):
  1. IYieldProvider가 buyPT/buyYT/redeemPT/addLiquidity/removeLiquidity 5개 표준 액션과 getMarkets/getPosition/getYieldForecast 3개 조회 메서드를 정의한다
...
**Success Criteria** (what must be TRUE):
  1. IPerpProvider가 openPosition/closePosition/modifyPosition/addMargin/withdrawMargin 5개 표준 액션과 getPosition/getMarginInfo/getMarkets 3개 조회 메서드를 정의한다
...
**Success Criteria** (what must be TRUE):
  1. SignableOrder Zod 타입이 EIP-712 도메인 파라미터(chainId, verifyingContract, nonce, deadline)를 포함하며, ActionProviderRegistry가 intent 타입을 지원하도록 확장 설계된다
...
**Execution Order:**
Phases execute in numeric order: 268 -> 269 -> 270 -> 271 -> 272 -> 273
...
| 273. Intent 서명 패턴 설계 | 2/2 | Complete    | 2026-02-26 | - |

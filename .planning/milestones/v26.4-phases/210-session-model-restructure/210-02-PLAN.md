...
must_haves:
  truths:
    ...
    - "session-auth 미들웨어가 defaultWalletId와 walletId(하위 호환) 모두를 context에 설정한다"
  artifacts:
    ...
    - path: "packages/daemon/src/api/middleware/session-auth.ts"
      provides: "Updated session-auth setting defaultWalletId + walletId (backward compat)"
...
b. 기존 `c.set('walletId', payload.wlt)` 행을 **듀얼 설정**으로 변경한다. `defaultWalletId`를 새로 추가하되, `walletId`도 하위 호환을 위해 유지한다 (Phase 211에서 `walletId` 설정을 제거).

```typescript
// Line 67 변경 (듀얼 설정 — 하위 호환 유지):
c.set('defaultWalletId', payload.wlt);  // 새 필드: 기본 지갑 ID
c.set('walletId', payload.wlt);         // 하위 호환: Phase 211에서 제거
```

c. JWT의 `wlt` 클레임은 여전히 기본 지갑 ID를 담는다 (하위 호환).

**2. jwt-secret-manager.ts JwtPayload 주석 변경**:

`JwtPayload` 인터페이스의 `wlt` 필드 주석을 업데이트한다:
...
**3. OpenAPI 스키마 변경** (`openapi-schemas.ts`):
...
f. 알림 발송의 walletId 파라미터를 defaultWalletId로 변경한다.
  ...
  <verify>
`cd /Users/minho.yoo/dev/wallet/WAIaaS && pnpm turbo run typecheck --filter=@waiaas/daemon` 가 통과한다. session-auth.ts가 `c.set('defaultWalletId', ...)` + `c.set('walletId', ...)`를 모두 설정하므로 기존 `c.get('walletId')` 참조에서 타입 에러가 발생하지 않는다. 다른 파일에서의 `c.get('walletId')` 일괄 변경은 Phase 211에서 처리한다.
  </verify>
  <done>
POST /sessions가 walletIds/walletId 파라미터를 정규화하고 session_wallets에 행을 삽입한다. GET /sessions가 wallets 배열을 포함한다. PUT /sessions/:id/renew가 session_wallets에서 기본 지갑을 조회하여 JWT를 발급한다. session-auth가 defaultWalletId + walletId(하위 호환) 모두를 context에 설정한다. jwt-secret-manager.ts의 JwtPayload.wlt 주석이 defaultWalletId로 업데이트된다.
  </done>
</task>
...
- `c.get('walletId')` 검증도 유지한다 (하위 호환 듀얼 설정 확인).
- 새 테스트 추가: session-auth가 defaultWalletId와 walletId 모두를 올바르게 설정하는지 확인.
...
c. `c.get('walletId' as never)`를 사용하는 **다른** 파일(wallet.ts, transactions.ts, actions.ts, x402.ts, wc.ts, owner-auth.ts 등)은 Phase 211에서 일괄 변경한다. 이 Plan에서는 sessions.ts와 session-auth.ts 내부에서만 변경하고, 다른 파일은 그대로 유지한다. session-auth 미들웨어가 `defaultWalletId`와 `walletId` 모두를 설정하므로 기존 코드는 정상 동작한다.
  </action>
  <verify>

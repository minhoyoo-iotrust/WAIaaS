# Requirements Archive: v31.12 External Action 프레임워크 구현
...
- [x] **TRACK-01**: AsyncTrackingResult.state 확장 — 기존 4종 + PARTIALLY_FILLED/FILLED/CANCELED/SETTLED/EXPIRED
- [ ] **TRACK-02**: AsyncPollingService 쿼리 확장 — bridge_status/bridge_metadata를 tracking_status/tracking_metadata로 리네임 *(deferred: bridge_status 확장으로 대체)*
- [ ] **TRACK-03**: ExternalActionTracker 구현 — IAsyncStatusTracker 구현체, venue별 상태 확인을 ActionProvider에 위임 *(deferred: AsyncPollingService 직접 확장으로 대체)*
- [x] **TRACK-04**: IActionProvider에 선택적 메서드 추가 — checkStatus(), execute()
- [ ] **TRACK-05**: EventBus 이벤트 — action:status-changed, action:completed, action:failed *(deferred: notification callbacks로 대체)*
- [x] **TRACK-06**: 알림 이벤트 6종 — external_action_partially_filled/filled/settled/canceled/expired/failed
...
| TRACK-01 | Phase 389 | Complete |
| TRACK-02 | Phase 389 | Deferred (bridge_status 확장으로 대체) |
| TRACK-03 | Phase 389 | Deferred (AsyncPollingService 직접 확장) |
| TRACK-04 | Phase 389 | Complete |
| TRACK-05 | Phase 389 | Deferred (notification callbacks로 대체) |
| TRACK-06 | Phase 389 | Complete |

...
- [x] **NOTE-01**: BalanceInfo.amount 단위 변환 규칙 (M2) — lamports/SOL 변환 명시
- [x] **NOTE-02**: 알림 채널 최소 요구 명확화 (M4) — config 표현과 요구사항 일치
- [x] **NOTE-03**: MCP 기능 패리티 매트릭스 (M6) — MCP 6개 도구 ↔ REST 31개 엔드포인트
- [x] **NOTE-04**: SDK 에러 타입 매핑 전략 (M7) — 36개 에러 코드의 SDK 매핑
- [x] **NOTE-05**: Tauri IPC + HTTP 이중 채널 전략 (M8) — 에러 처리 방안
- [x] **NOTE-06**: Setup Wizard vs CLI init 순서 정리 (M9) — 초기화 단계 통일
- [x] **NOTE-07**: Telegram SIWS 서명 방안 (M10) — Tier 2 인증 수행 방법 정의
- [x] **NOTE-08**: Docker graceful shutdown 검증 (M11) — 35초 + 10단계 합산 확인
- [x] **NOTE-09**: 에이전트 생명주기 매핑 (M12) — v0.1 5단계 ↔ v0.2 agents.status
- [x] **NOTE-10**: Python SDK 네이밍 검증 (M13) — snake_case 변환 일관성
- [x] **NOTE-11**: 커서 페이지네이션 파라미터 통일 (M14) — UUID v7 커서 파라미터명
...
| NOTE-04 | Phase 13 | Complete |
| NOTE-05 | Phase 13 | Complete |
| NOTE-06 | Phase 13 | Complete |
| NOTE-07 | Phase 13 | Complete |
| NOTE-08 | Phase 13 | Complete |
| NOTE-09 | Phase 13 | Complete |
| NOTE-10 | Phase 13 | Complete |
| NOTE-11 | Phase 13 | Complete |
...
*Last updated: 2026-02-06 (Phase 13 NOTE-01~11 complete — v0.3 37/37 requirements done)*

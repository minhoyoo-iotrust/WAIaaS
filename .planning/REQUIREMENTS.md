...
- [ ] **MIG-01**: DB v61 마이그레이션이 `wallet_apps(wallet_type) WHERE signing_enabled = 1`에 partial unique index를 생성한다
- [ ] **MIG-02**: 마이그레이션이 같은 wallet_type에 `signing_enabled = 1`인 앱이 여러 개인 경우 `created_at` 가장 빠른 앱만 유지하고 나머지를 비활성화한다
- [ ] **MIG-03**: 마이그레이션이 `signing_enabled IN (0, 1)` CHECK 제약 조건을 추가한다

### Backend

- [ ] **SVC-01**: WalletAppService `update()`에서 `signingEnabled = true` 설정 시 같은 wallet_type의 다른 앱을 트랜잭션 내에서 자동으로 `signing_enabled = 0`으로 변경한다
- [ ] **SVC-02**: WalletAppService `register()`에서 같은 wallet_type에 이미 `signing_enabled = 1`인 앱이 있으면 새 앱을 `signing_enabled = 0`으로 등록한다
- [ ] **SVC-03**: PresetAutoSetupService가 `signing_sdk.preferred_wallet` 설정 대신 `signing_enabled` 컬럼을 사용하도록 변경한다
...
- [ ] **TST-01**: signing_enabled 라디오 토글 시 같은 wallet_type의 다른 앱이 자동으로 0이 되는 단위 테스트
- [ ] **TST-02**: partial unique index가 같은 wallet_type에 signing_enabled = 1 중복 삽입을 거부하는 단위 테스트

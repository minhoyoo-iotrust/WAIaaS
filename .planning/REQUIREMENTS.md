...
- [ ] **PKGS-01**: packages/actions/ 디렉토리 구조가 확정되어 m28-01에서 패키지 스캐폴딩을 바로 생성할 수 있다
- [ ] **PKGS-02**: 내장 프로바이더 등록/해제 라이프사이클이 설계되어 데몬 시작 시 enabled 프로바이더만 로딩하는 방식이 명확하다
- [ ] **PKGS-03**: config.toml [actions.*] 공통 스키마 패턴이 정의되어 프로바이더별 설정 추가 시 일관된 구조를 따른다
- [ ] **PKGS-04**: Admin Settings 런타임 변경 가능 설정 항목(API 키, 슬리피지 등)이 정의되어 config.toml vs Admin Settings 경계가 명확하다

### API 변환 패턴 (DEFI-02)

- [ ] **APIC-01**: ActionApiClient 베이스 패턴(native fetch + AbortController 타임아웃 + Zod 응답 검증)이 설계되어 4개 프로바이더 API 클라이언트의 공통 기반으로 사용할 수 있다
- [ ] **APIC-02**: 외부 REST API 응답 → ContractCallRequest 변환 매핑이 Solana(programId/instructionData/accounts)와 EVM(to/data/value) 양쪽에 대해 정의된다
- [ ] **APIC-03**: DeFi 에러 코드(ACTION_API_ERROR, ACTION_RATE_LIMITED, PRICE_IMPACT_TOO_HIGH)가 기존 에러 체계에 추가 설계된다
- [ ] **APIC-04**: 슬리피지 제어 공통 로직(기본값/상한/클램핑)이 프로바이더별 단위(Jupiter=bps, 0x/LI.FI=pct)를 포함하여 설계된다
- [ ] **APIC-05**: 0x AllowanceHolder 토큰 승인 플로우가 Permit2 대신 설계되어, 기존 APPROVE 파이프라인 타입과의 연동이 명확하다

### 정책 연동 (DEFI-03)

- [ ] **PLCY-01**: ActionProvider → 정책 평가 연동 플로우가 다이어그램으로 정의되어 resolve() 결과가 파이프라인 Stage 2에 도달하는 경로가 명확하다
- [ ] **PLCY-02**: 4개 프로토콜의 CONTRACT_WHITELIST 등록 대상(프로그램/컨트랙트 주소)이 확정되어 구현 시 화이트리스트 설정을 바로 생성할 수 있다
- [ ] **PLCY-03**: 크로스체인 정책 평가 규칙이 확정되어 출발 체인 월렛의 정책으로 평가하는 방식이 명문화된다
- [ ] **PLCY-04**: 크로스체인 도착 주소 정책 검증 설계가 완료되어 목적지 주소 변조 공격을 방지할 수 있다
...
|-------------|-------|--------|
| PKGS-01 | Phase 244 | Pending |
| PKGS-02 | Phase 244 | Pending |
| PKGS-03 | Phase 244 | Pending |
| PKGS-04 | Phase 244 | Pending |
| APIC-01 | Phase 244 | Pending |
| APIC-02 | Phase 244 | Pending |
| APIC-03 | Phase 244 | Pending |
| APIC-04 | Phase 244 | Pending |
| APIC-05 | Phase 244 | Pending |
| PLCY-01 | Phase 244 | Pending |
| PLCY-02 | Phase 244 | Pending |
| PLCY-03 | Phase 244 | Pending |
| PLCY-04 | Phase 244 | Pending |

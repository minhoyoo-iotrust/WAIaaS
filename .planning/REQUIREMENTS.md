...
- [x] **ASNC-01**: AsyncStatusTracker 공통 인터페이스가 설계되어 브릿지/unstake/가스대기 3개 구현체가 동일 패턴을 따를 수 있다
- [x] **ASNC-02**: 폴링 스케줄러 설계(setTimeout 체인, 간격/최대횟수 설정)가 완료되어 BackgroundWorkers에 통합 가능하다
- [x] **ASNC-03**: 통합 DB 마이그레이션 설계(bridge_status + bridge_metadata 컬럼 + GAS_WAITING 상태)가 단일 마이그레이션으로 확정된다
- [x] **ASNC-04**: 트랜잭션 상태 머신이 8-state → 9-state(GAS_WAITING 추가)로 확장 설계되어 전이 다이어그램이 완성된다
- [x] **ASNC-05**: 브릿지 타임아웃 정책이 설계되어 2시간+ 폴링, 자동 취소 방지, BRIDGE_MONITORING 전환이 명확하다

### 테스트 전략 (DEFI-05)

- [x] **TEST-01**: mock API 응답 픽스처 공통 구조가 설계되어 4개 프로바이더 테스트에서 일관된 패턴으로 mock을 구성할 수 있다
- [x] **TEST-02**: 프로바이더 테스트 헬퍼(createMockApiResponse, assertContractCallRequest)가 설계되어 반복 코드를 최소화한다
- [x] **TEST-03**: 4개 프로토콜 × 공통 시나리오 매트릭스가 정의되어 각 프로바이더가 검증해야 할 테스트 목록이 명확하다

### 안전성 설계

- [x] **SAFE-01**: Jito MEV 보호 fail-closed 설계가 완료되어 Jito 미가용 시 공개 멤풀 폴백 없이 실패하는 방식이 확정된다
- [x] **SAFE-02**: stETH 리베이스 대응 아키텍처 결정(stETH vs wstETH)이 완료되어 잔액 추적, 정책 평가, 전액 전송 시나리오에 대한 방침이 확정된다
- [x] **SAFE-03**: 가스 조건부 실행 시 stale calldata 재조회 패턴이 설계되어 대기 후 실행 시 신선한 견적으로 교체하는 플로우가 명확하다
- [x] **SAFE-04**: 외부 API 드리프트 대응 전략(Zod strict 검증 + 버전 고정 + 실패 로깅)이 설계되어 API 변경 시 안전하게 실패하는 방식이 명확하다
...
| ASNC-01 | Phase 245 | Complete |
| ASNC-02 | Phase 245 | Complete |
| ASNC-03 | Phase 245 | Complete |
| ASNC-04 | Phase 245 | Complete |
| ASNC-05 | Phase 245 | Complete |
| TEST-01 | Phase 245 | Complete |
| TEST-02 | Phase 245 | Complete |
| TEST-03 | Phase 245 | Complete |
| SAFE-01 | Phase 245 | Complete |
| SAFE-02 | Phase 245 | Complete |
| SAFE-03 | Phase 245 | Complete |
| SAFE-04 | Phase 245 | Complete |

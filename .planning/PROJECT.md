...
- ~187,250 LOC TypeScript (ESM-only, Node.js 22, Admin UI ~26,000 LOC, Push Relay ~1,782 LOC)
- 4,975 테스트 (core + adapter-solana + adapter-evm + daemon + CLI + SDK + wallet-sdk + MCP + admin + push-relay + actions)
...
**Current Milestone: v28.1 Jupiter Swap (첫 번째 Action Provider 구현체)**

**Goal:** v1.5에서 구축된 Action Provider 프레임워크 위에 Jupiter Swap을 첫 번째 내장 프로바이더로 구현하여, AI 에이전트가 Solana DEX 토큰 스왑을 정책 평가 하에 실행할 수 있는 상태.

**Target features:**
- JupiterSwapActionProvider (IActionProvider 구현체, Quote API v1 + /swap-instructions → ContractCallRequest 변환)
- JupiterApiClient (native fetch, 10초 타임아웃, Zod 스키마 검증)
- 슬리피지 제어 (기본 50bps, 상한 500bps, config.toml 오버라이드)
- Jito MEV 보호 (tip amount 기본 1000 lamports, 블록 엔진 직접 전송)
- priceImpact 1% 초과 거부
- MCP waiaas_jupiter_swap 도구 자동 노출
- SDK executeAction('jupiter_swap') TS/Python 지원
- CONTRACT_WHITELIST 연동 (Jupiter 프로그램 주소 필수)
- config.toml [actions.jupiter_swap] 설정 섹션
...
---
*최종 업데이트: 2026-02-23 after v28.1 milestone*

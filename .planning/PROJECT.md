...
v0.1~v0.4 완료 (2026-02-07). 48개 플랜, 131개 요구사항, 17개 설계 문서 + 5개 대응표 + 11개 테스트 전략 문서.

## 현재 마일스톤: v0.5 인증 모델 재설계 + 개발자 경험 개선
...
**산출물:** 인증 모델 재설계 문서, 세션 갱신 프로토콜, 수정된 설계 문서 11개, CLI 플로우 재설계, DX 개선 스펙
...
- [ ] masterAuth/ownerAuth/sessionAuth 3-tier 인증 책임 분리 스펙
- [ ] 엔드포인트별 인증 맵 재배치 (ownerAuth → 거래승인+KS복구 2곳만)
- [ ] Owner 주소 에이전트 레벨 이동 (agents.owner_address)
- [ ] WalletConnect 선택적 편의 기능 전환
- [ ] 세션 낙관적 갱신 프로토콜 (PUT /renew, 안전 장치, DB 스키마)
- [ ] CLI DX: init 간소화, agent create --owner, --quickstart, --dev 모드
- [ ] actionable 에러 메시지 (hint 필드)
- [ ] MCP 데몬 내장 옵션 검토
- [ ] 11개 설계 문서 수정 반영
...
v0.5 인증 모델 재설계 + DX 개선 시작 (2026-02-07). 인증 3-tier 분리, 세션 갱신, CLI DX.
...
**설계 문서:** 17개 (deliverables 24-40.md) + 5개 대응표 (41-45.md) + 11개 테스트 전략 (41-51.md), ~47,000 lines
...
| masterAuth/ownerAuth 책임 분리 | Owner 서명은 자금 영향 시에만 요구, 로컬 관리는 마스터 패스워드 | — Pending (v0.5) |
| Owner 주소 에이전트별 귀속 | 멀티Owner/멀티체인 자연 지원, 에이전트간 Owner 격리 | — Pending (v0.5) |
| 세션 낙관적 갱신 | 에이전트 자율성 보장 + Owner 사후 거부권 | — Pending (v0.5) |
| WalletConnect 선택적 전환 | 초기 설정 마찰 제거, CLI 수동 서명으로 모든 기능 동작 | — Pending (v0.5) |

---
*최종 업데이트: 2026-02-07 after v0.5 milestone start*

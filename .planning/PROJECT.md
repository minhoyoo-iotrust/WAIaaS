...
<!-- Current Milestone: v30.6 ERC-4337 Account Abstraction 지원 -->

- [ ] SmartAccountService — viem toSoladySmartAccount() 기반 스마트 어카운트 생성/관리
- [ ] SmartAccountPipeline — accountType 분기 (EOA → sign+send / Smart → UserOp+Bundler)
- [ ] Paymaster 가스비 스폰서십 — createPaymasterClient() 연동, 가스비 0 실행
- [ ] BATCH 원자적 실행 — 스마트 어카운트 UserOperation 단일 calldata, atomic 응답 필드
- [ ] Lazy Deployment — CREATE2 예측 주소, 첫 UserOp 시 initCode 포함 배포
- [ ] Admin Settings — Bundler/Paymaster URL, 체인별 오버라이드, API 키 AES-256-GCM 암호화
- [ ] DB Migration v38 — wallets 테이블 account_type/signer_key/deployed/entry_point 컬럼 추가
- [ ] MCP/SDK/CLI 확장 — accountType 조회 응답, CLI --account-type 옵션, SDK createWallet() 파라미터
- [ ] Admin UI — 지갑 생성 폼 accountType 선택, System 페이지 Smart Account 설정 섹션
...
---
*최종 업데이트: 2026-03-03 after v30.2 milestone start*

...
## Current Milestone: v29.10 ntfy 토픽 지갑별 설정 전환

**Goal:** 글로벌 단일 ntfy_topic 설정을 제거하고, wallet_apps 테이블에서 지갑별 sign_topic/notify_topic을 관리하도록 전환하여 Push Relay 구독 토픽과 데몬 발송 토픽을 일치시키고 멀티 지갑 알림 구조를 정리한다.

**Target features:**
- wallet_apps 테이블에 sign_topic/notify_topic 컬럼 추가 (DB migration)
- NtfySigningChannel/WalletNotificationChannel 토픽 소스를 wallet_apps 테이블 기반으로 변경
- NotificationService 글로벌 NtfyChannel 제거
- Admin UI 글로벌 Ntfy 카드 제거 + Human Wallet Apps 토픽 표시/편집
- Admin Settings notifications.ntfy_topic 삭제
- Wallet App REST API에 signTopic/notifyTopic 필드 추가
...
- ~233,440 LOC TypeScript (ESM-only, Node.js 22, Admin UI ~28,000 LOC, Push Relay ~1,782 LOC)
- ~5,737+ 테스트 (core + adapter-solana + adapter-evm + daemon + CLI + SDK + wallet-sdk + MCP + admin + push-relay + actions)
...
- IActionProvider — ESM 플러그인 프레임워크, ActionProviderRegistry, MCP Tool 자동 변환, JupiterSwapActionProvider 내장(5-safety: 슬리피지 클램프/priceImpact 차단/Jito MEV tip/동일 토큰 차단/프로그램 주소 검증)
...
- MCP 25개 내장 도구 (DeFi positions/health-factor 2개 추가) + Action Provider 동적 도구 + 7개 스킬 리소스
...
- [ ] ntfy 토픽 지갑별 설정 전환 — wallet_apps 테이블에서 sign_topic/notify_topic 관리, 글로벌 NtfyChannel 제거
...
**누적:** 76 milestones (v0.1-v29.9), 301 phases, ~680 plans, ~1,924 requirements, 48 설계 문서(24-76 + m29-00), 10 objective 문서, ~233,440 LOC TS, ~5,737+ 테스트
...
---
*최종 업데이트: 2026-03-02 after v29.9 milestone*

...
## Current Milestone: v32.6 성능 + 구조 개선

**Goal:** N+1 쿼리 패턴 해소, 페이지네이션 미적용 엔드포인트 개선, 1,500줄 이상 대형 파일 4개 분할로 API 응답 성능과 코드 유지보수성 동시 개선.

**Target features:**
- N+1 쿼리 해소 (P-1~P-3, P-7: sessions/admin-monitoring/formatTxAmount)
- 세션/정책 API 페이지네이션 (limit/offset + SDK/MCP 연동)
- migrate.ts (3,525줄) 분할 — 10버전 묶음 6파일
- daemon.ts (2,390줄) 분할 — startup/shutdown/pipeline 4파일
- database-policy-engine.ts (2,318줄) 분할 — 정책 타입별 evaluator
- stages.ts (2,330줄) 분할 — 스테이지별 6파일
- Solana mapError 중앙화 + dynamic import 제거
...
---
*최종 업데이트: 2026-03-17 after v32.6 milestone start*

...
## Current Milestone: v27.4 Admin UI UX 개선

**Goal:** Admin UI에 부재한 핵심 기능(트랜잭션 페이지, 토큰 레지스트리 관리, 수신 TX 모니터링 UI)을 추가하고 기존 페이지 UX를 전반 개선

**Target features:**
- 트랜잭션 페이지 신규 (크로스 지갑 조회, 필터, 페이지네이션, txHash 익스플로러 링크)
- 토큰 레지스트리 관리 UI (네트워크별 토큰 목록/추가/삭제)
- 수신 트랜잭션 모니터링 UI (크로스 지갑 수신 TX 뷰어 + 설정 이전)
- 지갑 목록 개선 (검색/필터/잔액 컬럼)
- 지갑 상세 개선 (4탭 구조, 페이지네이션, txHash, USD)
- 대시보드 개선 (승인 대기 카드, 클릭 가능 카드, 네트워크/txHash)
- 알림 로그 필터 (이벤트/채널/상태 필터, 지갑 링크)
- Admin API 신규 2개 (GET /v1/admin/transactions, GET /v1/admin/incoming)
...
---
*최종 업데이트: 2026-02-22 after v27.4 milestone start*

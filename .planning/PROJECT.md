...
## Current Milestone: v32.9 Push Relay 직접 연동 (ntfy.sh 제거)

**Goal:** WAIaaS 데몬이 ntfy.sh를 경유하지 않고 Push Relay 서버에 직접 메시지를 POST하도록 전환. 서명 응답도 long-polling으로 수신. ntfy.sh 의존성 완전 제거.

**Target features:**
- 데몬 → Push Relay HTTP POST 직접 전송 (서명 요청 + 알림)
- Push Relay long-polling 서명 응답 수신 (ntfy SSE 제거)
- Push Relay ntfy.sh 의존성 전면 제거 + sign_responses DB 테이블
- DB v60 마이그레이션 (push_relay_url 컬럼, sign_topic/notify_topic 미참조)
- Core 타입 변경: ResponseChannel push_relay, ApprovalMethod sdk_push
- Admin UI Push Relay URL 관리 + Approval Method 라벨/동작 변경
- Wallet SDK ntfy 함수 deprecated 처리
- 데몬 ntfy 설정 전면 정리 (config/settings/hot-reload)
...
---
*최종 업데이트: 2026-03-18 after v32.9 milestone started*

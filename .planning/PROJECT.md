...
- ~188,500 LOC TypeScript (ESM-only, Node.js 22, Admin UI ~26,000 LOC, Push Relay ~1,782 LOC)
...
## Current Milestone: v28.5 가스비 조건부 실행

**Goal:** 트랜잭션 파이프라인에 가스비 조건부 실행 기능을 추가하여, AI 에이전트가 "가스비가 N gwei 이하일 때만 실행" 같은 비용 최적화 조건을 선언적으로 지정할 수 있는 상태.

**Target features:**
- GasCondition 스키마 (maxGasPrice/maxPriorityFee/timeout) + 기존 GAS_WAITING 상태 활용
- GasConditionEvaluator (Stage 3→4 사이 가스 조건 평가, RPC 가스 가격 조회)
- GasConditionWorker (백그라운드 폴링, 조건 충족 시 파이프라인 재개, 타임아웃 시 CANCELLED)
- Solana maxPriorityFee 조건 (computeUnitPrice 기반)
- TX_GAS_WAITING / TX_GAS_CONDITION_MET 알림 이벤트
- Admin Settings gas_condition 5개 키 + Admin UI Gas Condition 설정 섹션
- MCP/SDK gasCondition 파라미터 노출 + 스킬 파일 갱신

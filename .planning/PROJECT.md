...
## Current Milestone: v33.4 서명 앱 명시적 선택

**Goal:** 같은 wallet_type에 여러 앱이 등록된 경우, 서명 대상 앱을 wallet_type 그룹별 라디오 선택으로 명확하게 지정할 수 있도록 한다.

**Target features:**
- DB v61 마이그레이션 — `wallet_type + signing_enabled` partial unique index
- WalletAppService 백엔드 — signing_enabled 자동 토글 (같은 wallet_type 내 하나만 활성)
- SignRequestBuilder 조회 변경 — `walletName` → `wallet_type + signing_enabled` 기반
- Admin UI — wallet_type별 그룹 표시 + 서명 라디오 버튼 (알림은 체크박스 유지)
- `signing_sdk.preferred_wallet` 설정 deprecated
